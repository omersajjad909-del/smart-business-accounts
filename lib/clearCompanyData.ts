// FILE: lib/clearCompanyData.ts
//
// Empties a company of everything it has recorded, without deleting the
// company itself.
//
// Two places need this and they must not drift: Dev Test Mode's "Clear Data",
// and a demo visitor wiping the workspace they have been trying things in. A
// second copy of a forty-step delete is a second copy that will be one table
// out of date the next time the schema grows, and the symptom of that is a
// foreign key error in front of whoever pressed the button.
//
// The order matters throughout — children before parents, and every table
// whose foreign key has no cascade listed explicitly. That is why this reads
// as a numbered list rather than a loop.
//
// It checks no permission of its own. Callers do, and they check different
// things: dev test wants an admin in test mode, the demo wants a company
// actually flagged isDemo. Putting that here would mean one of them asking
// the wrong question.
...
export async function clearCompanyData(
  ...
  opts: {
    /**
     * An ActivityLog action to spare — the marker that says what this company
     * is. Clearing a workspace must not erase the fact that it is a test
     * workspace, or nothing afterwards knows what it is looking at.
     */
    keepActivityLogAction?: string;
  } = {},
): Promise<void> {
  const { keepActivityLogAction } = opts;
  ...
  await prisma.$transaction(async (tx) => {
    ...
    await tx.activityLog.deleteMany({
      where: keepActivityLogAction
        ? { companyId, action: { not: keepActivityLogAction } }
        : { companyId },
    });
  }, { timeout: 60_000 });
}
