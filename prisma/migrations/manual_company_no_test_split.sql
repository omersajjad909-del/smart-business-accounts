...
-- column default and remains customer-only, alongside the demo split.
--
...
-- ─────────────────────────────────────────────────────────────────────────────
-- OPTIONAL — closes the gap that already exists. Read before running.
--
...
-- numbering stays #100004, #100015, …
--
-- The block below renumbers every real customer contiguously from #100001,
-- oldest first — the same thing the demo migration did. The cost is that a
-- customer's visible Company ID changes: A-H-R Trader would stop being #100015.
-- With a handful of customers that is usually fine; once IDs have been quoted
-- in support threads or on paperwork, it is not. Your call — uncomment to run.
--
-- UPDATE "Company" SET "companyNo" = -"companyNo"
