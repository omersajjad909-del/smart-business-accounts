...
exports.FUNCTIONS = exports.FormulaError = void 0;
...
function parse(tokens) {
    ...
    function parsePrimary() {
        if (++depth > MAX_DEPTH)
            throw new FormulaError("Expression is nested too deeply");
        try {
            const t = peek();
            if (!t)
                ...
                if (peek()?.type !== "]") {
                    for (;;) {
                        ...
                        if (items.length > MAX_LIST_LENGTH)
                            throw new FormulaError("List is too long");
                        if (peek()?.type === ",") {
                            next();
                            continue;
                        }
...
exports.FUNCTIONS = [
    ...
    },
    {
        ...
        apply: (a) => {
            ...
            if (piece <= 0)
                throw new FormulaError("fitCount() needs a piece size greater than zero");
    ...
    },
    {
        ...
        description: "The stock size that wastes least when the piece is packed across it. " +
    ...
    },
    {
        ...
        description: "Whole multiplier that brings the value into a working range. " +
        ...
        apply: (a) => {
            ...
            if (v <= 0)
                throw new FormulaError("scaleToRange() needs a value greater than zero");
            if (lo > hi)
                throw new FormulaError("scaleToRange() minimum is greater than its maximum");
            ...
            throw new FormulaError(`No whole multiple of ${v} lands between ${lo} and ${hi}`);
        },
    },
    {
        name: "convert", arity: [3, 3], signature: "convert(value, from, to)",
        description: "Unit conversion. Units are written as bare words: convert(100, m, in). " +
...
function bestFit(args, fn) {
    const piece = num(args[0], fn, 0);
    const sizes = list(args[1], fn, 1);
    if (piece <= 0)
        throw new FormulaError(`${fn}() needs a piece size greater than zero`);
    let best = null;
    for (const stock of [...sizes].sort((x, y) => x - y)) {
        ...
        if (!best || waste < best.waste - 1e-9)
            best = { stock, count, waste };
    }
    if (!best)
        throw new FormulaError(`No stock size in the list fits a piece of ${piece}`);
    return { stock: best.stock, count: best.count };
}
const FUNCTION_MAP = new Map(exports.FUNCTIONS.map((f) => [f.name, f]));
/* ────────────────────────────── Evaluator ─────────────────────────── */
    ...
    switch (node.kind) {
        ...
        case "list":
            return node.items.map((item) => {
                ...
                if (typeof v !== "number")
                    throw new FormulaError("Lists can only hold numbers");
                return v;
            });
        case "var": {
        ...
        case "binary": {
            ...
            switch (node.op) {
                ...
                case "%":
                    if (r === 0)
                        throw new FormulaError("Remainder by zero");
        ...
        case "call": {
            ...
            if (fn.name === "convert") {
                ...
                if (!UNIT_TO_BASE[from] || !UNIT_TO_BASE[to]) {
                    throw new FormulaError(`convert() units must be one of: ${Object.keys(UNIT_TO_BASE).join(", ")}`);
                }
...
function validateKey(key) {
    ...
    if (RESERVED.has(key))
        return `"${key}" is a built-in name — pick another`;
    return null;
}
 ...
 */
function runFormula(formula, provided = {}) {
    const scope = new Map();
    const steps = [];
    ...
    for (const step of formula.steps) {
        try {
            ...
            if (!firstError)
                firstError = `${step.label || step.key}: ${message}`;
            steps.push({
                key: step.key,
                label: step.label || step.key,
                expression: step.expression,
