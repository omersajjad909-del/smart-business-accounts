...
export default function NotFoundPage() {
  ...
  return (
    <main style={{
      ...
      color:"var(--ink-solid, white)", fontFamily:"'DM Sans','Outfit',system-ui,sans-serif",
      ...
      <div style={{ maxWidth:560, position:"relative", zIndex:1 }}>
        ...
        <div style={{
          ...
          background:"linear-gradient(135deg,rgba(var(--txr-6366f1, 99,102,241),.3),rgba(var(--txr-818cf8, 129,140,248),.1))",
        ...
        <p style={{
          fontSize:15, color:"rgba(var(--ink),var(--ta-45, .45))", lineHeight:1.75,
        ...
        <div style={{
          ...
          marginBottom:36, fontSize:12, color:"rgba(var(--ink),var(--ta-50, .5))",
          ...
          Redirecting to home in <strong style={{color:"var(--tx-818cf8, #818cf8)"}}>{count}s</strong>
        </div>
        ...
        <div style={{
          ...
          {QUICK_LINKS.map(l=>(
            <Link key={l.href} href={l.href} style={{ textDecoration:"none", display:"flex", flexDirection:"column", alignItems:"center", gap:8, padding:"16px 12px", borderRadius:14, background:"rgba(var(--ink),.03)", border:"1px solid rgba(var(--ink),.07)", transition:"all .15s" }}
              ...
              onMouseLeave={e=>{ (e.currentTarget as HTMLElement).style.background="rgba(var(--ink),.03)"; (e.currentTarget as HTMLElement).style.borderColor="rgba(var(--ink),.07)"; (e.currentTarget as HTMLElement).style.transform="translateY(0)"; }}>
              <span style={{ fontSize:22 }}>{l.icon}</span>
              <span style={{ fontSize:12, fontWeight:600, color:"rgba(var(--ink),var(--ta-60, .6))" }}>{l.label}</span>
            </Link>
          ))}
        </div>
        ...
        <div style={{ marginTop:36, opacity:vis?1:0, transition:"opacity .6s ease .4s" }}>
          <p style={{ fontSize:13, color:"rgba(var(--ink),var(--ta-25, .25))", margin:"0 0 12px" }}>Or search for what you need</p>
          <div style={{ display:"flex", gap:10, maxWidth:380, margin:"0 auto" }}>
            <input placeholder="Search FinovaOS..." style={{ flex:1, padding:"11px 16px", borderRadius:10, background:"rgba(var(--ink),.05)", border:"1px solid rgba(var(--ink),.1)", color:"var(--ink-solid, white)", fontSize:13, outline:"none" }}
      ...
      <div style={{ position:"absolute", bottom:32, left:"50%", transform:"translateX(-50%)", fontSize:13, color:"rgba(var(--ink),var(--ta-15, .15))", fontWeight:700, letterSpacing:".08em" }}>
        FINOVA
      </div>
