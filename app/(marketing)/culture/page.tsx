...
const DEI_STATS = [
  { value:"47%", label:"Women on leadership team",  color:"var(--tx-f9a8d4, #f9a8d4)" },
  { value:"18",  label:"Nationalities in the team", color:"var(--tx-818cf8, #818cf8)" },
  { value:"38%", label:"Underrepresented backgrounds", color:"var(--tx-34d399, #34d399)" },
  { value:"100%",label:"Pay equity across genders",  color:"var(--tx-fbbf24, #fbbf24)" },
];
          ...
          {TEAM_MEMBERS.slice(0,8).map((m,i)=>{
            ...
            return (
              <div key={m.name} style={{ position:"absolute", ...pos,
                ...
                <div style={{ width:44, height:44, borderRadius:12, background:m.g, display:"flex", alignItems:"center", justifyContent:"center", fontSize:12, fontWeight:800, color:"white", border:"2px solid rgba(var(--ink),.1)", boxShadow:"0 4px 20px rgba(0,0,0,.4)" }}>
                  {m.avatar}
                </div>
                <div style={{ fontSize:9, color:"rgba(var(--ink),var(--ta-30, .3))", textAlign:"center", marginTop:4 }}>{m.country}</div>
              </div>
            );
          })}
        </div>
          ...
          <div style={{ display:"flex", flexWrap:"wrap", gap:12, justifyContent:"center" }}>
            {TEAM_MEMBERS.map(m=>(
              <div key={m.name} style={{ display:"flex", flexDirection:"column", alignItems:"center", gap:6 }}>
                <div style={{ width:56, height:56, borderRadius:16, background:m.g, display:"flex", alignItems:"center", justifyContent:"center", fontSize:14, fontWeight:800, color:"white", border:"2px solid rgba(var(--ink),.08)", transition:"transform .2s", cursor:"default" }}
                  onMouseEnter={e=>e.currentTarget.style.transform="scale(1.1)"}
                  onMouseLeave={e=>e.currentTarget.style.transform="scale(1)"}
                >
                  {m.avatar}
                </div>
                <div style={{ fontSize:10, color:"rgba(var(--ink),var(--ta-50, .5))", fontWeight:600 }}>{m.name}</div>
                <div style={{ fontSize:9, color:"rgba(var(--ink),var(--ta-25, .25))" }}>{m.country} {m.role}</div>
              </div>
            ))}
            <div style={{ display:"flex", flexDirection:"column", alignItems:"center", justifyContent:"center", gap:4 }}>
              <div style={{ width:56, height:56, borderRadius:16, background:"rgba(99,102,241,.15)", border:"1.5px dashed rgba(99,102,241,.4)", display:"flex", alignItems:"center", justifyContent:"center", fontSize:20, color:"var(--tx-818cf8, #818cf8)" }}>+</div>
              <div style={{ fontSize:10, color:"var(--tx-818cf8, #818cf8)", fontWeight:700 }}>108 more</div>
            </div>
          </div>
        </div>
      </Section>
      ...
      <Section>
        <div style={{ maxWidth:900, margin:"0 auto", padding:"0 24px 100px" }}>
          <div style={{ textAlign:"center", marginBottom:48 }}>
            ...
            <p style={{ fontSize:14, color:"rgba(var(--ink),var(--ta-40, .4))" }}>No two days are identical — but here's a realistic picture.</p>
          </div>
            ...
            {DAY_IN_LIFE.map((d,i)=>(
              <button key={d.role} onClick={()=>setActiveDay(i)}
                style={{ padding:"8px 20px", borderRadius:20, fontSize:12, fontWeight:700, cursor:"pointer", border:"none",
                  background:activeDay===i?"linear-gradient(135deg,#4f46e5,#7c3aed)":"rgba(var(--ink),.05)",
                  color:activeDay===i?"white":"rgba(var(--ink),var(--ta-40, .4))" }}>
                {d.role}
              </button>
            ))}
          </div>
          ...
          {(() => {
            ...
            return (
              <div style={{ background:"rgba(var(--ink),.03)", borderRadius:20, border:"1px solid rgba(var(--ink),.07)", overflow:"hidden" }}>
                {/* Header */}
                <div style={{ padding:"24px 28px", borderBottom:"1px solid rgba(var(--ink),.07)", display:"flex", alignItems:"center", gap:16, background:"rgba(99,102,241,.06)" }}>
                  ...
                  <div>
                    <div style={{ fontSize:16, fontWeight:700, color:"var(--ink-solid, white)" }}>{d.name}</div>
                    <div style={{ fontSize:12, color:"var(--tx-818cf8, #818cf8)", marginTop:2, fontWeight:600 }}>{d.role}</div>
                    <div style={{ fontSize:11, color:"rgba(var(--ink),var(--ta-35, .35))", marginTop:1 }}>📍 {d.location}</div>
                  </div>
                </div>
                {/* Schedule */}
                <div style={{ padding:"8px 0" }}>
                  {d.schedule.map((s,i)=>(
                    <div key={i} style={{ display:"flex", gap:20, padding:"14px 28px", borderBottom:i<d.schedule.length-1?"1px solid rgba(var(--ink),.04)":"none", alignItems:"center" }}>
                      <div style={{ width:60, fontSize:11, fontWeight:800, color:"var(--tx-818cf8, #818cf8)", flexShrink:0, fontFamily:"monospace" }}>{s.time}</div>
                      <div style={{ width:2, height:2, borderRadius:"50%", background:"rgba(99,102,241,.4)", flexShrink:0 }}/>
                      <div style={{ fontSize:13, color:"rgba(var(--ink),var(--ta-65, .65))", lineHeight:1.5 }}>{s.activity}</div>
                    </div>
                  ))}
                </div>
              </div>
      ...
      <Section>
        <div style={{ maxWidth:1060, margin:"0 auto", padding:"0 24px 100px" }}>
          <div style={{ textAlign:"center", marginBottom:52 }}>
            ...
            <p style={{ fontSize:14, color:"rgba(var(--ink),var(--ta-40, .4))", maxWidth:520, margin:"0 auto" }}>
          ...
          <div className="culture-4col" style={{ display:"grid", gridTemplateColumns:"repeat(4,1fr)", gap:14, marginBottom:32 }}>
            {DEI_STATS.map(s=>(
              <div key={s.label} style={{ background:"rgba(var(--ink),.03)", borderRadius:16, border:"1px solid rgba(var(--ink),.07)", padding:"24px 20px", textAlign:"center" }}>
                ...
                <div style={{ fontSize:12, color:"rgba(var(--ink),var(--ta-40, .4))", marginTop:6, lineHeight:1.4 }}>{s.label}</div>
              </div>
            ))}
          </div>
          ...
          <div style={{ display:"grid", gridTemplateColumns:"repeat(auto-fit,minmax(220px,1fr))", gap:14 }}>
            {DEI_COMMITMENTS.map(c=>(
              <div key={c.title} style={{ padding:"22px 20px", borderRadius:16, background:"rgba(var(--ink),.03)", border:"1px solid rgba(var(--ink),.07)" }}>
                <div style={{ fontSize:26, marginBottom:10 }}>{c.icon}</div>
                <div style={{ fontSize:14, fontWeight:700, color:"var(--ink-solid, white)", marginBottom:6 }}>{c.title}</div>
                <div style={{ fontSize:12, color:"rgba(var(--ink),var(--ta-40, .4))", lineHeight:1.65 }}>{c.desc}</div>
              </div>
            ))}
          </div>

          <div style={{ marginTop:24, padding:"18px 22px", borderRadius:14, background:"rgba(249,168,212,.06)", border:"1px solid rgba(249,168,212,.2)", fontSize:13, color:"rgba(var(--ink),var(--ta-50, .5))", lineHeight:1.7 }}>
