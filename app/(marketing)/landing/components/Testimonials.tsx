...
export function TestimonialCard({ t, i, vis }: { t: Testimonial; i: number; vis: boolean }) {
  ...
  return (
    ...
    >
      {t.featured && (
        <div style={{ position:"absolute", top:14, right:14, padding:"3px 9px", borderRadius:20, background:"rgba(251,191,36,.12)", border:"1px solid rgba(251,191,36,.25)", fontSize:9, fontWeight:700, color:"var(--tx-fbbf24, #fbbf24)" }}>⭐ FEATURED</div>
      )}
      ...
      <div style={{ display:"flex", gap:2 }}>
        {[1,2,3,4,5].map(n => (
          <svg key={n} width="12" height="12" viewBox="0 0 24 24" fill={n<=t.rating?"#fbbf24":"rgba(var(--ink),.1)"}>
            <polygon points="12 2 15.09 8.26 22 9.27 17 14.14 18.18 21.02 12 17.77 5.82 21.02 7 14.14 2 9.27 8.91 8.26 12 2"/>
          </svg>
        ))}
      </div>

      <p style={{ fontSize:14, color:"rgba(var(--ink),var(--ta-68, .68))", lineHeight:1.8, margin:0, flex:1, fontStyle:"italic" }}>
        {t.message}
      </p>
      ...
      <div style={{ display:"flex", alignItems:"center", gap:12, borderTop:"1px solid rgba(var(--ink),.06)", paddingTop:14 }}>
        ...
        <div>
          <div style={{ fontSize:13, fontWeight:700, color:"rgba(var(--ink),.9)" }}>{t.name}</div>
          <div style={{ fontSize:11, color:"rgba(var(--ink),var(--ta-38, .38))", marginTop:1 }}>{[t.role, t.company].filter(Boolean).join(" · ")}</div>
        </div>
      </div>
    </div>
  );
      ...
      <div ref={ref} style={{maxWidth:1160,margin:"0 auto",position:"relative"}}>
        ...
        <div style={{
          ...
          <div style={{ display:"inline-flex",alignItems:"center",gap:8, padding:"6px 16px",borderRadius:100,marginBottom:20, background:"rgba(251,191,36,.1)",border:"1.5px solid rgba(251,191,36,.22)" }}>
            ...
            <span style={{fontSize:11,fontWeight:700,color:"var(--tx-fbbf24, #fbbf24)",letterSpacing:".08em"}}>
          ...
          {total > 0 && (
            <div className="testi-proof" style={{display:"flex",alignItems:"center",justifyContent:"center",gap:16,flexWrap:"wrap"}}>
              <span style={{fontSize:13,color:"rgba(var(--ink),var(--ta-40, .4))"}}>{total} verified review{total !== 1 ? "s" : ""}</span>
            </div>
          )}
        </div>
          ...
          <div style={{
            ...
            <div style={{ display:"flex", alignItems:"center", gap:12, marginTop:22, paddingTop:18, borderTop:"1px solid rgba(var(--ink),.07)" }}>
              ...
              <div style={{ textAlign:"left" }}>
                <div style={{ fontSize:13, fontWeight:700, color:"rgba(var(--ink),.9)" }}>Umer Sajjad</div>
                <div style={{ fontSize:11, color:"rgba(var(--ink),var(--ta-38, .38))", marginTop:1 }}>Founder · FinovaOS</div>
              </div>
            </div>
          </div>
        )}
        ...
        {display.length === 0 && (
          <div style={{ textAlign:"center", opacity:vis?1:0, transition:"opacity .6s ease .25s" }}>
            <div style={{ fontSize:13, color:"rgba(var(--ink),var(--ta-35, .35))", marginBottom:16 }}>
            ...
            <div style={{ display:"inline-flex", gap:12, flexWrap:"wrap", justifyContent:"center" }}>
              <Link href="/demo" style={{
                ...
                border:"1.5px solid rgba(var(--ink),.12)", background:"rgba(var(--ink),.04)",
                color:"rgba(var(--ink),.75)", textDecoration:"none", transition:"all .25s",
              }}
                onMouseEnter={e=>{e.currentTarget.style.borderColor="rgba(var(--ink),.28)";e.currentTarget.style.color="var(--ink-solid, white)";}}
                onMouseLeave={e=>{e.currentTarget.style.borderColor="rgba(var(--ink),.12)";e.currentTarget.style.color="rgba(var(--ink),.75)";}}
              >
                See the live demo
              </Link>
        ...
        {hasMore && (
          <div style={{ textAlign:"center", opacity:vis?1:0, transition:"opacity .6s ease .4s" }}>
            <Link href="/testimonials" style={{
              ...
              onMouseEnter={e=>{e.currentTarget.style.borderColor="rgba(var(--ink),.28)";e.currentTarget.style.color="var(--ink-solid, white)";e.currentTarget.style.background="rgba(var(--ink),.08)";}}
              onMouseLeave={e=>{e.currentTarget.style.borderColor="rgba(var(--ink),.12)";e.currentTarget.style.color="rgba(var(--ink),var(--ta-70, .7))";e.currentTarget.style.background="rgba(var(--ink),.04)";}}
            >
