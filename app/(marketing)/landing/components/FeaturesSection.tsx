...
function PLVisual({ vis }: { vis: boolean }) {
  const rows = [
    { label: "Revenue",   val: "$248,000", change: "+18.2%", up: true,  color: "var(--tx-10b981, #10b981)" },
    { label: "COGS",      val: "$91,000",  change: "+4.1%",  up: false, color: "var(--tx-f87171, #f87171)" },
    { label: "Gross Profit", val: "$157,000", change: "+28%", up: true, color: "var(--tx-a5b4fc, #a5b4fc)" },
    { label: "Expenses",  val: "$42,000",  change: "-3.2%",  up: true,  color: "var(--tx-34d399, #34d399)" },
    { label: "Net Profit",val: "$115,000", change: "+31%",   up: true,  color: "var(--tx-818cf8, #818cf8)" },
  ];
  return (
    <div style={{
      borderRadius: 20, overflow: "hidden",
      background: "rgba(var(--dkr-0a0c1e, 10,12,30),0.9)", border: "1px solid rgba(var(--ink),.09)",
      ...
      <div style={{ padding: "18px 18px 20px" }}>
        <div style={{ fontSize: 11, color: "rgba(var(--ink),var(--ta-30, .3))", fontWeight: 600, letterSpacing: ".07em", marginBottom: 12 }}>PERIOD: JAN–MAR 2025</div>
        <div style={{ display: "flex", flexDirection: "column", gap: 6 }}>
          {rows.map((r, i) => (
            <div key={r.label} style={{
              display: "flex", justifyContent: "space-between", alignItems: "center",
              padding: "10px 12px", borderRadius: 10,
              background: i === rows.length - 1 ? "rgba(129,140,248,.1)" : "rgba(var(--ink),.03)",
              border: `1px solid ${i === rows.length - 1 ? "rgba(129,140,248,.2)" : "rgba(var(--ink),.06)"}`,
              opacity: vis ? 1 : 0,
              transform: vis ? "translateX(0)" : "translateX(16px)",
              transition: `all .4s ease ${i * 80}ms`,
            }}>
              <span style={{ fontSize: 12, color: "rgba(var(--ink),var(--ta-55, .55))", fontWeight: 500 }}>{r.label}</span>
              <div style={{ display: "flex", alignItems: "center", gap: 10 }}>
                ...
                <span style={{ fontSize: 10, fontWeight: 700, color: r.up ? "var(--tx-34d399, #34d399)" : "var(--tx-f87171, #f87171)", minWidth: 40, textAlign:"right" }}>{r.change}</span>
              </div>
            </div>
          ))}
        </div>
...
function InvoiceVisual({ vis }: { vis: boolean }) {
  const invoices = [
    { id:"INV-1042", client:"Alpha Trading Co.", amount:"$12,400", status:"Paid",    color:"var(--tx-34d399, #34d399)" },
    { id:"INV-1041", client:"Beta Distributors", amount:"$8,750",  status:"Pending", color:"var(--tx-fbbf24, #fbbf24)" },
    { id:"INV-1040", client:"Gamma Retail Ltd.", amount:"$5,200",  status:"Paid",    color:"var(--tx-34d399, #34d399)" },
    { id:"INV-1039", client:"Delta Supplies",    amount:"$3,900",  status:"Overdue", color:"var(--tx-f87171, #f87171)" },
  ];
  return (
    <div style={{
      borderRadius: 20, overflow: "hidden",
      ...
      <div style={{padding:"14px 16px 18px",display:"flex",flexDirection:"column",gap:6}}>
        {items.map((item,i)=>(
          <div key={item.sku} style={{
            ...
            background: item.ok ? "rgba(var(--ink),.03)" : "rgba(248,113,113,.06)",
            border:`1px solid ${item.ok ? "rgba(var(--ink),.06)" : "rgba(248,113,113,.18)"}`,
            opacity: vis ? 1 : 0,
            transform: vis ? "translateX(0)" : "translateX(16px)",
            transition:`all .4s ease ${i*80}ms`,
          }}>
            <div style={{display:"flex",justifyContent:"space-between",alignItems:"flex-start",marginBottom:6}}>
              <div>
                <div style={{fontSize:11,fontWeight:700,color:"rgba(var(--ink),.85)"}}>{item.name}</div>
                <div style={{fontSize:9.5,color:"rgba(var(--ink),var(--ta-30, .3))",marginTop:1}}>SKU: {item.sku}</div>
              </div>
              <div style={{textAlign:"right"}}>
                <div style={{fontSize:12,fontWeight:800,color:"var(--tx-a5b4fc, #a5b4fc)",fontFamily:"'Lora',serif"}}>{item.val}</div>
                {!item.ok && <div style={{fontSize:9,color:"var(--tx-f87171, #f87171)",fontWeight:700,marginTop:1}}>LOW STOCK</div>}
              </div>
            </div>
            <div style={{display:"flex",alignItems:"center",gap:8}}>
              <div style={{flex:1,height:4,borderRadius:2,background:"rgba(var(--ink),.08)",overflow:"hidden"}}>
              ...
              <span style={{fontSize:10,color:"rgba(var(--ink),var(--ta-40, .4))",fontWeight:600,minWidth:50}}>{item.stock} units</span>
            </div>
          </div>
        ))}
      </div>
...
function BranchVisual({ vis }: { vis: boolean }) {
  ...
  return (
    <div style={{
      ...
      {/* Consolidated total */}
      <div style={{padding:"14px 16px 10px",borderBottom:"1px solid rgba(var(--ink),.05)"}}>
        <div style={{fontSize:10,color:"rgba(var(--ink),var(--ta-30, .3))",fontWeight:600,letterSpacing:".07em",marginBottom:6}}>CONSOLIDATED REVENUE</div>
        <div style={{display:"flex",alignItems:"baseline",gap:12}}>
          <div style={{fontSize:28,fontWeight:800,color:"var(--ink-solid, white)",fontFamily:"'Lora',serif",letterSpacing:"-.8px"}}>$270k</div>
          <div style={{fontSize:12,color:"var(--tx-34d399, #34d399)",fontWeight:700}}>↑ +22% vs last quarter</div>
        </div>
      </div>
        ...
        {branches.map((b,i)=>(
          <div key={b.name} style={{
            ...
            <div style={{display:"flex",alignItems:"center",gap:8}}>
              ...
              <span style={{fontSize:11,fontWeight:600,color:"rgba(var(--ink),.75)"}}>{b.name}</span>
            </div>
            <div style={{display:"flex",gap:16}}>
              <div style={{textAlign:"right"}}>
                <div style={{fontSize:11,fontWeight:700,color:"var(--tx-a5b4fc, #a5b4fc)"}}>{b.revenue}</div>
                <div style={{fontSize:9,color:"rgba(var(--ink),var(--ta-30, .3))"}}>Revenue</div>
              </div>
              <div style={{textAlign:"right"}}>
                <div style={{fontSize:11,fontWeight:700,color:"var(--tx-34d399, #34d399)"}}>{b.profit}</div>
                <div style={{fontSize:9,color:"rgba(var(--ink),var(--ta-30, .3))"}}>Profit</div>
              </div>
            </div>
          </div>
        ))}
    ...
    <div style={{
      ...
      <div style={{ padding: "16px 18px 20px" }}>
        <div style={{ display: "flex", alignItems: "center", gap: 10, marginBottom: 16 }}>
          ...
          <div>
            <div style={{ fontSize: 12, fontWeight: 700, color: "var(--ink-solid, white)" }}>AI Financial Assistant</div>
            <div style={{ fontSize: 10, color: "rgba(var(--ink),var(--ta-35, .35))" }}>Analyzing your business data…</div>
          </div>
          <div style={{ marginLeft: "auto", textAlign: "right" }}>
            <div style={{ fontSize: 20, fontWeight: 800, color: "var(--tx-a78bfa, #a78bfa)" }}>87</div>
            <div style={{ fontSize: 9, color: "rgba(var(--ink),var(--ta-30, .3))" }}>Health Score</div>
          </div>
        </div>
...
const FEATURES = [
  {
    tag: "Finance", color: "var(--tx-818cf8, #818cf8)",
  ...
  {
    tag: "Trade Flow", color: "var(--tx-34d399, #34d399)",
  ...
  {
    tag: "Purchasing", color: "var(--tx-fbbf24, #fbbf24)",
  ...
  {
    tag: "Scale", color: "var(--tx-06b6d4, #06b6d4)",
  ...
  {
    tag: "AI Intelligence", color: "var(--tx-a78bfa, #a78bfa)",
...
function FeatureRow({ f, i }: { f: typeof FEATURES[0]; i: number }) {
  ...
  const text = (
    <div className="feat-text" style={{
      ...
      <p style={{fontSize:15.5,color:"rgba(var(--ink),var(--ta-45, .45))",lineHeight:1.85,marginBottom:28,maxWidth:440}}>
        {f.desc}
      </p>
      <div style={{display:"flex",flexDirection:"column",gap:10,marginBottom:32}}>
        {f.bullets.map((b,bi)=>(
          <div key={bi} style={{display:"flex",alignItems:"center",gap:10}}>
            ...
            <span style={{fontSize:14,color:"rgba(var(--ink),var(--ta-65, .65))",fontWeight:500}}>{b}</span>
          </div>
        ))}
      </div>
      ...
      <div style={{maxWidth:1160,margin:"0 auto",position:"relative"}}>
        ...
        <div ref={hRef} style={{
          ...
          <div style={{
            ...
            <span style={{fontSize:11,fontWeight:700,color:"var(--tx-a5b4fc, #a5b4fc)",letterSpacing:".08em"}}>BUILT FOR TRADE</span>
          </div>
          <h2 style={{
            fontFamily:"'Lora',serif",
          ...
          <p style={{fontSize:16,color:"rgba(var(--ink),var(--ta-40, .4))",lineHeight:1.8,maxWidth:560,margin:"0 auto"}}>
