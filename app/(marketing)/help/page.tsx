    ...
    <div ref={ref} id={cat.id} style={{
      ...
      <div style={{ padding: "22px 22px 0" }}>
        <div style={{ display: "flex", alignItems: "flex-start", gap: 14, marginBottom: 18 }}>
          ...
          <div>
            <h3 style={{ fontFamily: "'Lora',serif", fontSize: 16, fontWeight: 700, color: "var(--ink-solid, white)", marginBottom: 3 }}>{cat.title}</h3>
            <p style={{ fontSize: 12, color: "rgba(var(--ink),var(--ta-35, .35))", fontWeight: 400 }}>{cat.desc}</p>
          </div>
          <div style={{
          ...
          {shown.map((a, i) => (
            <Link
              ...
              <svg width="12" height="12" viewBox="0 0 24 24" fill="none"
                stroke={hovered === i ? cat.color : "rgba(var(--ink),.2)"} strokeWidth="2.5"
                style={{ flexShrink: 0, transition: "stroke .2s" }}>
                <path d="M14 2H6a2 2 0 0 0-2 2v16a2 2 0 0 0 2 2h12a2 2 0 0 0 2-2V8z"/>
                <polyline points="14 2 14 8 20 8"/>
              </svg>
              <span style={{ flex: 1, fontSize: 13, color: hovered === i ? "var(--ink-solid, white)" : "rgba(var(--ink),var(--ta-60, .6))", fontWeight: 500, transition: "color .2s" }}>
                {a.title}
              </span>
              {a.popular && (
              ...
              <span style={{ fontSize: 10.5, color: "rgba(var(--ink),var(--ta-20, .2))", flexShrink: 0, fontWeight: 500 }}>{a.time}</span>
              <svg width="10" height="10" viewBox="0 0 24 24" fill="none"
                stroke={hovered === i ? cat.color : "rgba(var(--ink),.15)"} strokeWidth="2.5"
                style={{ flexShrink: 0, transition: "stroke .2s" }}>
      ...
      {cat.articles.length > 4 && (
        <button onClick={() => setExpanded(e => !e)} style={{
          width: "100%", padding: "12px", background: "rgba(var(--ink),.02)",
          borderTop: "1px solid rgba(var(--ink),.06)", border: "none",
          ...
          onMouseLeave={e => (e.currentTarget.style.background = "rgba(var(--ink),.02)")}
        >
      ...
      <div style={{ position: "relative", zIndex: 1, maxWidth: 1100, margin: "0 auto", padding: "72px 24px 100px" }}>
        ...
        <div ref={heroRef} style={{ textAlign: "center", marginBottom: 56 }}>
          <div style={{ display: "flex", alignItems: "center", justifyContent: "center", gap: 6, marginBottom: 28, opacity: heroVisible ? 1 : 0, transition: "opacity .5s ease" }}>
            ...
            <span style={{ fontSize: 12, color: "rgba(var(--ink),var(--ta-40, .4))", fontWeight: 500 }}>Help Center</span>
          </div>

          <div style={{ opacity: heroVisible ? 1 : 0, transform: heroVisible ? "translateY(0)" : "translateY(20px)", transition: "all .6s ease .08s" }}>
            <div style={{ display: "inline-flex", alignItems: "center", gap: 7, padding: "5px 14px", borderRadius: 22, background: "rgba(99,102,241,.1)", border: "1.5px solid rgba(99,102,241,.28)", fontSize: 10.5, fontWeight: 700, color: "var(--tx-a5b4fc, #a5b4fc)", letterSpacing: ".09em", textTransform: "uppercase", marginBottom: 20 }}>
              <span style={{ width: 5, height: 5, borderRadius: "50%", background: "#6366f1", animation: "blink 2s ease infinite" }}/>
              Help & Documentation
            </div>

            <h1 style={{ fontFamily: "'Lora',serif", fontSize: "clamp(30px,4vw,52px)", fontWeight: 700, color: "var(--ink-solid, white)", letterSpacing: "-1.5px", lineHeight: 1.1, marginBottom: 14 }}>
              How can we
            ...
            <p style={{ fontSize: 15, color: "rgba(var(--ink),var(--ta-38, .38))", maxWidth: 480, margin: "0 auto 32px", lineHeight: 1.7 }}>
            ...
            <div style={{ position: "relative", maxWidth: 560, margin: "0 auto" }}>
              <div style={{ position: "absolute", left: 18, top: "50%", transform: "translateY(-50%)", pointerEvents: "none" }}>
                <svg width="16" height="16" viewBox="0 0 24 24" fill="none" stroke={searchFocused ? "#818cf8" : "rgba(var(--ink),.3)"} strokeWidth="2.2" style={{ transition: "stroke .2s" }}>
              ...
              <input
                ...
                style={{
                  ...
                  border: `2px solid ${searchFocused ? "rgba(129,140,248,.6)" : "rgba(var(--ink),.1)"}`,
                  background: searchFocused ? "rgba(99,102,241,.08)" : "rgba(var(--ink),.04)",
              ...
              />
              {search && (
                <button onClick={() => setSearch("")} style={{ position: "absolute", right: 14, top: "50%", transform: "translateY(-50%)", background: "none", border: "none", cursor: "pointer", color: "rgba(var(--ink),var(--ta-30, .3))", fontSize: 16, fontFamily: "inherit", padding: 4, transition: "color .2s" }}
                  onMouseEnter={e => (e.currentTarget.style.color = "var(--ink-solid, white)")}
                  onMouseLeave={e => (e.currentTarget.style.color = "rgba(var(--ink),var(--ta-30, .3))")}>✕</button>
              )}
            </div>

            {/* Search results — each result is a Link */}
            {filtered.length > 0 && (
              <div style={{ maxWidth: 560, margin: "12px auto 0", borderRadius: 14, background: "rgba(var(--dkr-080c1e, 8,12,30),0.97)", border: "1.5px solid rgba(129,140,248,.3)", backdropFilter: "blur(20px)", overflow: "hidden", boxShadow: "0 20px 60px rgba(0,0,0,.6)", textAlign: "left" }}>
                <div style={{ padding: "10px 14px 6px", fontSize: 10, fontWeight: 700, color: "rgba(var(--ink),var(--ta-30, .3))", letterSpacing: ".08em", textTransform: "uppercase" }}>
                  {filtered.length} result{filtered.length !== 1 ? "s" : ""}
                </div>
                {filtered.map((a, i) => (
                  <Link key={i} href={`/help/${a.slug}`}
                    style={{ display: "flex", alignItems: "center", gap: 10, padding: "10px 14px", borderTop: i === 0 ? "none" : "1px solid rgba(var(--ink),.05)", transition: "background .18s", textDecoration: "none" }}
                    onMouseEnter={e => (e.currentTarget.style.background = "rgba(99,102,241,.08)")}
                    onMouseLeave={e => (e.currentTarget.style.background = "transparent")}
                  >
                    <span style={{ fontSize: 14 }}>{a.icon}</span>
                    <div style={{ flex: 1 }}>
                      <div style={{ fontSize: 13, fontWeight: 600, color: "rgba(var(--ink),.8)" }}>{a.title}</div>
                      <div style={{ fontSize: 11, color: "rgba(var(--ink),var(--ta-28, .28))", marginTop: 1 }}>{a.category} · {a.time} read</div>
                    </div>
                    <svg width="12" height="12" viewBox="0 0 24 24" fill="none" stroke="rgba(var(--ink),.2)" strokeWidth="2.5">
                      <line x1="5" y1="12" x2="19" y2="12"/><polyline points="12 5 19 12 12 19"/>
                    </svg>
                  </Link>
                ))}
            ...
            {search.length > 1 && filtered.length === 0 && (
              <div style={{ maxWidth: 560, margin: "12px auto 0", padding: "24px", borderRadius: 14, background: "rgba(var(--ink),.03)", border: "1px solid rgba(var(--ink),.08)", textAlign: "center" }}>
                <div style={{ fontSize: 24, marginBottom: 8 }}>🔍</div>
                <div style={{ fontSize: 14, color: "rgba(var(--ink),var(--ta-40, .4))" }}>No articles found for “<strong style={{ color: "var(--ink-solid, white)" }}>{search}</strong>”</div>
                <div style={{ fontSize: 12, color: "rgba(var(--ink),var(--ta-20, .2))", marginTop: 4 }}>Try different keywords or contact support</div>
              </div>
            )}
          </div>
        </div>
        ...
        <div style={{ display: "flex", justifyContent: "center", gap: 32, flexWrap: "wrap", marginBottom: 56 }}>
          {[
            { val: "50+", label: "Help Articles",     color: "var(--tx-818cf8, #818cf8)" },
            { val: "6",   label: "Categories",         color: "var(--tx-34d399, #34d399)" },
            { val: "< 2h",label: "Support Response",   color: "var(--tx-fbbf24, #fbbf24)" },
            { val: "24/7",label: "Live Chat (Pro+)",    color: "var(--tx-a78bfa, #a78bfa)" },
          ].map(({ val, label, color }) => (
            <div key={label} style={{ textAlign: "center" }}>
              <div style={{ fontFamily: "'Lora',serif", fontSize: 26, fontWeight: 700, color, letterSpacing: "-.5px" }}>{val}</div>
              <div style={{ fontSize: 11.5, color: "rgba(var(--ink),var(--ta-30, .3))", fontWeight: 500, marginTop: 2 }}>{label}</div>
            </div>
          ))}
        </div>
        ...
        <div style={{ marginBottom: 56 }}>
          <div style={{ display: "flex", alignItems: "center", gap: 10, marginBottom: 20 }}>
            ...
            <h2 style={{ fontFamily: "'Lora',serif", fontSize: 20, fontWeight: 700, color: "var(--ink-solid, white)", margin: 0 }}>Popular Articles</h2>
          </div>
          <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fill, minmax(280px, 1fr))", gap: 10 }}>
            {POPULAR.map((a, i) => (
              <Link key={i} href={`/help/${a.slug}`}
                ...
                onMouseEnter={e => { e.currentTarget.style.background = "rgba(var(--ink),.06)"; e.currentTarget.style.borderColor = "rgba(var(--ink),.14)"; e.currentTarget.style.transform = "translateY(-2px)"; }}
                onMouseLeave={e => { e.currentTarget.style.background = "rgba(var(--ink),.03)"; e.currentTarget.style.borderColor = "rgba(var(--ink),.07)"; e.currentTarget.style.transform = "translateY(0)"; }}
              >
                <span style={{ fontSize: 16, flexShrink: 0 }}>{a.icon}</span>
                <div style={{ flex: 1, minWidth: 0 }}>
                  <div style={{ fontSize: 13, fontWeight: 600, color: "rgba(var(--ink),.75)", whiteSpace: "nowrap", overflow: "hidden", textOverflow: "ellipsis" }}>{a.title}</div>
                  <div style={{ fontSize: 11, color: "rgba(var(--ink),var(--ta-25, .25))", marginTop: 2 }}>{a.category} · {a.time}</div>
                </div>
        ...
        {/* CATEGORY GRID */}
        <h2 style={{ fontFamily: "'Lora',serif", fontSize: 22, fontWeight: 700, color: "var(--ink-solid, white)", marginBottom: 24 }}>Browse by Category</h2>
        ...
        <div style={{
          ...
          <div>
            <div style={{ fontFamily: "'Lora',serif", fontSize: 22, fontWeight: 700, color: "var(--ink-solid, white)", marginBottom: 8 }}>
              Still need help?
            </div>
            <p style={{ fontSize: 14, color: "rgba(var(--ink),var(--ta-45, .45))", lineHeight: 1.7, maxWidth: 480 }}>
            ...
            <div style={{ display: "flex", gap: 16, flexWrap: "wrap", marginTop: 16 }}>
              {[
                { icon: "💬", label: "Live Chat", sub: "Pro & Enterprise", color: "var(--tx-818cf8, #818cf8)" },
                { icon: "✉️", label: "Email Us",  sub: "support@finovaos.app", color: "var(--tx-34d399, #34d399)" },
                { icon: "📱", label: "WhatsApp",  sub: "+1 (800) 555-0200",  color: "var(--tx-fbbf24, #fbbf24)" },
              ].map(({ icon, label, sub, color }) => (
                <div key={label} style={{ display: "flex", alignItems: "center", gap: 8 }}>
                  <div style={{ width: 34, height: 34, borderRadius: 9, background: `color-mix(in srgb, ${color} 9.4%, transparent)`, border: `1px solid color-mix(in srgb, ${color} 18.8%, transparent)`, display: "flex", alignItems: "center", justifyContent: "center", fontSize: 15 }}>{icon}</div>
                  <div>
                    <div style={{ fontSize: 12.5, fontWeight: 700, color: "rgba(var(--ink),.75)" }}>{label}</div>
                    <div style={{ fontSize: 11, color: "rgba(var(--ink),var(--ta-30, .3))" }}>{sub}</div>
                  </div>
                </div>
              ))}
            </div>
          ...
          <div style={{ display: "flex", flexDirection: "column", gap: 10, flexShrink: 0 }}>
            ...
            <a href="https://mail.google.com/mail/?view=cm&fs=1&to=support@finovaos.app" style={{ display: "inline-flex", alignItems: "center", justifyContent: "center", gap: 7, padding: "11px 24px", borderRadius: 12, background: "rgba(var(--ink),.05)", color: "rgba(var(--ink),var(--ta-60, .6))", fontWeight: 600, fontSize: 13, textDecoration: "none", border: "1px solid rgba(var(--ink),.1)", transition: "all .25s", whiteSpace: "nowrap" }}
              onMouseEnter={e => { e.currentTarget.style.background = "rgba(var(--ink),.09)"; e.currentTarget.style.color = "var(--ink-solid, white)"; }}
              onMouseLeave={e => { e.currentTarget.style.background = "rgba(var(--ink),.05)"; e.currentTarget.style.color = "rgba(var(--ink),var(--ta-60, .6))"; }}
            >
              ✉️ support@finovaos.app
            </a>
          </div>
        </div>
