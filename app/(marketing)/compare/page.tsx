...
function Cell({ val, highlight }: { val: Val; highlight?: boolean }) {
  const cls = "col";
  if (val === true)  return <td className={cls} style={{ textAlign: "center", padding: "10px 14px" }}><span style={{ color: highlight ? "var(--tx-34d399, #34d399)" : "var(--tx-4ade80, #4ade80)", fontSize: 16 }}>✓</span></td>;
  ...
  if (typeof val === "string" && val !== "") return <td className={cls} style={{ textAlign: "center", padding: "10px 14px", fontSize: 12, color: highlight ? "var(--tx-fbbf24, #fbbf24)" : "#6b7280", fontWeight: 600 }}>{val}</td>;
  ...
  return (
    <div style={{ minHeight: "100vh", background: "linear-gradient(160deg,var(--dk-04061a, #04061a) 0%,var(--dk-080c2a, #080c2a) 60%,var(--dk-04061a, #04061a) 100%)", fontFamily: "'Outfit','Inter',sans-serif", color: "var(--tx-e2e8f0, #e2e8f0)" }}>
      ...
      <div style={{ textAlign: "center", padding: "60px 24px 48px", maxWidth: 700, margin: "0 auto" }}>
        ...
        <p style={{ color: "var(--tx-94a3b8, #94a3b8)", fontSize: 16, marginBottom: 32 }}>
        ...
        <div style={{ display: "flex", justifyContent: "center", gap: 12, flexWrap: "wrap", marginBottom: 32 }}>
          {[
            ...
            <div key={b.label} style={{ background: "rgba(129,140,248,.08)", border: "1px solid rgba(129,140,248,.2)", borderRadius: 12, padding: "8px 14px", fontSize: 12, color: "var(--tx-94a3b8, #94a3b8)", display: "flex", alignItems: "center", gap: 6 }}>
        ...
        <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fit,minmax(160px,1fr))", gap: 10, marginBottom: 30 }}>
          {RIVALS.map(r => (
            <Link key={r.slug} href={`/compare/${r.slug}`} style={{
              ...
              background: "rgba(var(--ink),.03)", border: "1px solid rgba(var(--ink),.09)",
              ...
              <span style={{ fontSize: 13, color: "var(--tx-cbd5e1, #cbd5e1)", fontWeight: 600 }}>vs {r.name}</span>
            </Link>
          ))}
        </div>
        ...
        <input
          ...
          style={{ width: "100%", maxWidth: 400, background: "rgba(var(--ink),.05)", border: "1px solid rgba(var(--ink),.1)", borderRadius: 12, color: "var(--tx-e2e8f0, #e2e8f0)", padding: "10px 16px", fontSize: 14, outline: "none", fontFamily: "inherit" }}
        />
      </div>
      ...
      <div ref={ref} className="cmp-wrap" style={{ maxWidth: 1200, margin: "0 auto", padding: "0 12px 60px", opacity: vis ? 1 : 0, transition: "opacity .5s" }}>
        <style>{`
          .cmp-scroll{overflow-x:auto;border-radius:20px;border:1px solid rgba(var(--ink),.08);background:var(--dk-070a22, #070a22);}
          .cmp-table{width:100%;border-collapse:collapse;background:var(--dk-070a22, #070a22);}
          .cmp-table th,.cmp-table td{border-bottom:1px solid rgba(var(--ink),.04);}
          ...
          <table className="cmp-table">
                ...
                {COMPETITORS.map(c => (
                  <th key={c.key} className="col" style={{ textAlign: "center", padding: "16px 14px", fontWeight: 700, fontSize: 13 }}>
                    <div style={{ display: "flex", flexDirection: "column", alignItems: "center", gap: 4 }}>
                      <div style={{ width: 30, height: 30, borderRadius: 8, background: c.highlight ? "linear-gradient(135deg,#4f46e5,#7c3aed)" : "rgba(var(--ink),.1)", display: "flex", alignItems: "center", justifyContent: "center", fontSize: 13, fontWeight: 800, color: c.highlight ? "#fff" : "#64748b" }}>
                        {c.logo}
                      </div>
                      <span style={{ color: c.highlight ? "var(--tx-818cf8, #818cf8)" : "#64748b", fontSize: 12 }}>{c.label}</span>
                    </div>
                  </th>
                ))}
              </tr>
            ...
            <tbody>
              {filtered.map((row, i) => {
                if (row.category) {
                  return (
                    <tr key={i} className="cat-row" style={{ background: "rgba(var(--ink),.03)" }}>
                      ...
                      <td className="col" colSpan={5} style={{ background: "rgba(var(--ink),.03)" }} />
                    </tr>
                  );
                }
                return (
                  <tr key={i} style={{ borderTop: "1px solid rgba(var(--ink),.04)", background: i % 2 === 0 ? "transparent" : "rgba(var(--ink),.01)" }}>
      ...
      <div style={{ textAlign: "center", padding: "60px 24px 100px", borderTop: "1px solid rgba(var(--ink),.06)" }}>
        ...
        <div style={{ display: "flex", gap: 12, justifyContent: "center", flexWrap: "wrap" }}>
          ...
          <Link href="/pricing" style={{ background: "rgba(var(--ink),.06)", border: "1px solid rgba(var(--ink),.1)", color: "var(--tx-e2e8f0, #e2e8f0)", padding: "13px 28px", borderRadius: 12, fontWeight: 600, textDecoration: "none", fontSize: 15 }}>
            View Pricing
          </Link>
