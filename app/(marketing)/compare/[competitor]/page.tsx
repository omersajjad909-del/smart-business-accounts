...
function Mark({ val }: { val: Val }) {
  if (val === true) return <span style={{ color: "var(--tx-4ade80, #4ade80)", fontSize: 16 }}>✓</span>;
  if (val === false) return <span style={{ color: "#374151", fontSize: 14 }}>—</span>;
  if (typeof val === "string" && val !== "")
    return <span style={{ fontSize: 12, color: "var(--tx-94a3b8, #94a3b8)", fontWeight: 600 }}>{val}</span>;
  return null;
}
...
export default async function CompetitorComparePage({
  ...
  return (
    <div style={{
      minHeight: "100vh",
      background: "linear-gradient(160deg,var(--dk-04061a, #04061a) 0%,var(--dk-080c2a, #080c2a) 60%,var(--dk-04061a, #04061a) 100%)",
      ...
      <section style={{ textAlign: "center", padding: "48px 24px 40px", maxWidth: 760, margin: "0 auto" }}>
        <div style={{ display: "flex", alignItems: "center", justifyContent: "center", gap: 16, marginBottom: 26 }}>
          {[
            { logo: "F", label: "FinovaOS", color: "var(--tx-818cf8, #818cf8)", bg: "rgba(129,140,248,.14)", border: "rgba(129,140,248,.4)" },
            { logo: rival.logo, label: rival.name, color: "var(--tx-94a3b8, #94a3b8)", bg: "rgba(var(--ink),.05)", border: BORDER },
          ].map((b, i) => (
            <div key={b.label} style={{ display: "flex", alignItems: "center", gap: 16 }}>
              ...
              <div style={{ display: "flex", flexDirection: "column", alignItems: "center", gap: 8 }}>
                ...
                <span style={{ fontSize: 12.5, color: "var(--tx-94a3b8, #94a3b8)", fontWeight: 600 }}>{b.label}</span>
              </div>
            </div>
          ))}
        </div>
        ...
        <p style={{ color: "var(--tx-94a3b8, #94a3b8)", fontSize: 16, lineHeight: 1.7, margin: 0 }}>{rival.tagline}</p>
      </section>

      {/* Where the rival is the right call — stated first, on purpose. */}
      <section style={{ maxWidth: 760, margin: "0 auto", padding: "0 24px 40px" }}>
        <div style={{ background: "rgba(var(--ink),.03)", border: `1px solid ${BORDER}`, borderRadius: 16, padding: "22px 24px" }}>
          ...
          <p style={{ margin: 0, fontSize: 14.5, color: "var(--tx-94a3b8, #94a3b8)", lineHeight: 1.8 }}>{rival.goodFor}</p>
        </div>
      </section>
        ...
        <div style={{ display: "flex", flexDirection: "column", gap: 10 }}>
          {rival.switchReasons.map((reason) => (
            <div key={reason} style={{
              ...
              <span style={{ color: "var(--tx-818cf8, #818cf8)", fontSize: 15, lineHeight: 1.4, flexShrink: 0 }}>◆</span>
              <span style={{ fontSize: 14, color: "var(--tx-cbd5e1, #cbd5e1)", lineHeight: 1.75 }}>{reason}</span>
            </div>
          ))}
        </div>
      </section>
      ...
      <section style={{ maxWidth: 860, margin: "0 auto", padding: "0 24px 56px" }}>
        ...
        <div style={{ overflowX: "auto", border: `1px solid ${BORDER}`, borderRadius: 16 }}>
          <table style={{ width: "100%", borderCollapse: "collapse", minWidth: 480 }}>
            <thead>
              <tr style={{ background: "rgba(var(--ink),.035)" }}>
                <th style={{ textAlign: "left", padding: "14px 18px", fontSize: 12, fontWeight: 700, color: "var(--tx-94a3b8, #94a3b8)" }}>Feature</th>
                <th style={{ textAlign: "center", padding: "14px 14px", fontSize: 12, fontWeight: 800, color: "var(--tx-818cf8, #818cf8)", whiteSpace: "nowrap" }}>FinovaOS</th>
                <th style={{ textAlign: "center", padding: "14px 14px", fontSize: 12, fontWeight: 700, color: "var(--tx-94a3b8, #94a3b8)", whiteSpace: "nowrap" }}>{rival.name}</th>
              </tr>
            </thead>
            <tbody>
              {ROWS.map((row, i) => (
                row.category ? (
                  <tr key={`${row.feature}-${i}`} style={{ background: "rgba(var(--ink),.02)" }}>
                    <td colSpan={3} style={{ padding: "12px 18px", fontSize: 11.5, fontWeight: 800, color: "var(--tx-818cf8, #818cf8)", letterSpacing: ".06em" }}>
                      {row.feature}
                    </td>
                  </tr>
                ) : (
                  <tr key={`${row.feature}-${i}`} style={{ borderTop: `1px solid rgba(var(--ink),.05)` }}>
                    <td style={{ padding: "11px 18px", fontSize: 13.5, color: "var(--tx-cbd5e1, #cbd5e1)" }}>
                      {row.feature}
                      {row.note && (
      ...
      {/* Other comparisons */}
      <section style={{ maxWidth: 860, margin: "0 auto", padding: "0 24px 56px" }}>
        <h2 style={{ fontSize: 16, fontWeight: 700, margin: "0 0 14px", color: "var(--tx-94a3b8, #94a3b8)" }}>Other comparisons</h2>
        <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fill,minmax(190px,1fr))", gap: 10 }}>
          {others.map((o) => (
            <Link key={o.slug} href={`/compare/${o.slug}`} style={{
              display: "flex", alignItems: "center", gap: 10, textDecoration: "none",
              background: "rgba(var(--ink),.03)", border: `1px solid ${BORDER}`,
              ...
              <div style={{
                ...
                display: "flex", alignItems: "center", justifyContent: "center",
                fontSize: 13, fontWeight: 800, color: "var(--tx-94a3b8, #94a3b8)",
              }}>{o.logo}</div>
              <span style={{ fontSize: 13, color: "var(--tx-cbd5e1, #cbd5e1)", fontWeight: 600 }}>vs {o.name}</span>
            </Link>
          ))}
        </div>
      </section>
      ...
      <section style={{ maxWidth: 860, margin: "0 auto", padding: "0 24px 100px" }}>
        <div style={{
          ...
          <h2 style={{ fontSize: 22, fontWeight: 800, margin: "0 0 10px", color: "var(--ink-solid, white)" }}>
            See it against your own numbers
          </h2>
          <p style={{ fontSize: 14.5, color: "var(--tx-94a3b8, #94a3b8)", margin: "0 0 24px", lineHeight: 1.7 }}>
          ...
          <div style={{ display: "flex", gap: 12, justifyContent: "center", flexWrap: "wrap" }}>
            ...
            <Link href="/pricing" style={{
              padding: "12px 24px", borderRadius: 11, textDecoration: "none",
              background: "rgba(var(--ink),.05)", border: `1px solid ${BORDER}`,
              color: "var(--tx-cbd5e1, #cbd5e1)", fontSize: 14, fontWeight: 600,
            }}>View Pricing</Link>
          </div>
        </div>
      </section>
