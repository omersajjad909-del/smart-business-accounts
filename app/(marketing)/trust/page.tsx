...
const CERTIFICATIONS = [
  { name:"Security Program", icon:"🛡️", color:"var(--tx-818cf8, #818cf8)", glow:"rgba(129,140,248,.15)", desc:"Security, access control, and data protection practices are reviewed regularly as the platform grows.", badge:"Active" },
  { name:"Policy Controls",  icon:"🔐", color:"var(--tx-34d399, #34d399)", glow:"rgba(52,211,153,.12)",  desc:"Internal controls, operating procedures, and secure handling standards help protect customer data.", badge:"Managed" },
  { name:"GDPR Compliant", icon:"🇪🇺", color:"var(--tx-38bdf8, #38bdf8)", glow:"rgba(56,189,248,.12)",  desc:"Full compliance with EU General Data Protection Regulation. DPA available on request.", badge:"Compliant" },
  { name:"Payment Security",icon:"💳", color:"var(--tx-fbbf24, #fbbf24)", glow:"rgba(251,191,36,.12)",  desc:"Payments are handled through secure providers, with encrypted transport and careful operational controls.", badge:"Protected" },
  { name:"CCPA Ready",     icon:"🏛️", color:"var(--tx-c4b5fd, #c4b5fd)", glow:"rgba(196,181,253,.12)", desc:"California Consumer Privacy Act compliance for all US-based users and data processing.", badge:"Compliant" },
  { name:"Service Standards", icon:"✅", color:"var(--tx-f9a8d4, #f9a8d4)", glow:"rgba(249,168,212,.12)",  desc:"Operational standards and documented processes help us deliver support and product updates consistently.", badge:"Documented" },
];
      ...
      <Section style={{ padding:"120px 24px 80px", textAlign:"center", position:"relative", overflow:"hidden" }}>
        ...
        <div style={{ maxWidth:660, margin:"0 auto", position:"relative", zIndex:1 }}>
          <div style={{ display:"inline-flex", gap:8, padding:"6px 16px", borderRadius:24, background:"rgba(52,211,153,.1)", border:"1px solid rgba(52,211,153,.25)", marginBottom:22, opacity:heroVis?1:0, transition:"all .5s" }}>
            ...
            <span style={{ fontSize:12, fontWeight:800, color:"var(--tx-34d399, #34d399)", letterSpacing:".06em" }}>SECURITY & COMPLIANCE</span>
          </div>
          <h1 style={{ fontSize:"clamp(34px,5vw,56px)", fontWeight:900, letterSpacing:"-.03em", lineHeight:1.1, fontFamily:"Lora,serif", margin:"0 0 20px", opacity:heroVis?1:0, transition:"all .6s ease .1s" }}>
            ...
            <span style={{ display:"block", background:"linear-gradient(90deg,var(--tx-34d399, #34d399),var(--tx-38bdf8, #38bdf8))", WebkitBackgroundClip:"text", WebkitTextFillColor:"transparent" }}>enterprise-grade protection</span>
          </h1>
          <p style={{ fontSize:16, color:"rgba(var(--ink),var(--ta-50, .5))", lineHeight:1.75, maxWidth:500, margin:"0 auto 36px", opacity:heroVis?1:0, transition:"all .6s ease .2s" }}>
          ...
          <div style={{ display:"flex", gap:20, justifyContent:"center", flexWrap:"wrap", opacity:heroVis?1:0, transition:"all .6s ease .25s" }}>
            {[["🛡️","Security Program"],["🔐","Policy Controls"],["🇪🇺","GDPR"],["💳","Payment Security"]].map(([icon,name])=>(
              <div key={name} style={{ display:"flex", alignItems:"center", gap:8, padding:"8px 16px", borderRadius:20, background:"rgba(52,211,153,.08)", border:"1px solid rgba(52,211,153,.2)" }}>
                ...
                <span style={{ fontSize:12, fontWeight:700, color:"var(--tx-34d399, #34d399)" }}>{name}</span>
              </div>
            ))}
          </div>
        </div>
        ...
        <div style={{ maxWidth:1060, margin:"0 auto", padding:"0 24px 80px" }}>
          ...
          <div style={{ display:"grid", gridTemplateColumns:"repeat(auto-fit,minmax(290px,1fr))", gap:16 }}>
            {CERTIFICATIONS.map(c=>(
              <div key={c.name} style={{ background:`linear-gradient(135deg,${c.glow},rgba(var(--ink),.02))`, borderRadius:18, border:`1px solid color-mix(in srgb, ${c.color} 26.7%, transparent)`, padding:"24px 22px", display:"flex", gap:16 }}>
                <div style={{ width:52, height:52, borderRadius:14, background:`color-mix(in srgb, ${c.color} 12.5%, transparent)`, border:`1px solid color-mix(in srgb, ${c.color} 25.1%, transparent)`, display:"flex", alignItems:"center", justifyContent:"center", fontSize:22, flexShrink:0 }}>{c.icon}</div>
                <div>
                  <div style={{ display:"flex", alignItems:"center", gap:8, marginBottom:6 }}>
                    <span style={{ fontSize:14, fontWeight:800, color:"var(--ink-solid, white)" }}>{c.name}</span>
                    <span style={{ padding:"2px 8px", borderRadius:20, background:`color-mix(in srgb, ${c.color} 12.5%, transparent)`, color:c.color, fontSize:9, fontWeight:800 }}>{c.badge}</span>
                  </div>
                  <div style={{ fontSize:12, color:"rgba(var(--ink),var(--ta-45, .45))", lineHeight:1.6 }}>{c.desc}</div>
                </div>
              </div>
            ))}
          </div>
        ...
        <div style={{ maxWidth:1060, margin:"0 auto", padding:"0 24px 80px" }}>
          ...
          <div style={{ display:"grid", gridTemplateColumns:"repeat(auto-fit,minmax(240px,1fr))", gap:14 }}>
            {SECURITY_FEATURES.map(f=>(
              <div key={f.title} style={{ background:"rgba(var(--ink),.03)", borderRadius:16, border:"1px solid rgba(var(--ink),.07)", padding:"22px 20px", transition:"transform .2s, border-color .2s" }}
                ...
                <div style={{ fontSize:13, fontWeight:700, color:"var(--ink-solid, white)", marginBottom:6 }}>{f.title}</div>
                <div style={{ fontSize:12, color:"rgba(var(--ink),var(--ta-40, .4))", lineHeight:1.65 }}>{f.desc}</div>
              </div>
            ))}
          </div>
        </div>
      ...
      <Section>
        <div style={{ maxWidth:900, margin:"0 auto", padding:"0 24px 80px" }}>
          ...
          <div style={{ display:"grid", gridTemplateColumns:"repeat(auto-fit,minmax(200px,1fr))", gap:14 }}>
            {DATA_CENTRES.map(dc=>(
              <div key={dc.region} style={{ background:"rgba(var(--ink),.03)", borderRadius:16, border:`1px solid ${dc.primary?"rgba(52,211,153,.25)":"rgba(var(--ink),.07)"}`, padding:"22px 18px", textAlign:"center", position:"relative" }}>
                ...
                <div style={{ fontSize:14, fontWeight:700, color:"var(--ink-solid, white)", marginBottom:4 }}>{dc.location}</div>
                <div style={{ fontSize:11, color:"rgba(var(--ink),var(--ta-35, .35))", marginBottom:8 }}>{dc.region}</div>
                <div style={{ fontSize:10, fontWeight:700, color:"var(--tx-818cf8, #818cf8)", fontFamily:"monospace" }}>{dc.provider}</div>
              </div>
            ))}
          </div>
          <div style={{ marginTop:20, padding:"16px 20px", borderRadius:12, background:"rgba(99,102,241,.08)", border:"1px solid rgba(99,102,241,.2)", fontSize:13, color:"rgba(var(--ink),var(--ta-50, .5))", textAlign:"center", lineHeight:1.7 }}>
            🏛️ <strong style={{color:"var(--ink-solid, white)"}}>Enterprise customers</strong> can choose their preferred data residency region for full regulatory compliance.
          </div>
        </div>
      </Section>
      ...
      <Section>
        <div style={{ maxWidth:740, margin:"0 auto", padding:"0 24px 80px" }}>
          ...
          <div style={{ display:"flex", flexDirection:"column", gap:10 }}>
            {FAQS.map((f,i)=>(
              <div key={i} style={{ background:"rgba(var(--ink),.03)", borderRadius:14, border:`1px solid ${openFaq===i?"rgba(52,211,153,.3)":"rgba(var(--ink),.07)"}`, overflow:"hidden", transition:"border-color .2s" }}>
                <button onClick={()=>setOpenFaq(openFaq===i?null:i)} style={{ width:"100%", display:"flex", justifyContent:"space-between", alignItems:"center", padding:"18px 20px", background:"none", border:"none", cursor:"pointer", textAlign:"left", gap:12 }}>
                  <span style={{ fontSize:14, fontWeight:700, color:"var(--ink-solid, white)", lineHeight:1.4 }}>{f.q}</span>
                  <span style={{ fontSize:18, color:"var(--tx-34d399, #34d399)", flexShrink:0, transition:"transform .2s", transform:openFaq===i?"rotate(45deg)":"rotate(0)" }}>+</span>
                </button>
                {openFaq===i && (
                  <div style={{ padding:"0 20px 18px" }}>
                    <div style={{ fontSize:14, color:"rgba(var(--ink),var(--ta-50, .5))", lineHeight:1.75 }}>{f.a}</div>
                  </div>
                )}
              </div>
            ))}
        ...
        <div style={{ maxWidth:640, margin:"0 auto", padding:"0 24px 120px", textAlign:"center" }}>
          <div style={{ padding:"48px 36px", borderRadius:22, background:"linear-gradient(135deg,rgba(52,211,153,.1),rgba(56,189,248,.06))", border:"1px solid rgba(52,211,153,.2)", position:"relative", overflow:"hidden" }}>
            <div style={{ position:"relative", zIndex:1 }}>
              ...
              <p style={{ fontSize:14, color:"rgba(var(--ink),var(--ta-45, .45))", lineHeight:1.7, margin:"0 auto 24px", maxWidth:400 }}>
              ...
              <div style={{ display:"flex", gap:12, justifyContent:"center", flexWrap:"wrap" }}>
                ...
                <Link href="/contact" style={{ padding:"12px 26px", borderRadius:11, fontWeight:700, fontSize:13, background:"rgba(var(--ink),.08)", border:"1px solid rgba(var(--ink),.12)", color:"rgba(var(--ink),.8)", textDecoration:"none" }}>
                  Request Docs
                </Link>
              </div>
            </div>
