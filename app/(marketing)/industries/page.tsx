...
export default function IndustriesPage() {
  ...
  return (
    <div style={{
      minHeight: "100vh",
      background: "linear-gradient(180deg,var(--dk-080c1e, #080c1e) 0%,var(--dk-0a0e28, #0a0e28) 50%,var(--dk-060919, #060919) 100%)",
      ...
      <style>{`
        ...
        *, *::before, *::after { box-sizing: border-box; }
        ...
        .ind-filter-btn {
          ...
          font-weight: 600; border: 1px solid rgba(var(--ink),.1);
          background: transparent; color:rgba(var(--ink),var(--ta-45, .45));
          cursor: pointer; transition: all .18s; font-family: 'Outfit',sans-serif;
          white-space: nowrap;
        }
        .ind-filter-btn:hover { border-color: rgba(var(--ink),.25); color:rgba(var(--ink),.75); }
        .ind-filter-btn.active { color:var(--ink-solid, white); border-color: currentColor; }

        .ind-search {
          background: rgba(var(--ink),.05); border: 1px solid rgba(var(--ink),.1);
          border-radius: 12px; padding: 10px 16px; color:var(--ink-solid, white);
          font-size: 13.5px; font-family: 'Outfit',sans-serif; width: 100%; max-width: 360px;
          outline: none; transition: border-color .2s;
        }
        .ind-search::placeholder { color:rgba(var(--ink),var(--ta-30, .3)); }
      ...
      <section className="ind-hero" style={{ padding: "90px 24px 60px", textAlign: "center", position: "relative", overflow: "hidden" }}>
        ...
        <div style={{ position: "relative", maxWidth: 720, margin: "0 auto" }}>
          <div style={{ display: "inline-flex", alignItems: "center", gap: 8, padding: "5px 14px", borderRadius: 100, background: "rgba(52,211,153,.1)", border: "1px solid rgba(52,211,153,.25)", marginBottom: 22 }}>
            ...
            <span style={{ fontSize: 11, fontWeight: 700, color: "var(--tx-6ee7b7, #6ee7b7)", letterSpacing: ".1em", textTransform: "uppercase" }}>
          ...
          <p style={{ fontSize: 16, color: "rgba(var(--ink),var(--ta-45, .45))", lineHeight: 1.8, marginBottom: 36, maxWidth: 560, margin: "0 auto 36px" }}>
          ...
          <div style={{ display: "flex", justifyContent: "center", gap: 0, flexWrap: "wrap", marginBottom: 8 }}>
            {([1,2,3,4] as const).map((ph, i) => {
              ...
              return (
                <div key={ph} style={{ padding: "10px 22px", textAlign: "center", borderRight: i < 3 ? "1px solid rgba(var(--ink),.06)" : "none" }}>
                  <div style={{ fontFamily: "'Lora',serif", fontSize: 22, fontWeight: 700, color }}>{count}</div>
                  <div style={{ fontSize: 10, color: "rgba(var(--ink),var(--ta-30, .3))", fontWeight: 600, letterSpacing: ".06em", textTransform: "uppercase", marginTop: 2 }}>Phase {ph}</div>
                </div>
              );
            })}
          </div>
      ...
      <div className="ind-stats-wrap" style={{ maxWidth: 1180, margin: "0 auto", padding: "0 24px 36px" }}>
        <div style={{ display: "flex", gap: 10, flexWrap: "wrap", alignItems: "center" }}>
          ...
          />
          <button className={`ind-filter-btn${activePhase === null ? " active" : ""}`} style={{ color: activePhase === null ? "var(--ink-solid, white)" : undefined }} onClick={() => setActivePhase(null)}>
            All Phases
          </button>
      ...
      <div className="ind-phase-wrap" style={{ maxWidth: 1180, margin: "0 auto", padding: "0 24px 80px" }}>
        {loading ? (
          <div style={{ textAlign: "center", padding: "60px 0", color: "rgba(var(--ink),var(--ta-30, .3))", fontSize: 14 }}>Loading...</div>
        ) : (
              ...
              <div key={phase} className="ind-phase" style={{ marginBottom: 64 }}>
                ...
                <div style={{ display: "flex", alignItems: "center", gap: 14, marginBottom: 24, flexWrap: "wrap" }}>
                  <div style={{ display: "flex", alignItems: "center", gap: 8 }}>
                    ...
                    <h2 style={{ fontFamily: "'Lora',serif", fontSize: 20, fontWeight: 700, color: "var(--ink-solid, white)", margin: 0, letterSpacing: "-.3px" }}>
                      {PHASE_LABELS[phase]}
                    </h2>
                  </div>
                  {isLivePhase && (
                    <div style={{ display: "flex", alignItems: "center", gap: 5, padding: "3px 10px", borderRadius: 100, background: "rgba(52,211,153,.1)", border: "1px solid rgba(52,211,153,.25)" }}>
                      ...
                      <span style={{ fontSize: 10.5, fontWeight: 700, color: "var(--tx-6ee7b7, #6ee7b7)" }}>LIVE</span>
                    </div>
                  )}
                  <span style={{ fontSize: 12.5, color: "rgba(var(--ink),var(--ta-30, .3))", marginLeft: 4 }}>{PHASE_DESC[phase]}</span>
                  <span style={{ marginLeft: "auto", fontSize: 11.5, color: "rgba(var(--ink),var(--ta-22, .22))", fontWeight: 600 }}>{list.length} industries</span>
                </div>
                  ...
                  {list.map(b => {
                    ...
                    const card = (
                      ...
                      >
                        ...
                        {!isLive && (
                          <div style={{ position: "absolute", top: 9, right: 10, fontSize: 9, fontWeight: 700, color: `color-mix(in srgb, ${color} 60%, transparent)`, letterSpacing: ".06em", textTransform: "uppercase" }}>
                            Soon
                          </div>
                        )}

                        <div style={{ fontSize: 26, marginBottom: 10 }}>{b.icon}</div>
                        <div style={{ fontSize: 13.5, fontWeight: 700, color: isLive ? "rgba(var(--ink),.9)" : "rgba(var(--ink),var(--ta-55, .55))", marginBottom: 5, lineHeight: 1.3 }}>
                          {b.label}
                        </div>
                        <div style={{ fontSize: 11, color: "rgba(var(--ink),var(--ta-28, .28))", lineHeight: 1.6, marginBottom: isLive ? 12 : 0 }}>
                          {b.description}
                        </div>
        ...
        {!loading && filtered.length === 0 && (
          <div style={{ textAlign: "center", padding: "60px 0" }}>
            ...
            <p style={{ color: "rgba(var(--ink),var(--ta-35, .35))", fontSize: 14 }}>No business type found for &quot;{search}&quot;</p>
          </div>
        )}
      </div>

      {/* Bottom CTA */}
      <div style={{ borderTop: "1px solid rgba(var(--ink),.05)", padding: "48px 24px", textAlign: "center" }}>
        <p style={{ fontSize: 13.5, color: "rgba(var(--ink),var(--ta-30, .3))", marginBottom: 8 }}>Don&apos;t see your industry?</p>
        <a href="https://mail.google.com/mail/?view=cm&fs=1&to=partnerships@finovaos.app" style={{ fontSize: 14, fontWeight: 700, color: "var(--tx-818cf8, #818cf8)", textDecoration: "none" }}>
