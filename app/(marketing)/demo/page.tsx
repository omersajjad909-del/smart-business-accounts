...
const CATEGORY_COLORS: Record<string, string> = {
  Commerce: "var(--tx-38bdf8, #38bdf8)",
  Logistics: "var(--tx-f59e0b, #f59e0b)",
  "Food & Beverage": "var(--tx-f97316, #f97316)",
  Production: "#6366f1",
  Healthcare: "#ec4899",
  Education: "var(--tx-14b8a6, #14b8a6)",
  Hospitality: "var(--tx-a78bfa, #a78bfa)",
  Projects: "var(--tx-fb923c, #fb923c)",
  Property: "var(--tx-f472b6, #f472b6)",
  Trade: "var(--tx-4ade80, #4ade80)",
...
export default function DemoPage() {
  ...
  return (
    <div style={{ minHeight: "100vh", background: "var(--dk-07091a, #07091a)", color: "var(--ink-solid, #fff)", fontFamily: FONT }}>
      <style>{`
        ...
        .cat-btn:hover { border-color: rgba(var(--ink),.2); }
        ...
        .plan-card:hover { transform: translateY(-3px); border-color: rgba(var(--ink),.22); }
        ...
        .mod-chip:hover { background: rgba(var(--ink),.07); }
      ...
      <div style={{ textAlign: "center", padding: "80px 24px 48px", maxWidth: 900, margin: "0 auto", animation: "fadeUp .5s ease both" }}>
        <div style={{ display: "inline-flex", alignItems: "center", gap: 8, padding: "6px 16px", borderRadius: 999, background: "rgba(99,102,241,.12)", border: "1px solid rgba(99,102,241,.3)", marginBottom: 22 }}>
          ...
          <span style={{ fontSize: 11, fontWeight: 800, color: "var(--tx-a5b4fc, #a5b4fc)", letterSpacing: ".08em", textTransform: "uppercase" }}>Live Interactive Demo</span>
        </div>

        <h1 style={{ margin: "0 0 18px", fontSize: "clamp(36px,5.5vw,62px)", fontWeight: 900, letterSpacing: -1.8, lineHeight: 1.04 }}>
          Pick your business type.
          <br />
          <span style={{ background: "linear-gradient(90deg,var(--tx-a5b4fc, #a5b4fc),var(--tx-38bdf8, #38bdf8))", WebkitBackgroundClip: "text", WebkitTextFillColor: "transparent" }}>
        ...
        <div style={{ display: "flex", justifyContent: "center", flexWrap: "wrap", gap: 10, marginBottom: 8 }}>
          {TRUST_STATS.map((t) => (
            <div key={t.label} style={{ display: "flex", alignItems: "center", gap: 8, padding: "8px 18px", borderRadius: 999, background: "rgba(var(--ink),.04)", border: "1px solid rgba(var(--ink),.08)" }}>
              <span style={{ fontSize: 15 }}>{t.icon}</span>
              <span style={{ fontSize: 14, fontWeight: 800, color: "var(--ink-solid, #fff)" }}>{t.value}</span>
              <span style={{ fontSize: 12, color: "rgba(var(--ink),var(--ta-40, .4))" }}>{t.label}</span>
            </div>
          ))}
        </div>
      </div>
      ...
      <div style={{ maxWidth: 1280, margin: "0 auto", padding: "0 24px 100px" }}>
          ...
          {CATEGORIES.map((cat) => {
            ...
            return (
              <button
                ...
                style={{ padding: "7px 16px", borderRadius: 999, background: active ? `color-mix(in srgb, ${c} 9.4%, transparent)` : "rgba(var(--ink),.03)", color: active ? c : "rgba(var(--ink),var(--ta-50, .5))", borderColor: active ? `color-mix(in srgb, ${c} 25.1%, transparent)` : "transparent", fontSize: 12, fontWeight: 700, fontFamily: FONT }}
              >
                {cat}
              </button>
            );
          ...
          {filteredBusinesses.map((entry, i) => {
            ...
            return (
              <div
                ...
                style={{
                  background: active ? `linear-gradient(135deg,color-mix(in srgb, ${entry.color} 9.4%, transparent),rgba(var(--ink),.04))` : "rgba(var(--ink),.03)",
                  border: `1.5px solid ${active ? entry.color : "rgba(var(--ink),.07)"}`,
                  borderRadius: 20,
                  padding: "20px 18px",
                  boxShadow: active ? `0 16px 40px color-mix(in srgb, ${entry.color} 13.3%, transparent)` : "none",
              ...
              >
                ...
                <div style={{ width: 48, height: 48, borderRadius: 14, background: entry.gradient, display: "flex", alignItems: "center", justifyContent: "center", fontSize: 22, marginBottom: 12, boxShadow: `0 6px 20px color-mix(in srgb, ${entry.color} 20.8%, transparent)` }}>
                  {entry.icon}
                </div>
                <div style={{ fontSize: 11, fontWeight: 800, color: CATEGORY_COLORS[entry.category] || "var(--tx-a5b4fc, #a5b4fc)", letterSpacing: ".06em", textTransform: "uppercase", marginBottom: 6 }}>
                  {entry.category}
                </div>
                <div style={{ fontSize: 15, fontWeight: 800, color: active ? entry.color : "var(--ink-solid, #fff)", marginBottom: 4 }}>{entry.label}</div>
                <div style={{ fontSize: 11.5, color: "rgba(var(--ink),var(--ta-38, .38))", lineHeight: 1.5 }}>{entry.tagline}</div>
                <div style={{ marginTop: 8 }}>
                  <span
                    style={{
                      ...
                      color: isDemoLive(entry.liveBusinessType) ? "var(--tx-34d399, #34d399)" : "var(--tx-fbbf24, #fbbf24)",
                      fontSize: 10,
                      fontWeight: 800,
                      letterSpacing: ".04em",
                ...
                <div style={{ marginTop: 10, fontSize: 11, color: "rgba(var(--ink),var(--ta-25, .25))", fontWeight: 700, letterSpacing: ".04em", textTransform: "uppercase" }}>{entry.category}</div>
              </div>
            );
          })}
        </div>
        ...
        {!biz && (
          <div style={{ textAlign: "center", padding: "40px 24px", borderRadius: 24, background: "rgba(var(--ink),.02)", border: "1px solid rgba(var(--ink),.06)" }}>
            <div style={{ fontSize: 36, marginBottom: 12 }}>👆</div>
            <div style={{ fontSize: 18, fontWeight: 800, marginBottom: 8 }}>Select a business type above</div>
            <div style={{ fontSize: 14, color: "rgba(var(--ink),var(--ta-35, .35))" }}>
        ...
        {biz && (
          <div id="biz-detail" style={{ animation: "fadeUp .35s ease both" }}>
            ...
            <div style={{ marginBottom: 22, padding: "24px 24px 22px", borderRadius: 24, background: "rgba(var(--ink),.025)", border: "1px solid rgba(var(--ink),.07)" }}>
              ...
              <div style={{ fontSize: 13, color: "rgba(var(--ink),var(--ta-42, .42))", lineHeight: 1.6, marginBottom: 18 }}>
                  ...
                  return (
                    <button
                      ...
                      style={{
                        ...
                        borderRadius: 20,
                        background: active ? `linear-gradient(135deg,color-mix(in srgb, ${biz.color} 8.6%, transparent),rgba(var(--ink),.03))` : "rgba(var(--ink),.03)",
                        border: `1.5px solid ${active ? biz.color : "rgba(var(--ink),.08)"}`,
                        boxShadow: active ? `0 14px 34px color-mix(in srgb, ${biz.color} 13.3%, transparent)` : "none",
                        color: "#fff",
                      }}
                    >
                      <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", gap: 10, marginBottom: 8 }}>
                        <div style={{ fontSize: 15, fontWeight: 900, color: active ? biz.color : "var(--ink-solid, #fff)" }}>{planNames[code]}</div>
                        <span style={{ width: 16, height: 16, borderRadius: "50%", flexShrink: 0, border: `1.5px solid ${active ? biz.color : "rgba(var(--ink),.25)"}`, background: active ? biz.color : "transparent", display: "flex", alignItems: "center", justifyContent: "center", fontSize: 10, fontWeight: 900, color: "#07091a" }}>
                          {active ? "✓" : ""}
                        </span>
                      </div>

                      {price && price.monthly > 0 && (
                        <div style={{ fontSize: 13, fontWeight: 800, color: "rgba(var(--ink),var(--ta-72, .72))", marginBottom: 8 }}>
                          {price.currency === "PKR" ? `Rs ${price.monthly.toLocaleString()}` : `${price.monthly}`}
                          <span style={{ fontSize: 11, fontWeight: 600, color: "rgba(var(--ink),var(--ta-35, .35))" }}> /month</span>
                        </div>
                      )}

                      <div style={{ fontSize: 11.5, color: "rgba(var(--ink),var(--ta-40, .4))", lineHeight: 1.55, marginBottom: bullets.length ? 10 : 0 }}>
                        {PLAN_BLURB[code]}
                      </div>

                      {bullets.map((f) => (
                        <div key={f} style={{ display: "flex", gap: 7, alignItems: "flex-start", fontSize: 11.5, color: "rgba(var(--ink),var(--ta-50, .5))", lineHeight: 1.5, marginTop: 5 }}>
                          <span style={{ color: active ? biz.color : "rgba(var(--ink),var(--ta-30, .3))", fontWeight: 900 }}>·</span>
                          <span>{f}</span>
                        </div>
                      ))}
                    </button>
            ...
            {/* Header */}
            <div className="demo-detail-header" style={{ display: "flex", alignItems: "center", gap: 18, marginBottom: 28, padding: "28px 28px", background: `linear-gradient(135deg,color-mix(in srgb, ${biz.color} 7.1%, transparent),rgba(var(--ink),.02))`, border: `1px solid color-mix(in srgb, ${biz.color} 14.5%, transparent)`, borderRadius: 24 }}>
              <div style={{ width: 72, height: 72, borderRadius: 20, background: biz.gradient, display: "flex", alignItems: "center", justifyContent: "center", fontSize: 36, flexShrink: 0, boxShadow: `0 10px 30px color-mix(in srgb, ${biz.color} 25.1%, transparent)` }}>
                {biz.icon}
              </div>
              <div style={{ flex: 1 }}>
                <div style={{ display: "flex", alignItems: "center", gap: 10, marginBottom: 6, flexWrap: "wrap" }}>
                  <div style={{ fontSize: 28, fontWeight: 900, letterSpacing: -0.8 }}>{biz.label}</div>
                  <div style={{ padding: "4px 12px", borderRadius: 999, background: `color-mix(in srgb, ${biz.color} 12.5%, transparent)`, color: biz.color, fontSize: 11, fontWeight: 800 }}>
                    {biz.category}
                  </div>
                  <div style={{ padding: "4px 12px", borderRadius: 999, background: isDemoLive(biz.liveBusinessType) ? "rgba(16,185,129,.14)" : "rgba(245,158,11,.12)", color: isDemoLive(biz.liveBusinessType) ? "var(--tx-34d399, #34d399)" : "var(--tx-fbbf24, #fbbf24)", fontSize: 11, fontWeight: 800 }}>
                    {isDemoLive(biz.liveBusinessType) ? "Live Demo Ready" : "Coming Soon"}
                  </div>
                </div>
                <div style={{ fontSize: 14, color: "rgba(var(--ink),var(--ta-50, .5))", lineHeight: 1.7, maxWidth: 680 }}>{biz.description}</div>
              </div>
              <div style={{ display: "flex", gap: 10, flexShrink: 0, flexWrap: "wrap" }}>
                <button
                  ...
                  style={{
                    background: isDemoLive(biz.liveBusinessType) ? biz.gradient : "rgba(var(--ink),.08)",
                    color: isDemoLive(biz.liveBusinessType) ? "#fff" : "rgba(var(--ink),var(--ta-45, .45))",
                    borderRadius: 16,
                    padding: "14px 28px",
                    fontSize: 14,
                    fontWeight: 800,
                    cursor: !isDemoLive(biz.liveBusinessType) || launching ? "not-allowed" : "pointer",
                    boxShadow: isDemoLive(biz.liveBusinessType) ? `0 10px 28px color-mix(in srgb, ${biz.color} 20.8%, transparent)` : "none",
                    whiteSpace: "nowrap",
            ...
            {launchError && (
              <div style={{ marginTop: -8, marginBottom: 20, padding: "14px 18px", borderRadius: 16, background: "rgba(239,68,68,.1)", border: "1px solid rgba(239,68,68,.28)", color: "var(--tx-fca5a5, #fca5a5)", fontSize: 13, lineHeight: 1.6 }}>
                {launchError}
              </div>
            )}

            {isDemoLive(biz.liveBusinessType) && (
              <div style={{ marginTop: -8, marginBottom: 20, padding: "14px 18px", borderRadius: 16, background: "rgba(16,185,129,.07)", border: "1px solid rgba(16,185,129,.2)", color: "rgba(var(--ink),var(--ta-72, .72))", fontSize: 13, lineHeight: 1.7 }}>
                Opens a private {DEMO_SESSION_LABEL} workspace on the <strong style={{ color: "var(--ink-solid, #fff)" }}>{planNames[selectedPlan]}</strong> plan,
                ...
                Sign-in is <strong style={{ color: "var(--ink-solid, #fff)" }}>{DEMO_EMAIL}</strong> / <strong style={{ color: "var(--ink-solid, #fff)" }}>{DEMO_PASSWORD}</strong> if you would rather log in yourself.
              </div>
            )}

            {!isDemoLive(biz.liveBusinessType) && (
              <div style={{ marginTop: -8, marginBottom: 20, padding: "14px 18px", borderRadius: 16, background: "rgba(245,158,11,.08)", border: "1px solid rgba(245,158,11,.2)", color: "rgba(var(--ink),var(--ta-72, .72))", fontSize: 13, lineHeight: 1.6 }}>
            ...
            <div className="demo-highlights" style={{ display: "grid", gridTemplateColumns: "repeat(3,minmax(0,1fr))", gap: 12, marginBottom: 22 }}>
              {biz.highlights.map((h) => (
                <div key={h.label} style={{ background: "rgba(var(--ink),.03)", border: `1px solid color-mix(in srgb, ${biz.color} 9.4%, transparent)`, borderRadius: 18, padding: "20px 18px" }}>
                  <div style={{ fontSize: 11, color: "rgba(var(--ink),var(--ta-38, .38))", fontWeight: 700, letterSpacing: ".05em", textTransform: "uppercase", marginBottom: 8 }}>{h.label}</div>
                  <div style={{ fontSize: 28, fontWeight: 900, color: biz.color, marginBottom: 6 }}>{h.value}</div>
                  <div style={{ fontSize: 12, color: "rgba(var(--ink),var(--ta-40, .4))", lineHeight: 1.5 }}>{h.sub}</div>
                </div>
              ))}
            </div>

            {/* Tab switcher */}
              <div className="demo-tabs" style={{ display: "flex", gap: 6, marginBottom: 22, background: "rgba(var(--ink),.03)", padding: 6, borderRadius: 16, border: "1px solid rgba(var(--ink),.06)", width: "fit-content" }}>
                ...
                return (
                  <button
                    ...
                    style={{
                      ...
                      color: active ? "#fff" : "rgba(var(--ink),var(--ta-45, .45))",
                      fontSize: 13,
                      fontWeight: 700,
                      boxShadow: active ? `0 4px 14px color-mix(in srgb, ${biz.color} 18.8%, transparent)` : "none",
                    }}
                  >
                    {labels[tab]}
                  </button>
            ...
            {activeTab === "overview" && (
              <div className="demo-overview-grid" style={{ display: "grid", gridTemplateColumns: "1.3fr 1fr", gap: 18, animation: "fadeUp .3s ease both" }}>
                {/* Modules */}
                <div style={{ background: "rgba(var(--ink),.025)", border: `1px solid color-mix(in srgb, ${biz.color} 9.4%, transparent)`, borderRadius: 22, padding: "22px 22px" }}>
                  <div style={{ fontSize: 17, fontWeight: 800, marginBottom: 6 }}>Modules included for {biz.label}</div>
                  <div style={{ fontSize: 13, color: "rgba(var(--ink),var(--ta-35, .35))", marginBottom: 16 }}>These modules are pre-loaded in your live demo workspace.</div>
                  <div className="demo-module-grid" style={{ display: "grid", gridTemplateColumns: "repeat(2,1fr)", gap: 8 }}>
                    {biz.modules.map((mod) => (
                      <div key={mod} className="mod-chip" style={{ padding: "12px 14px", borderRadius: 12, background: "rgba(var(--ink),.03)", border: "1px solid rgba(var(--ink),.06)", fontSize: 13, fontWeight: 700, color: "rgba(var(--ink),.82)", display: "flex", alignItems: "center", gap: 8 }}>
                ...
                <div style={{ display: "flex", flexDirection: "column", gap: 14 }}>
                  <div style={{ background: "rgba(var(--ink),.025)", border: "1px solid rgba(var(--ink),.07)", borderRadius: 20, padding: 20, flex: 1 }}>
                    <div style={{ fontSize: 15, fontWeight: 800, marginBottom: 14 }}>Why this matters</div>
                    <div style={{ display: "grid", gap: 10 }}>
                      {biz.proof.map((p) => (
                        <div key={p} style={{ display: "flex", gap: 10, alignItems: "flex-start", padding: "10px 12px", borderRadius: 12, background: "rgba(var(--ink),.025)" }}>
                          <div style={{ width: 22, height: 22, borderRadius: 7, background: `color-mix(in srgb, ${biz.color} 12.5%, transparent)`, color: biz.color, display: "flex", alignItems: "center", justifyContent: "center", fontSize: 11, fontWeight: 900, flexShrink: 0 }}>✓</div>
                          <div style={{ fontSize: 13, color: "rgba(var(--ink),var(--ta-65, .65))", lineHeight: 1.55 }}>{p}</div>
                        </div>
                      ))}
                    </div>
                  </div>
                  <div style={{ background: "rgba(var(--ink),.02)", border: "1px solid rgba(var(--ink),.06)", borderRadius: 18, padding: "16px 18px" }}>
                    <div style={{ fontSize: 12, fontWeight: 800, color: "rgba(var(--ink),var(--ta-40, .4))", textTransform: "uppercase", letterSpacing: ".07em", marginBottom: 10 }}>Sample documents in the demo</div>
                    <div style={{ display: "flex", flexWrap: "wrap", gap: 7 }}>
                      {biz.sampleDocs.map((doc) => (
                        <span key={doc} style={{ padding: "6px 12px", borderRadius: 999, background: `color-mix(in srgb, ${biz.color} 7.1%, transparent)`, border: `1px solid color-mix(in srgb, ${biz.color} 14.5%, transparent)`, color: "rgba(var(--ink),var(--ta-72, .72))", fontSize: 12, fontWeight: 700 }}>
                          {doc}
                        </span>
                      ))}
                    </div>
            ...
            {activeTab === "workflow" && (
              <div className="demo-workflow-grid" style={{ display: "grid", gridTemplateColumns: "1.2fr 1fr", gap: 18, animation: "fadeUp .3s ease both" }}>
                <div style={{ background: "rgba(var(--ink),.025)", border: `1px solid color-mix(in srgb, ${biz.color} 9.4%, transparent)`, borderRadius: 22, padding: "24px 24px" }}>
                  ...
                  <div style={{ display: "grid", gap: 16 }}>
                    {biz.workflow.map((step, i) => (
                      <div key={step.step} style={{ display: "flex", gap: 14, alignItems: "flex-start" }}>
                        <div style={{ width: 34, height: 34, borderRadius: 999, background: i === 0 ? biz.gradient : `color-mix(in srgb, ${biz.color} 12.5%, transparent)`, color: i === 0 ? "#fff" : biz.color, display: "flex", alignItems: "center", justifyContent: "center", fontSize: 14, fontWeight: 900, flexShrink: 0, boxShadow: i === 0 ? `0 4px 14px color-mix(in srgb, ${biz.color} 20.8%, transparent)` : "none" }}>
                          {i + 1}
                        </div>
                        <div>
                          <div style={{ fontSize: 14, fontWeight: 800, marginBottom: 4 }}>{step.step}</div>
                          <div style={{ fontSize: 13, color: "rgba(var(--ink),var(--ta-48, .48))", lineHeight: 1.6 }}>{step.detail}</div>
                        </div>
                      </div>
                    ))}
                  </div>
                </div>
                <div style={{ background: "rgba(var(--ink),.025)", border: "1px solid rgba(var(--ink),.07)", borderRadius: 22, padding: 22 }}>
                  <div style={{ fontSize: 15, fontWeight: 800, marginBottom: 14 }}>What the owner sees</div>
                  <div style={{ display: "grid", gap: 10 }}>
                    {biz.insights.map((ins) => (
                      <div key={ins} style={{ padding: "13px 14px", borderRadius: 14, background: "rgba(var(--ink),.03)", border: "1px solid rgba(var(--ink),.05)", fontSize: 13, color: "rgba(var(--ink),var(--ta-68, .68))", lineHeight: 1.6 }}>
                        {ins}
                      </div>
                    ))}
                  </div>
                  <div style={{ marginTop: 18, padding: "16px 18px", borderRadius: 16, background: `color-mix(in srgb, ${biz.color} 6.3%, transparent)`, border: `1px solid color-mix(in srgb, ${biz.color} 14.5%, transparent)` }}>
              ...
              <div style={{ animation: "fadeUp .3s ease both" }}>
                <div className="demo-ai-feature-grid" style={{ display: "grid", gridTemplateColumns: "1fr 1fr 1fr", gap: 14, marginBottom: 18 }}>
                  {biz.aiFeatures.map((feat) => (
                    <div key={feat} style={{ padding: "20px 18px", borderRadius: 18, background: "linear-gradient(135deg,rgba(99,102,241,.1),rgba(167,139,250,.06))", border: "1px solid rgba(99,102,241,.2)", fontSize: 14, color: "rgba(var(--ink),.78)", lineHeight: 1.65, fontWeight: 600 }}>
                      {feat}
                    </div>
                  ))}
                </div>
                <div className="demo-ai-bottom" style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 14 }}>
                  <div style={{ padding: "22px 22px", borderRadius: 20, background: "rgba(var(--ink),.025)", border: "1px solid rgba(var(--ink),.07)" }}>
                    <div style={{ fontSize: 15, fontWeight: 800, marginBottom: 4 }}>AI Chat — Ask anything</div>
                    <div style={{ fontSize: 13, color: "rgba(var(--ink),var(--ta-45, .45))", lineHeight: 1.7, marginBottom: 14 }}>
                    ...
                    <div style={{ display: "flex", flexDirection: "column", gap: 8 }}>
                      {["What is my revenue trend this month?", "Which customers owe the most?", "What should I focus on this week?"].map((q) => (
                        <div key={q} style={{ padding: "9px 12px", borderRadius: 10, background: "rgba(var(--ink),.04)", border: "1px solid rgba(var(--ink),.06)", fontSize: 12, color: "rgba(var(--ink),var(--ta-55, .55))", fontStyle: "italic" }}>
                          &ldquo;{q}&rdquo;
                        </div>
                      ))}
                    </div>
                  ...
                  <div style={{ padding: "22px 22px", borderRadius: 20, background: "rgba(var(--ink),.025)", border: "1px solid rgba(var(--ink),.07)" }}>
                    ...
                    <div className="demo-ai-metric-grid" style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 8 }}>
                      {["Revenue Forecast", "Anomaly Alerts", "Health Score", "Smart Recommendations", "Market Intelligence", "Business Advisor"].map((feat) => (
                        <div key={feat} style={{ padding: "8px 10px", borderRadius: 10, background: "rgba(99,102,241,.08)", border: "1px solid rgba(99,102,241,.15)", fontSize: 12, fontWeight: 700, color: "var(--tx-a5b4fc, #a5b4fc)", textAlign: "center" }}>
                          {feat}
                        </div>
                      ))}
                    </div>
            ...
            <div className="demo-bottom-cta" style={{ marginTop: 28, padding: "28px 32px", borderRadius: 24, background: `linear-gradient(135deg,color-mix(in srgb, ${biz.color} 7.1%, transparent),rgba(var(--ink),.03))`, border: `1px solid color-mix(in srgb, ${biz.color} 14.5%, transparent)` }}>
              <div style={{ fontSize: 20, fontWeight: 900, letterSpacing: -0.5, marginBottom: 8 }}>
                How the {biz.label} demo works
              </div>
              <div style={{ fontSize: 13, color: "rgba(var(--ink),var(--ta-55, .55))", lineHeight: 1.75, marginBottom: 16 }}>
              ...
              <div style={{ display: "flex", flexWrap: "wrap", gap: 10 }}>
                {[
                  ...
                  <div key={pt.text} style={{ display: "inline-flex", alignItems: "center", gap: 8, padding: "8px 14px", borderRadius: 999, background: "rgba(var(--ink),.04)", border: "1px solid rgba(var(--ink),.08)", fontSize: 12, color: "rgba(var(--ink),var(--ta-70, .7))", fontWeight: 600 }}>
