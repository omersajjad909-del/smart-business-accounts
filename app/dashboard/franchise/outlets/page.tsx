...
const S = {
  ...
  row3:        { display: "grid", gridTemplateColumns: isMobile ? "1fr" : "1fr 1fr 1fr", gap: "1rem" },
    ...
    <div style={S.page}>
      ...
      <div style={S.kpiGrid}>
        ...
        <div style={S.kpi}>
          ...
          <div style={{ ...S.kpiVal, color: "var(--tx-34d399, #34d399)" }}>{kpis.activeCount}</div>
        </div>
        <div style={S.kpi}>
          <div style={S.kpiLabel}>Monthly Revenue</div>
          <div style={{ ...S.kpiVal, color: "var(--tx-818cf8, #818cf8)" }}>${kpis.totalRev.toLocaleString()}</div>
        </div>
        <div style={S.kpi}>
          <div style={S.kpiLabel}>Avg Sales / Outlet</div>
          <div style={{ ...S.kpiVal, color: "var(--tx-38bdf8, #38bdf8)" }}>${kpis.avgSales.toLocaleString(undefined, { maximumFractionDigits: 0 })}</div>
        </div>
      </div>
