  ...
  return (
    <div style={{ padding: isMobile ? "15px 14px" : "28px 32px", color: "var(--tx-e2e8f0, #e2e8f0)", fontFamily: "'Outfit','Inter',sans-serif" }}>
      <div style={{ marginBottom: 24 }}>
        <h1 style={{ margin: "0 0 6px", fontSize: 24, fontWeight: 800, color: "var(--ink-solid, white)" }}>Franchise Analytics</h1>
      ...
      <div style={{ display: "grid", gridTemplateColumns: isMobile ? "1fr" : "repeat(auto-fit,minmax(180px,1fr))", gap: 14, marginBottom: 24 }}>
        {card("Outlets", stats.outlets, "#22c55e", isMobile)}
        {card("Active Outlets", stats.activeOutlets, "#60a5fa", isMobile)}
        {card("Monthly Sales", stats.monthlySales.toLocaleString(), "#fbbf24", isMobile)}
        {card("Royalty Value", stats.royaltyValue.toLocaleString(), "#a78bfa", isMobile)}
        {card("Received Cycles", stats.received, "#34d399", isMobile)}
      </div>

      <div style={{ display: "grid", gridTemplateColumns: "1.1fr .9fr", gap: 16 }}>
        <div style={{ background: "rgba(var(--ink),.03)", border: "1px solid rgba(var(--ink),.07)", borderRadius: 16, padding: 20 }}>
          <div style={{ fontSize: 15, fontWeight: 700, color: "var(--ink-solid, white)", marginBottom: 14 }}>Outlet Snapshot</div>
          {outletStore.loading ? (
            <div style={{ fontSize: 13, color: "rgba(var(--ink),var(--ta-45, .45))" }}>Loading outlets...</div>
          ) : outlets.length ? (
            <table style={{ width: "100%", borderCollapse: "collapse" }}>
              ...
              <tbody>
                {outlets.slice(0, 8).map((item) => (
                  <tr key={item.id}>
                    <td style={{ padding: "14px 12px 14px 0", borderBottom: "1px solid rgba(var(--ink),.04)", fontSize: 13 }}>{item.outletName}</td>
                    <td style={{ padding: "14px 12px 14px 0", borderBottom: "1px solid rgba(var(--ink),.04)", fontSize: 13 }}>{item.location || "-"}</td>
                    <td style={{ padding: "14px 12px 14px 0", borderBottom: "1px solid rgba(var(--ink),.04)", fontSize: 13 }}>{item.franchisee || "-"}</td>
                    <td style={{ padding: "14px 12px 14px 0", borderBottom: "1px solid rgba(var(--ink),.04)", fontSize: 13 }}>{Number(item.monthlySales || 0).toLocaleString()}</td>
            ...
            <div style={{ fontSize: 13, color: "rgba(var(--ink),var(--ta-45, .45))" }}>No outlets available yet.</div>
          )}
        </div>

        <div style={{ background: "rgba(var(--ink),.03)", border: "1px solid rgba(var(--ink),.07)", borderRadius: 16, padding: 20 }}>
          <div style={{ fontSize: 15, fontWeight: 700, color: "var(--ink-solid, white)", marginBottom: 12 }}>Royalty Snapshot</div>
          ...
          )) : <div style={{ fontSize: 13, color: "rgba(var(--ink),var(--ta-45, .45))" }}>No royalty cycles available yet.</div>}
        </div>
      </div>
    </div>
  );
