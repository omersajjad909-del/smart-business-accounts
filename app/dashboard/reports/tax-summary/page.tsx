...
export default function TaxSummaryPage() {
  ...
  const th: React.CSSProperties = { padding:"10px 14px", fontSize:10, fontWeight:700, color:"rgba(var(--ink),var(--ta-35, .35))", textTransform:"uppercase", letterSpacing:".08em", borderBottom:"1px solid rgba(var(--ink),.07)", background:"rgba(var(--ink),.03)", whiteSpace:"nowrap" };
              ...
              <div style={{ background:"rgba(var(--ink),.03)", border:"1px solid rgba(var(--ink),.08)", borderRadius:16, overflow:"hidden" }}>
                <div style={{ padding: isMobile ? "12px 11px" : "20px 24px", background:"linear-gradient(135deg,rgba(245,158,11,.1),rgba(217,119,6,.05))", borderBottom:"1px solid rgba(var(--ink),.07)", display:"flex", justifyContent:"space-between", alignItems:"center" }}>
                  <div>
                    ...
                    <div style={{ fontSize:11, color:"rgba(var(--ink),var(--ta-35, .35))", marginTop:3 }}>Period: {from} — {to}</div>
                  </div>
                ...
                <div style={{ overflowX:"auto" }}>
                  <table style={{ width:"100%", borderCollapse:"collapse" }}>
                    ...
                    <tbody>
                      {data.length === 0 ? (
                        <tr><td colSpan={7} style={{ padding: isMobile ? "22px 10px" : "40px 16px", textAlign:"center", color:"rgba(var(--ink),var(--ta-20, .2))", fontSize:13 }}>No tax data found for this period</td></tr>
                      ) : data.map((d, i) => (
