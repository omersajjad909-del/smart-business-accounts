...
export default function ProductProfitabilityPage() {
  ...
  useEffect(() => { setLoading(true); fetch(`/api/reports/product-profitability?period=${period}`, { headers: h() }).then(r => r.ok ? r.json() : {}).then((d: any) => { setData(d.rows || []); setLoading(false); }).catch(() => setLoading(false)); }, [period]);
