...
export default function TrialBalancePage() {
  ...
  const thStyle = (right = false): React.CSSProperties => ({
    padding: "10px 14px", fontSize: 10, fontWeight: 700, color: "rgba(var(--ink),.35)",
        ...
        <>
          ...
          <div style={{ display:"flex", alignItems:"center", justifyContent:"space-between", marginBottom:24, flexWrap:"wrap", gap:10 }} className="print:hidden">
            ...
            <div style={{ display:"flex", gap:8 }}>
              ...
              <button onClick={() => window.print()} style={{ padding:"8px 16px", borderRadius:9, border:"1px solid rgba(var(--ink),.1)", background:"rgba(var(--ink),.04)", color:"rgba(var(--ink),.5)", fontSize:12, fontWeight:700, cursor:"pointer", fontFamily:"inherit" }}>
                🖨 Print
              </button>
              <button
          ...
          <div className="print-doc-a4 tb-doc" style={{ background:"rgba(var(--ink),.03)", border:"1px solid rgba(var(--ink),.08)", borderRadius:16, overflow:"hidden" }}>
            ...
            {loading ? (
              <div style={{ padding:"80px 0", textAlign:"center", color:"rgba(var(--ink),.25)", fontSize:13 }}>Loading report…</div>
            ) : rows.length === 0 ? (
              <div style={{ padding:"80px 0", textAlign:"center", color:"rgba(var(--ink),.2)", fontSize:13 }}>No data found for this period</div>
            ) : (
              <>
                  ...
                  <table style={{ width:"100%", minWidth:1040, borderCollapse:"collapse", tableLayout:"fixed", fontSize:12 }}>
                    ...
                    <tfoot>
                      <tr className="tb-grand" style={{ background:"rgba(99,102,241,.08)" }}>
                        <td colSpan={2} style={{ ...grandTd, textAlign:"left", fontSize:10, fontWeight:800, color:"rgba(var(--ink),.4)", letterSpacing:".1em", textTransform:"uppercase" }}>
                          Grand Total{cur ? ` (${cur})` : ""}
                        </td>
                ...
                {cur && <div style={{ padding:"12px 24px", borderTop:"1px solid rgba(var(--ink),.05)", fontSize:11, color:"rgba(var(--ink),.2)" }}>All amounts in {cur}</div>}
              </>
            )}
          </div>
        </>
