...
export default function CustomerProfitabilityPage() {
  ...
  useEffect(() => { setLoading(true); fetch(`/api/reports/customer-profitability?period=${period}`, { headers: h() }).then(r => r.ok ? r.json() : {}).then((d: any) => { setData(d.rows || []); setLoading(false); }).catch(() => setLoading(false)); }, [period]);
