...
function PLRow({ label, amount, variant = "normal", indent = false, dimZero = false }: {
  ...
  const styles: Record<RowVariant, React.CSSProperties> = {
    normal:   { fontSize: 13, color: "rgba(var(--ink),var(--ta-65, .65))", fontWeight: 400 },
      ...
      {showModal && (
        <div style={{ position: "fixed", inset: 0, zIndex: 9000, background: "rgba(0,0,0,.82)", backdropFilter: "blur(14px)", display: "flex", alignItems: "center", justifyContent: "center", padding: 24 }}>
          <div style={{ width: "100%", maxWidth: 460, background: "rgba(var(--dkr-0a0d20, 10,13,32),0.97)", border: "1px solid rgba(var(--ink),.12)", borderRadius: 22, padding: "40px 40px 36px", boxShadow: "0 40px 100px rgba(0,0,0,.8)", position: "relative" }}>
            <button onClick={() => report ? setShowModal(false) : router.back()} style={{ position: "absolute", top: 14, right: 16, background: "none", border: "none", color: "rgba(var(--ink),var(--ta-35, .35))", fontSize: 20, cursor: "pointer", padding: 4, borderRadius: 6 }}>✕</button>
      ...
      {!showModal && (
        <>
          ...
          <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", marginBottom: 24, flexWrap: "wrap", gap: 10 }}>
            <div>
              <div style={{ fontSize: 11, color: "rgba(var(--ink),var(--ta-30, .3))", marginBottom: 4 }}>Reports › <span style={{ color: "var(--tx-a5b4fc, #a5b4fc)" }}>Profit & Loss</span></div>
            ...
            <div style={{ display: "flex", gap: 8, alignItems: "center" }}>
              <button onClick={() => setShowModal(true)} style={{ padding: "8px 14px", borderRadius: 9, border: "1px solid rgba(var(--ink),.1)", background: "rgba(var(--ink),.04)", color: "rgba(var(--ink),var(--ta-55, .55))", fontSize: 12, fontWeight: 600, cursor: "pointer", fontFamily: ff }}>
                ⟵ Change Dates
              </button>
              <button onClick={() => window.print()} style={{ padding: "8px 14px", borderRadius: 9, border: "1px solid rgba(var(--ink),.1)", background: "rgba(var(--ink),.04)", color: "rgba(var(--ink),var(--ta-50, .5))", fontSize: 12, fontWeight: 600, cursor: "pointer", fontFamily: ff }}>
                🖨 Print
              </button>
          ...
          {loading && (
            <div style={{ display: "flex", flexDirection: "column", alignItems: "center", justifyContent: "center", padding: "80px 0", gap: 16, color: "rgba(var(--ink),var(--ta-30, .3))" }}>
              ...
              <>
                ...
                <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fit,minmax(170px,1fr))", gap: 12, marginBottom: 24 }}>
                  {[
                    ...
                    <div key={k.label} style={{ background: `${k.color}0e`, border: `1px solid ${k.color}28`, borderRadius: 13, padding: isMobile ? "12px 10px" : "14px 16px" }}>
                      <div style={{ fontSize: 9, fontWeight: 700, color: "rgba(var(--ink),var(--ta-32, .32))", letterSpacing: ".08em", textTransform: "uppercase", marginBottom: 8, display: "flex", alignItems: "center", gap: 5 }}>
                ...
                <Section title="Cost of Goods Sold — COGS" color="#fb923c" icon="📦">
                  {r.cogsLines.length === 0
                    ? <div style={{ textAlign: "center", padding: "16px 0", color: "rgba(var(--ink),var(--ta-25, .25))", fontSize: 13 }}>No cost of sales recorded in this period</div>
                ...
                <Section title="Operating Expenses" color="#f87171" icon="⚙️">
                  {r.operatingExpenses.length === 0 ? (
                    <div style={{ textAlign: "center", padding: "16px 0", color: "rgba(var(--ink),var(--ta-25, .25))", fontSize: 13 }}>No operating expenses recorded in this period</div>
                ...
                <div style={{
                  ...
                  <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center" }}>
                    <div>
                      <div style={{ fontSize: 10, fontWeight: 700, color: "rgba(var(--ink),var(--ta-35, .35))", letterSpacing: ".1em", textTransform: "uppercase", marginBottom: 5 }}>
                        {isProfit ? "✅ NET PROFIT AFTER TAX" : "⚠️ NET LOSS AFTER TAX"}
                      </div>
                      <div style={{ fontSize: 11, color: "rgba(var(--ink),var(--ta-30, .3))" }}>
                        Period: {from} → {to}
