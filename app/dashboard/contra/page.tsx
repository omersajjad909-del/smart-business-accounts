        ...
        <div style={{ display:"flex", alignItems:"center", gap:10 }}>
          {queryIdx>=0 && !queryMode && (
            <div style={{ display:"flex", alignItems:"center", gap:6, background:"rgba(20,184,166,.08)", border:"1px solid rgba(20,184,166,.2)", borderRadius:10, padding:"6px 12px" }}>
              <button onClick={()=>navTo(queryIdx-1)} disabled={queryIdx===0} style={{ padding:"4px 10px", borderRadius:6, background:"rgba(var(--ink),.06)", border:"1px solid rgba(var(--ink),.1)", color:queryIdx===0?"rgba(var(--ink),var(--ta-20, .2))":"rgba(var(--ink),var(--ta-70, .7))", fontSize:13, cursor:queryIdx===0?"default":"pointer", fontFamily:ff }}>◀</button>
              ...
              <button onClick={()=>navTo(queryIdx+1)} disabled={queryIdx===queryResults.length-1} style={{ padding:"4px 10px", borderRadius:6, background:"rgba(var(--ink),.06)", border:"1px solid rgba(var(--ink),.1)", color:queryIdx===queryResults.length-1?"rgba(var(--ink),var(--ta-20, .2))":"rgba(var(--ink),var(--ta-70, .7))", fontSize:13, cursor:queryIdx===queryResults.length-1?"default":"pointer", fontFamily:ff }}>▶</button>
