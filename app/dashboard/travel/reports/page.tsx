...
const th: React.CSSProperties = {
  ...
  letterSpacing: ".05em", textTransform: "uppercase", color: "rgba(255,255,255,.38)",
  borderBottom: `1px solid ${border}`, whiteSpace: "nowrap",
};
const td: React.CSSProperties = {
  padding: "10px 12px", fontSize: 13, borderBottom: `1px solid ${border}`, color: "rgba(255,255,255,.8)",
};
...
export default function TravelReportsPage() {
  ...
  return (
    <div style={{ padding: isMobile ? "15px 13px" : "24px 28px", fontFamily: ff, color: "#fff" }}>
      <h1 style={{ fontSize: 22, fontWeight: 800, margin: "0 0 5px" }}>Travel Reports</h1>
      <p style={{ fontSize: 13, color: "rgba(255,255,255,.42)", margin: "0 0 18px" }}>
        ...
        {TABS.map((t) => (
          <button key={t.key} onClick={() => setTab(t.key)}
            style={{
              ...
              color: tab === t.key ? accent : "rgba(255,255,255,.6)",
            }}>
            {t.label}
          </button>
        ))}
            ...
            <tbody>
              {profit.map((p) => (
                <tr key={p.id}>
                  ...
                  <td style={{ ...num, color: "rgba(255,255,255,.5)" }}>{p.cost.toLocaleString()}</td>
                  <td style={{ ...num, fontWeight: 700, color: p.margin >= 0 ? "#34d399" : "#fca5a5" }}>
                    {p.margin.toLocaleString()}
                  </td>
                  <td style={{ ...num, color: "rgba(255,255,255,.5)" }}>{p.marginPercent}%</td>
                  <td style={num}>
              ...
              {profit.length === 0 && (
                <tr><td colSpan={7} style={{ ...td, textAlign: "center", color: "rgba(255,255,255,.3)", padding: 36 }}>No departures yet.</td></tr>
              )}
            </tbody>
          </table>
          {profit.length > 0 && (
            <div style={{ padding: "11px 14px", fontSize: 11.5, color: "rgba(255,255,255,.35)", lineHeight: 1.6 }}>
          ...
          <div style={{ display: "grid", gridTemplateColumns: isMobile ? "1fr 1fr" : "repeat(4,1fr)", gap: 12, marginBottom: 16 }}>
            {buckets.map((b) => (
              <div key={b.key} style={{ background: bg, border: `1px solid ${b.key === "current" ? border : "rgba(239,68,68,.2)"}`, borderRadius: 14, padding: "14px 16px" }}>
                <div style={{ fontSize: 11.5, color: "rgba(255,255,255,.45)", marginBottom: 5 }}>{b.label}</div>
                <div style={{ fontSize: 18, fontWeight: 800, color: b.key === "current" ? "rgba(255,255,255,.75)" : b.total > 0 ? "#fca5a5" : "rgba(255,255,255,.3)" }}>
                  {b.total.toLocaleString()}
                </div>
                <div style={{ fontSize: 11, color: "rgba(255,255,255,.3)" }}>{b.count} instalment{b.count === 1 ? "" : "s"}</div>
              </div>
            ))}
          </div>
          <div style={{ background: bg, border: `1px solid ${border}`, borderRadius: 14, overflowX: "auto" }}>
              ...
              <tbody>
                {owing.map((l) => (
                  <tr key={l.id}>
                    ...
                    <td style={{ ...td, color: "rgba(255,255,255,.5)" }}>{l.departure}</td>
                    <td style={td}>{l.due || "—"}</td>
                    <td style={{ ...num, color: l.daysLate > 0 ? "#fca5a5" : "rgba(255,255,255,.35)", fontWeight: l.daysLate > 0 ? 700 : 400 }}>
                ...
                {owing.length === 0 && (
                  <tr><td colSpan={5} style={{ ...td, textAlign: "center", color: "rgba(255,255,255,.3)", padding: 36 }}>Nothing outstanding.</td></tr>
                )}
              </tbody>
            </table>
          </div>
        ...
        <>
          <div className="no-print" style={{ display: "flex", gap: 10, marginBottom: 16, flexWrap: "wrap" }}>
            <select value={manifestId} onChange={(e) => setManifestId(e.target.value)}
              style={{ minWidth: 280, background: "#161b27", border: `1px solid ${border}`, borderRadius: 9, padding: "9px 12px", color: "#fff", fontSize: 13, fontFamily: "inherit" }}>
              ...
              <table style={{ width: "100%", borderCollapse: "collapse", minWidth: 720 }}>
                ...
                <tbody>
                  {manifest.people.map((p, i) => (
                    <tr key={p.id}>
                      <td style={{ ...td, color: "rgba(255,255,255,.35)" }}>{i + 1}</td>
                      ...
                      <td style={{ ...td, color: "rgba(255,255,255,.5)" }}>{p.party}</td>
                      <td style={{ ...td, color: "rgba(255,255,255,.5)" }}>{p.sharing}</td>
                    </tr>
                  ))}
                  {manifest.people.length === 0 && (
                    <tr><td colSpan={7} style={{ ...td, textAlign: "center", color: "rgba(255,255,255,.3)", padding: 36 }}>Nobody booked on this departure yet.</td></tr>
                  )}
                </tbody>
              </table>
            </div>
