    ...
    <div style={{ padding: isMobile ? "15px 14px" : "28px 32px", fontFamily: ff, color: "var(--tx-e2e8f0, #e2e8f0)" }}>
      ...
      <div style={{ display: "flex", justifyContent: "space-between", alignItems: "flex-start", gap: 20, flexWrap: "wrap", marginBottom: 28 }}>
        <div>
          ...
          <p style={{ margin: 0, fontSize: 13, color: "rgba(var(--ink),var(--ta-45, .45))" }}>Complete business performance — revenue, margin, pipeline, and supplier exposure.</p>
        </div>
      ...
      {loading && <div style={{ textAlign: "center", padding: 60, color: "rgba(var(--ink),var(--ta-40, .4))" }}>Loading analytics...</div>}
        ...
        <>
          ...
          <div style={{ display: "grid", gridTemplateColumns: isMobile ? "1fr" : "repeat(auto-fit,minmax(160px,1fr))", gap: 14, marginBottom: 24 }}>
            {[
              ...
              <div key={k.label} style={{ background: panelBg, border: `1px solid ${panelBorder}`, borderRadius: 14, padding: isMobile ? "12px 10px" : "16px 20px" }}>
                <div style={{ fontSize: 11, color: "rgba(var(--ink),var(--ta-40, .4))", textTransform: "uppercase", letterSpacing: ".06em", marginBottom: 8 }}>{k.label}</div>
          ...
          <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 20, marginBottom: 20 }}>
            ...
            <div style={{ background: panelBg, border: `1px solid ${panelBorder}`, borderRadius: 16, padding: 22 }}>
              ...
              {byModule.length === 0 ? (
                <div style={{ fontSize: 13, color: "rgba(var(--ink),var(--ta-35, .35))", padding: "20px 0" }}>No data yet.</div>
              ) : byModule.map(m => {
                ...
                return (
                  <div key={m.module} style={{ marginBottom: 16 }}>
                    <div style={{ display: "flex", justifyContent: "space-between", marginBottom: 6 }}>
                      ...
                      <span style={{ fontSize: 13, color: "rgba(var(--ink),var(--ta-60, .6))" }}>Rs. {m.revenue.toLocaleString()} ({pct}%)</span>
                    </div>
                    ...
                    <div style={{ fontSize: 11, color: "rgba(var(--ink),var(--ta-35, .35))", marginTop: 4 }}>{m.count} records | Margin: Rs. {(m.revenue - m.cost).toLocaleString()}</div>
                  </div>
                );
              })}
            </div>
            ...
            <div style={{ background: panelBg, border: `1px solid ${panelBorder}`, borderRadius: 16, padding: 22 }}>
              ...
              {topSuppliers.length === 0 ? (
                <div style={{ fontSize: 13, color: "rgba(var(--ink),var(--ta-35, .35))", padding: "20px 0" }}>No settlements yet.</div>
                ...
                <div key={s.name} style={{ display: "flex", justifyContent: "space-between", alignItems: "center", padding: "12px 0", borderBottom: i < topSuppliers.length - 1 ? `1px solid ${panelBorder}` : "none" }}>
                  <div>
                    ...
                    <div style={{ fontSize: 11, color: "rgba(var(--ink),var(--ta-40, .4))", marginTop: 2 }}>{s.count} settlement{s.count !== 1 ? "s" : ""}</div>
                  </div>
          ...
          <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 20 }}>
            ...
            <div style={{ background: panelBg, border: `1px solid ${panelBorder}`, borderRadius: 16, padding: 22 }}>
              ...
              {statusBreakdown.map(mod => (
                <div key={mod.module} style={{ marginBottom: 18 }}>
                  <div style={{ fontSize: 12, color: "rgba(var(--ink),var(--ta-50, .5))", fontWeight: 700, textTransform: "uppercase", letterSpacing: ".06em", marginBottom: 8 }}>{mod.module}</div>
                  <div style={{ display: "flex", gap: 8, flexWrap: "wrap" }}>
                    {mod.statuses.map(s => (
              ...
              {statusBreakdown.length === 0 && <div style={{ fontSize: 13, color: "rgba(var(--ink),var(--ta-35, .35))" }}>No records yet.</div>}
            </div>
            ...
            <div style={{ background: panelBg, border: `1px solid ${panelBorder}`, borderRadius: 16, padding: 22 }}>
              ...
              {recentActivity.length === 0 ? (
                <div style={{ fontSize: 13, color: "rgba(var(--ink),var(--ta-35, .35))" }}>No recent activity.</div>
                ...
                <div key={a.id} style={{ display: "flex", justifyContent: "space-between", alignItems: "center", padding: "10px 0", borderBottom: `1px solid ${panelBorder}` }}>
                  <div>
                    ...
                    <div style={{ fontSize: 11, color: "rgba(var(--ink),var(--ta-40, .4))", marginTop: 2 }}>{a.type} | {a.customer}</div>
                  </div>
