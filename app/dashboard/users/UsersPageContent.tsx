...
export default function UsersPage() {
  ...
  if (!currentUser || currentUser.role !== "ADMIN") return (
    <div style={{ display:"flex", alignItems:"center", justifyContent:"center", minHeight:"50vh" }}>
      <div style={{ padding:32, borderRadius:16, background:"rgba(239,68,68,0.08)", border:"1px solid rgba(239,68,68,0.2)", textAlign:"center" }}>
        ...
        <div style={{ fontSize:13, color:"rgba(var(--ink),var(--ta-40, 0.4))", marginTop:6 }}>Only ADMIN can manage users.</div>
      </div>
    </div>
  );
  ...
  const btnGhost: React.CSSProperties = {
    padding:"10px 20px", borderRadius:8,
    background:"rgba(var(--ink),0.05)", border:"1px solid rgba(var(--ink),0.1)",
    color:"rgba(var(--ink),var(--ta-60, 0.6))", fontWeight:600, fontSize:13,
    cursor:"pointer", transition:"all .2s",
  };
    ...
    <div style={{ maxWidth:900, margin:"0 auto", fontFamily:"inherit" }}>
      ...
      <div style={{ display:"flex", alignItems:"center", justifyContent:"space-between", marginBottom:24 }}>
        <div>
          ...
          <p style={{ fontSize:12, color:"rgba(var(--ink),var(--ta-35, 0.35))", marginTop:4 }}>Manage team members and their access</p>
        </div>
        ...
        <div style={{ display:"flex", alignItems:"center", gap:10 }}>
          <div style={{
            ...
            <div style={{ fontSize:10, fontWeight:700, textTransform:"uppercase", letterSpacing:".06em", color:"rgba(var(--ink),var(--ta-40, 0.4))", marginBottom:2 }}>
              {plan} Plan — Users
            </div>
      ...
      {atLimit && (
        <div style={{ ...card, background:"rgba(248,113,113,0.08)", border:"1px solid rgba(248,113,113,0.25)", marginBottom:16, display:"flex", alignItems:"center", gap:14 }}>
          ...
          <div>
            ...
            <div style={{ fontSize:12, color:"rgba(var(--ink),var(--ta-45, 0.45))", marginTop:2 }}>
      ...
      {/* Invite User */}
      <div style={card}>
        <div style={{ fontSize:13, fontWeight:700, color:"rgba(var(--ink),var(--ta-70, 0.7))", marginBottom:14 }}>
          Invite Team Member
        </div>
        ...
        {inviteLink && (
          <div style={{ marginTop:12 }}>
            <div style={{ fontSize:11, fontWeight:600, color:"rgba(var(--ink),var(--ta-40, 0.4))", marginBottom:6 }}>Invite Link</div>
          ...
          <thead>
            <tr style={{ borderBottom:"1px solid rgba(var(--ink),0.06)" }}>
              {["Name","Email","Role","Status","Actions"].map((h,i) => (
                <th key={h} style={{ padding:"12px 16px", textAlign: i===4 ? "right" : "left", fontSize:11, fontWeight:700, color:"rgba(var(--ink),var(--ta-35, 0.35))", textTransform:"uppercase", letterSpacing:".06em" }}>{h}</th>
              ))}
            </tr>
          </thead>
          <tbody>
            {users.length === 0 ? (
              <tr><td colSpan={5} style={{ padding:32, textAlign:"center", fontSize:13, color:"rgba(var(--ink),var(--ta-20, 0.2))" }}>No users found</td></tr>
            ) : users.map((u) => (
              <tr key={u.id} style={{ borderBottom:"1px solid rgba(var(--ink),0.04)" }}>
                ...
                <td style={{ padding:"12px 16px", fontSize:12, color:"rgba(var(--ink),var(--ta-45, 0.45))" }}>{u.email}</td>
                ...
                <td style={{ padding:"12px 16px" }}>
                  <div style={{ display:"flex", alignItems:"center", gap:6 }}>
                    ...
                    <span style={{ fontSize:12, color:"rgba(var(--ink),var(--ta-45, 0.45))" }}>{u.active ? "Active" : "Disabled"}</span>
                  </div>
                </td>
