  ...
  return (
    <div style={{ padding: isMobile ? "15px 14px" : "28px 32px", fontFamily: ff, color: "var(--ink-solid, #fff)", minHeight: "100vh" }}>
      <div style={{ marginBottom: 24 }}>
        ...
        <p style={{ fontSize: 13, color: "rgba(var(--ink),var(--ta-45, .45))", margin: 0 }}>Production throughput, material risk, and quality performance.</p>
      </div>
      ...
      <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 18 }}>
        <div style={{ background: bg, border: `1px solid ${border}`, borderRadius: 12, padding: 18 }}>
          ...
          <div style={{ display: "grid", gap: 10 }}>
            ...
            {data.materials.filter((item) => item.isLow).length === 0 && <div style={{ color: "rgba(var(--ink),var(--ta-45, .45))", fontSize: 13 }}>No low materials.</div>}
          </div>
        </div>
