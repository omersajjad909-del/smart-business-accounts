...
/**
 * Trading goods are bought-in items a factory resells as-is or drops straight
 * into a BOM (buttons, zips, packaging) — not made on the floor, so they carry
 * no production cost of their own. Same real-inventory items as Raw Materials
 * and Finished Goods (ItemNew, category TRADING), stock and cost read from
 * InventoryTxn rather than typed in here.
 */
export default function TradingGoodsPage() {
  ...
  const [form, setForm] = useState({ name: "", unit: "pcs", minStock: 10, purchaseRate: 0, rate: 0 });

  const boms = useMemo(() => bomStore.records.map(mapBomRecord), [bomStore.records]);

  const reload = useCallback(async () => {
    setLoading(true);
    setItems(await loadManufacturingItems("TRADING"));
    setLoading(false);
  }, []);
  ...
  async function save() {
    ...
    try {
      const res = await fetch("/api/manufacturing/items", {
        ...
        body: JSON.stringify({ ...form, name, category: "TRADING" }),
      });
      const body = await res.json();
      if (!res.ok) throw new Error(body?.error || "Could not save the item.");
      setShowModal(false);
      setForm({ name: "", unit: "pcs", minStock: 10, purchaseRate: 0, rate: 0 });
      await reload();
    } catch (e) {
  ...
  return (
    <div style={{ padding: isMobile ? "15px 14px" : "28px 32px", fontFamily: ff, color: "var(--ink-solid, #fff)", minHeight: "100vh" }}>
      <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 22, gap: 14, flexWrap: "wrap" }}>
        ...
        <button onClick={() => { setShowModal(true); setError(""); }} style={{ padding: "10px 20px", borderRadius: 10, border: "none", background: "#818cf8", color: "#fff", fontSize: 13, fontWeight: 700, cursor: "pointer" }}>
          + New Trading Good
        </button>
      </div>
      ...
      {showModal && (
        <div style={{ position: "fixed", inset: 0, background: "rgba(0,0,0,.7)", zIndex: 50, display: "flex", alignItems: "center", justifyContent: "center", padding: 16 }}>
          <div style={{ background: "var(--dk-161b27, #161b27)", border: `1px solid ${border}`, borderRadius: 16, padding: 30, width: 500, fontFamily: ff }}>
            <h2 style={{ margin: "0 0 20px", fontSize: 18, fontWeight: 700 }}>New Trading Good</h2>
