  ...
  return (
    <div style={{ padding: isMobile ? "15px 14px" : "28px 32px", color: "#e2e8f0", fontFamily: "'Outfit','Inter',sans-serif" }}>
      ...
      <div style={{ display: "grid", gridTemplateColumns: isMobile ? "1fr" : "repeat(auto-fit,minmax(180px,1fr))", gap: 14, marginBottom: 24 }}>
        {card("Active Accounts", stats.activeAccounts, utilityAccent, isMobile)}
        {card("Open Bills", stats.openBills, "#fbbf24", isMobile)}
        {card("Verified Readings", stats.verifiedReadings, "#34d399", isMobile)}
        {card("Billed Value", stats.billed.toLocaleString(), "#60a5fa", isMobile)}
        {card("Units Logged", stats.units.toLocaleString(), "#a78bfa", isMobile)}
      </div>
