...
const STATUS_PALETTE: Record<string, { bg: string; color: string }> = {
  Available:    { bg: "rgba(34,197,94,0.15)",   color: "var(--tx-4ade80, #4ade80)" },
  Reserved:     { bg: "rgba(251,191,36,0.15)",  color: "var(--tx-fbbf24, #fbbf24)" },
  Sold:         { bg: "rgba(239,68,68,0.15)",   color: "var(--tx-f87171, #f87171)" },
};
const TYPE_PALETTE: Record<string, { bg: string; color: string }> = {
  New:          { bg: "rgba(99,102,241,0.15)",  color: "var(--tx-a5b4fc, #a5b4fc)" },
  Used:         { bg: "rgba(251,191,36,0.12)",  color: "var(--tx-fbbf24, #fbbf24)" },
  "In-Transit": { bg: "rgba(139,92,246,0.15)",  color: "var(--tx-c084fc, #c084fc)" },
};
...
function Badge({ label, palette }: { label: string; palette: Record<string, { bg: string; color: string }> }) {
  const style = palette[label] ?? { bg: "rgba(120,120,120,0.2)", color: "var(--tx-aaaaaa, #aaa)" };
  return (
                  ...
                  <td style={td}>
                    <div style={{ display: "flex", gap: 7, alignItems: "center" }}>
                      ...
                      <button
                        onClick={() => remove(v.id)}
                        title="Delete"
                        style={{ background: "transparent", border: "none", color: "var(--tx-f87171, #f87171)", cursor: "pointer", fontSize: 18, padding: "0 4px", lineHeight: 1, fontFamily: FONT }}
                      >
                        ×
                      </button>
                    </div>
