    ...
    <div style={{ padding: isMobile ? "15px 14px" : "28px 32px", fontFamily: ff, color: "var(--ink-solid, #fff)", minHeight: "100vh" }}>
      <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 24 }}>
        <div>
          ...
          <p style={{ fontSize: 13, color: "rgba(var(--ink),var(--ta-42, .42))", margin: 0 }}>Define service lines, rates, and expected turnaround.</p>
        </div>
      ...
      <div style={{ display: "grid", gridTemplateColumns: isMobile ? "1fr" : "repeat(auto-fill,minmax(260px,1fr))", gap: 14 }}>
        {services.map((service) => (
          <div key={service.id} style={{ background: bg, border: `1px solid ${border}`, borderRadius: 14, padding: 20 }}>
            ...
            <div style={{ marginTop: 8, fontSize: 12, color: "rgba(var(--ink),var(--ta-45, .45))" }}>{service.scope || "No scope added"}</div>
            ...
            <div style={{ marginTop: 6, fontSize: 11, color: "rgba(var(--ink),var(--ta-40, .4))" }}>TAT: {service.turnaroundDays} days</div>
          </div>
        ))}
        {!store.loading && services.length === 0 && <div style={{ color: "rgba(var(--ink),var(--ta-30, .3))" }}>No services configured yet.</div>}
      </div>
          ...
          <div style={{ width: 520, background: "var(--dk-161b27, #161b27)", border: `1px solid ${border}`, borderRadius: 16, padding: 28 }}>
            ...
            <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 14 }}>
              <div style={{ gridColumn: "span 2" }}>
                <label style={{ display: "block", marginBottom: 6, fontSize: 12, color: "rgba(var(--ink),var(--ta-45, .45))" }}>Service Name</label>
              ...
              <div>
                <label style={{ display: "block", marginBottom: 6, fontSize: 12, color: "rgba(var(--ink),var(--ta-45, .45))" }}>Billing Type</label>
                <select value={form.billingType} onChange={(e) => setForm((current) => ({ ...current, billingType: e.target.value }))} style={{ width: "100%", background: "var(--dk-161b27, #161b27)", border: `1px solid ${border}`, borderRadius: 8, padding: "9px 12px", color: "var(--ink-solid, #fff)" }}>
              ...
              <div>
                <label style={{ display: "block", marginBottom: 6, fontSize: 12, color: "rgba(var(--ink),var(--ta-45, .45))" }}>Rate</label>
              ...
              <div style={{ gridColumn: "span 2" }}>
                <label style={{ display: "block", marginBottom: 6, fontSize: 12, color: "rgba(var(--ink),var(--ta-45, .45))" }}>Turnaround Days</label>
              ...
              <div style={{ gridColumn: "span 2" }}>
                <label style={{ display: "block", marginBottom: 6, fontSize: 12, color: "rgba(var(--ink),var(--ta-45, .45))" }}>Scope</label>
            ...
            <div style={{ display: "flex", gap: 12, marginTop: 18 }}>
              ...
              <button onClick={() => setShowModal(false)} style={{ padding: "11px 24px", borderRadius: 8, border: `1px solid ${border}`, background: "transparent", color: "rgba(var(--ink),var(--ta-65, .65))", cursor: "pointer" }}>Cancel</button>
            </div>
          </div>
        </div>
      )}
