...
export default function MaintenanceOverviewPage() {
  ...
  const dueVisits = schedules.filter((row) => row.status === "due_today" || row.status === "scheduled").length;
  const openJobs = jobs.filter((row) => !["Completed", "Cancelled"].includes(String(row.status))).length;
  const lowStock = parts.filter((row) => row.status === "low_stock" || row.quantity <= row.reorderLevel).length;
  const contractValue = contracts.reduce((sum, row) => sum + row.value, 0);
  ...
  return (
    <div style={{ minHeight: "100vh", padding: "28px 32px", color: "#fff", fontFamily: maintenanceFont }}>
      ...
      <div style={{ display: "grid", gridTemplateColumns: "repeat(5,minmax(0,1fr))", gap: 12, marginBottom: 24 }}>
        {[
          { label: "Contracts", value: activeContracts, color: "#34d399" },
          { label: "Due Visits", value: dueVisits, color: "#f59e0b" },
          { label: "Open Jobs", value: openJobs, color: "#60a5fa" },
          { label: "Low Stock", value: lowStock, color: "#f87171" },
          { label: "Contract Value", value: `Rs. ${contractValue.toLocaleString()}`, color: "#22c55e" },
        ].map((card) => (
        ...
        <div style={{ display: "grid", gap: 18 }}>
          ...
          <div style={{ background: maintenanceBg, border: `1px solid ${maintenanceBorder}`, borderRadius: 20, padding: 22 }}>
            ...
            <div style={{ display: "grid", gap: 12 }}>
              {[
                { label: "Scheduled visits", value: `${schedules.filter((row) => row.status === "scheduled").length} upcoming`, tone: "#60a5fa" },
                { label: "Completed jobs", value: `${jobs.filter((row) => row.status === "Completed").length} closed`, tone: "#22c55e" },
                { label: "Urgent jobs", value: `${jobs.filter((row) => row.priority === "Urgent").length} urgent`, tone: "#f87171" },
