...
export default function MaintenanceAnalyticsPage() {
  const contracts = useBusinessRecords("maintenance_contract").records.map(mapMaintenanceContract);
  const schedules = useBusinessRecords("maintenance_schedule").records.map(mapMaintenanceSchedule);
  const jobs = useBusinessRecords("maintenance_job").records.map(mapMaintenanceJob);
  const parts = useBusinessRecords("maintenance_part").records.map(mapMaintenancePart);

  const totalValue = contracts.reduce((sum, row) => sum + row.value, 0);
  ...
  const scheduleAdherence = schedules.length
    ? Math.round((schedules.filter((row) => row.status === "completed").length / schedules.length) * 100)
    : 0;
  const lowStockParts = parts.filter((row) => row.status === "low_stock" || row.quantity <= row.reorderLevel).length;
  ...
  return (
    <div style={{ minHeight: "100vh", padding: "28px 32px", color: "#fff", fontFamily: maintenanceFont }}>
      ...
      <div style={{ display: "grid", gridTemplateColumns: "repeat(4,minmax(0,1fr))", gap: 14, marginBottom: 24 }}>
        {[
          { label: "Contract Value", value: `Rs. ${totalValue.toLocaleString()}`, color: "#34d399" },
          { label: "Job Completion", value: `${completionRate}%`, color: "#60a5fa" },
          { label: "Schedule Adherence", value: `${scheduleAdherence}%`, color: "#22c55e" },
          { label: "Low Stock Parts", value: lowStockParts, color: "#f87171" },
        ].map((card) => (
      ...
      <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 18 }}>
        <div style={{ background: maintenanceBg, border: `1px solid ${maintenanceBorder}`, borderRadius: 20, padding: 22 }}>
          ...
          <div style={{ display: "grid", gap: 12 }}>
            {[
              { label: "Active contracts", value: `${contracts.filter((row) => row.status === "active").length} active`, tone: "#34d399" },
              { label: "Renewal due", value: `${contracts.filter((row) => row.status === "renewal_due").length} at risk`, tone: "#f59e0b" },
              { label: "Paused contracts", value: `${contracts.filter((row) => row.status === "paused").length} paused`, tone: "#94a3b8" },
              { label: "Average contract size", value: `Rs. ${Math.round(totalValue / Math.max(contracts.length, 1)).toLocaleString()}`, tone: "#60a5fa" },
            ].map((row) => (
        ...
        <div style={{ background: maintenanceBg, border: `1px solid ${maintenanceBorder}`, borderRadius: 20, padding: 22 }}>
          ...
          <div style={{ display: "grid", gap: 12 }}>
            {[
              { label: "Open jobs", value: `${jobs.filter((row) => !["Completed", "Cancelled"].includes(String(row.status))).length} open`, tone: "#60a5fa" },
              { label: "Urgent backlog", value: `${jobs.filter((row) => row.priority === "Urgent" && !["Completed", "Cancelled"].includes(String(row.status))).length} urgent`, tone: "#f87171" },
              { label: "Completed visits", value: `${schedules.filter((row) => row.status === "completed").length} done`, tone: "#22c55e" },
