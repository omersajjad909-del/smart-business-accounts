...
export default function BranchesPage() {
  ...
  async function submit(e: React.FormEvent) {
    ...
    setForm({ code: "", name: "", city: "", isActive: true });
    setEditId(null);
    setLoading(false);
    load();
  }
  ...
  return (
    <ResponsiveContainer>
      <PageHeader title="Branches" description="Manage company branches and locations." />
      <Card>
        ...
        {items.map((b) => (
          <Card key={b.id} className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-3">
            <div>
              ...
              <div className="text-xs text-[var(--text-muted)]">{b.city || "No city"} · {b.isActive ? "Active" : "Inactive"}</div>
            </div>
            <div className="flex gap-2">
              <Button
                ...
                onClick={() => {
                  setEditId(b.id);
                  setForm({ code: b.code, name: b.name, city: b.city || "", isActive: b.isActive });
                }}
              >
                Edit
              </Button>
