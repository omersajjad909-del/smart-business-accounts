          ...
          <tbody>
            {materials.map((m) => (
              <tr key={m.id}>
                ...
                <td style={{ padding: isMobile ? "12px 10px" : "14px 16px", borderBottom: "1px solid rgba(var(--ink),.04)", fontSize: 13, color: "rgba(var(--ink),var(--ta-50, .5))" }}>{m.supplier || "—"}</td>
                <td style={{ padding: isMobile ? "12px 10px" : "14px 16px", borderBottom: "1px solid rgba(var(--ink),.04)", fontSize: 13, color: "rgba(var(--ink),var(--ta-50, .5))" }}>{m.site}</td>
            ...
            {!loading && materials.length === 0 && <tr><td colSpan={8} style={{ padding: 40, textAlign: "center", color: "rgba(var(--ink),var(--ta-25, .25))" }}>No materials yet.</td></tr>}
          </tbody>
        </table>
      </div>
          ...
          <div style={{ background: "var(--dk-161b27, #161b27)", border: `1px solid ${constructionBorder}`, borderRadius: 16, padding: 32, width: 520, fontFamily: constructionFont }}>
            ...
            <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 14 }}>
              ...
              <div style={{ gridColumn: "span 2" }}>
                <label style={{ display: "block", fontSize: 12, color: "rgba(var(--ink),var(--ta-45, .45))", marginBottom: 6 }}>Unit Cost (Rs.)</label>
