  ...
  return (
    <div style={{ minHeight: "100vh", padding: isMobile ? "17px 16px" : "28px 32px", color: "var(--ink-solid, #fff)", fontFamily: constructionFont }}>
      <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 24, gap: 16, flexWrap: "wrap" }}>
        ...
        <button onClick={() => setShowModal(true)} style={{ padding: isMobile ? "8px 9px" : "10px 18px", borderRadius: 10, border: "none", background: "#f97316", color: "#fff", fontWeight: 700, cursor: "pointer" }}>Add Expense</button>
      </div>
        ...
        <table style={{ width: "100%", borderCollapse: "collapse" }}>
          <thead>
            <tr>{["Title", "Project", "Site", "Category", "Vendor", "Amount", "Date", "Status", "Action"].map((head) => (
              <th key={head} style={{ textAlign: "left", padding: isMobile ? "8px 8px" : "12px 16px", borderBottom: `1px solid ${constructionBorder}`, fontSize: 12, color: constructionMuted }}>{head}</th>
            ))}</tr>
          </thead>
          <tbody>
            {expenses.map((row) => (
              <tr key={row.id}>
                ...
                <td style={{ padding: isMobile ? "8px 8px" : "14px 16px", borderBottom: "1px solid rgba(var(--ink),.04)", color: "var(--tx-f87171, #f87171)" }}>Rs. {row.amount.toLocaleString()}</td>
            ...
            {!loading && expenses.length === 0 && <tr><td colSpan={9} style={{ padding: 40, textAlign: "center", color: "rgba(var(--ink),var(--ta-25, .25))" }}>No expenses yet.</td></tr>}
          </tbody>
        </table>
      </div>
