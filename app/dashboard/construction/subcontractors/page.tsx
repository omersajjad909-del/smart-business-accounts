          ...
          <tbody>
            {subs.map((sub) => (
              <tr key={sub.id}>
                ...
                <td style={{ padding: isMobile ? "12px 10px" : "14px 16px", borderBottom: "1px solid rgba(var(--ink),.04)" }}>
                  {sub.status === "active" && <button onClick={() => closeContract(sub.id)} style={{ padding: "5px 10px", borderRadius: 6, border: "1px solid rgba(var(--ink),.12)", background: "rgba(var(--ink),.05)", color: "var(--ink-solid, #fff)", fontSize: 11, cursor: "pointer" }}>Close</button>}
                </td>
              </tr>
            ))}
