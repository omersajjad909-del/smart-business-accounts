  ...
  return (
    <div style={{ padding: isMobile ? "15px 14px" : "28px 32px", color: "var(--tx-e2e8f0, #e2e8f0)", fontFamily: "'Outfit','Inter',sans-serif" }}>
      <div style={{ marginBottom: 24 }}>
        <h1 style={{ margin: "0 0 6px", fontSize: 24, fontWeight: 800, color: "var(--ink-solid, white)" }}>Media Analytics</h1>
      ...
      <div style={{ display: "grid", gridTemplateColumns: "1.1fr .9fr", gap: 16 }}>
        <div style={{ background: "rgba(var(--ink),.03)", border: "1px solid rgba(var(--ink),.07)", borderRadius: 16, padding: 20 }}>
          <div style={{ fontSize: 15, fontWeight: 700, color: "var(--ink-solid, white)", marginBottom: 14 }}>Campaign Snapshot</div>
          {campaignStore.loading ? (
            <div style={{ fontSize: 13, color: "rgba(var(--ink),var(--ta-45, .45))" }}>Loading campaigns...</div>
          ) : campaigns.length ? (
            <table style={{ width: "100%", borderCollapse: "collapse" }}>
              ...
              <tbody>
                {campaigns.slice(0, 8).map((item) => (
                  <tr key={item.id}>
                    <td style={{ padding: "14px 12px 14px 0", borderBottom: "1px solid rgba(var(--ink),.04)", fontSize: 13 }}>{item.campaign}</td>
                    <td style={{ padding: "14px 12px 14px 0", borderBottom: "1px solid rgba(var(--ink),.04)", fontSize: 13 }}>{item.client || "-"}</td>
                    <td style={{ padding: "14px 12px 14px 0", borderBottom: "1px solid rgba(var(--ink),.04)", fontSize: 13 }}>{item.channel || "-"}</td>
                    <td style={{ padding: "14px 12px 14px 0", borderBottom: "1px solid rgba(var(--ink),.04)", fontSize: 13 }}>{Number(item.budget || 0).toLocaleString()}</td>
            ...
            <div style={{ fontSize: 13, color: "rgba(var(--ink),var(--ta-45, .45))" }}>No campaigns available yet.</div>
          )}
        </div>

        <div style={{ display: "grid", gap: 16 }}>
          <div style={{ background: "rgba(var(--ink),.03)", border: "1px solid rgba(var(--ink),.07)", borderRadius: 16, padding: 20 }}>
            <div style={{ fontSize: 15, fontWeight: 700, color: "var(--ink-solid, white)", marginBottom: 12 }}>Client Portfolio</div>
            ...
            )) : <div style={{ fontSize: 13, color: "rgba(var(--ink),var(--ta-45, .45))" }}>No clients available yet.</div>}
          </div>

          <div style={{ background: "rgba(var(--ink),.03)", border: "1px solid rgba(var(--ink),.07)", borderRadius: 16, padding: 20 }}>
            <div style={{ fontSize: 15, fontWeight: 700, color: "var(--ink-solid, white)", marginBottom: 12 }}>Planning Readiness</div>
            ...
            )) : <div style={{ fontSize: 13, color: "rgba(var(--ink),var(--ta-45, .45))" }}>No media plans available yet.</div>}
          </div>
        </div>
      </div>
    </div>
