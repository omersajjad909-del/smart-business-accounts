    ...
    <div style={{ padding: "28px", color: "var(--ink-solid, white)", fontFamily: restaurantFont }}>
      ...
      <div style={{ display: "grid", gridTemplateColumns: isMobile ? "repeat(2,1fr)" : "repeat(4,1fr)", gap: 12, marginBottom: 28 }}>
        {(Object.entries(counts) as [TableStatus, number][]).map(([status, count]) => {
          ...
          return <div key={status} style={{ padding: isMobile ? "12px 10px" : "16px 18px", borderRadius: 13, background: meta.bg, border: `1px solid ${meta.color}30` }}><div style={{ fontSize: 22, fontWeight: 800, color: meta.color }}>{count}</div><div style={{ fontSize: 12, color: "rgba(var(--ink),var(--ta-60, .6))", marginTop: 3 }}>{meta.emoji} {meta.label}</div></div>;
        })}
      </div>
      ...
      <div style={{ display: "grid", gridTemplateColumns: isMobile ? "1fr" : selected ? "1fr 340px" : "1fr", gap: 20 }}>
        <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fill,minmax(120px,1fr))", gap: 12 }}>
          ...
          {!loading && tables.length === 0 && <div style={{ background: restaurantBg, border: `1px solid ${restaurantBorder}`, borderRadius: 12, padding: 40, textAlign: "center", color: "rgba(var(--ink),var(--ta-25, .25))", gridColumn: "1/-1" }}>No tables added yet.</div>}
        </div>
          ...
          <div style={{ padding: isMobile ? "12px 10px" : "20px 22px", borderRadius: 16, background: restaurantBg, border: `1px solid ${restaurantBorder}`, height: "fit-content" }}>
            ...
            <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 8 }}>
              {(["available", "occupied", "reserved", "cleaning"] as TableStatus[]).map((status) => {
                const meta = STATUS_META[status];
                return <button key={status} onClick={() => void changeStatus(selectedTable.id, status)} style={{ padding: "9px", borderRadius: 9, border: `1px solid ${selectedTable.status === status ? meta.color : "rgba(var(--ink),.08)"}`, background: selectedTable.status === status ? meta.bg : "transparent", color: selectedTable.status === status ? meta.color : "rgba(var(--ink),var(--ta-40, .4))", fontSize: 12, fontWeight: 700, cursor: "pointer" }}>{meta.emoji} {meta.label}</button>;
              })}
            </div>
          </div>
        )}
