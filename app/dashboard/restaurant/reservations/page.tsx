    ...
    <div style={{ padding: isMobile ? "15px 14px" : "28px 32px", color: "var(--ink-solid, #fff)", fontFamily: restaurantFont, minHeight: "100vh" }}>
      <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 26 }}>
        <div>
          ...
          <p style={{ fontSize: 13, color: "rgba(var(--ink),var(--ta-45, .45))", margin: 0 }}>Track table bookings from booking to arrival.</p>
        </div>
          ...
          <tbody>
            {reservations.map((reservation) => (
              <tr key={reservation.id}>
                <td style={{ padding: "12px 16px", borderBottom: "1px solid rgba(var(--ink),.04)", fontWeight: 700 }}>{reservation.guestName}</td>
                <td style={{ padding: "12px 16px", borderBottom: "1px solid rgba(var(--ink),.04)" }}>{reservation.phone}</td>
            ...
            {!reservationStore.loading && reservations.length === 0 && (
              <tr>
                <td colSpan={7} style={{ padding: 36, textAlign: "center", color: "rgba(var(--ink),var(--ta-25, .25))" }}>No reservations yet.</td>
              </tr>
            )}
          </tbody>
        </table>
          ...
          <div style={{ width: 520, background: "var(--dk-161b27, #161b27)", border: `1px solid ${restaurantBorder}`, borderRadius: 16, padding: 28 }}>
            ...
            <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 14 }}>
              <div style={{ gridColumn: "span 2" }}>
                <label style={{ display: "block", fontSize: 12, color: "rgba(var(--ink),var(--ta-45, .45))", marginBottom: 6 }}>Guest Name</label>
              ...
              <div>
                <label style={{ display: "block", fontSize: 12, color: "rgba(var(--ink),var(--ta-45, .45))", marginBottom: 6 }}>Reservation Date</label>
