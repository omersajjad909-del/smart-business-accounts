...
export default function BulkPaymentsPage() {
  ...
  if (loading) return (
    <div style={{ ...s.page, display: "flex", alignItems: "center", justifyContent: "center", minHeight: 300 }}>
      <div style={{ color: "var(--text-muted)", fontSize: 14 }}>Loading batches…</div>
    </div>
  );
    ...
    <div style={s.page}>
      ...
      <div style={s.header}>
        ...
        <div style={{ display: "flex", gap: 10 }}>
          <button style={{ ...s.btn, background: "var(--panel-bg)", border: "1px solid var(--border)" }} onClick={() => setShowNewBatch(true)}>
            + New Batch
          </button>
          {activeBatch && (
            <>
              <button style={{ ...s.btn, background: "var(--panel-bg)", border: "1px solid var(--border)" }} onClick={() => setShowAdd(true)}>
                + Add Row
              </button>
        ...
        <div style={{ display: "flex", gap: 8, marginBottom: 20, flexWrap: "wrap" as const }}>
          {batches.map(b => (
            <button key={b.id} onClick={() => { setActiveBatchId(b.id); setSelected(new Set()); }}
              style={{ padding: "8px 16px", borderRadius: 10, border: `1px solid ${b.id === activeBatchId ? "rgba(99,102,241,.6)" : "var(--border)"}`, background: b.id === activeBatchId ? "rgba(99,102,241,.15)" : "var(--panel-bg)", color: b.id === activeBatchId ? "#818cf8" : "var(--text-muted)", fontSize: 13, fontWeight: b.id === activeBatchId ? 700 : 400, cursor: "pointer" }}>
              {b.name}
              <span style={{ marginLeft: 8, fontSize: 10, padding: "2px 6px", borderRadius: 6, background: "var(--panel-bg)", color: "var(--text-muted)" }}>{b.status}</span>
            </button>
          ))}
        </div>
      )}
                  ...
                  {activeBatch.rows.map(p => {
                    ...
                    return (
                      <tr key={p.id} style={{ background: selected.has(p.id) ? "rgba(99,102,241,.08)" : "transparent" }}>
                        ...
                        <td style={s.td}>
                          <div style={{ fontWeight: 700, color: "var(--text-primary)" }}>{p.partyName}</div>
                          <div style={{ fontSize: 11, color: "var(--text-muted)", marginTop: 2 }}>{p.partyType}</div>
                        </td>
                        <td style={{ ...s.td, color: "var(--text-muted)", fontSize: 12 }}>{p.reference || "—"}</td>
                        <td style={s.td}>
                          <span style={{ fontSize: 13 }}>{METHOD_ICON[p.method]}</span>{" "}
                          <span style={{ fontSize: 12, color: "var(--text-muted)" }}>{p.method}</span>
                        </td>
                        <td style={{ ...s.td, fontWeight: 700, color: "var(--text-primary)" }}>Rs. {p.amount.toLocaleString()}</td>
                        <td style={s.td}>
      ...
      {showNewBatch && (
        <div style={s.modal} onClick={() => setShowNewBatch(false)}>
          <div style={s.mbox} onClick={e => e.stopPropagation()}>
            <div style={{ fontSize: 16, fontWeight: 800, color: "var(--text-primary)", marginBottom: 20 }}>Create New Batch</div>
            <div style={{ display: "flex", flexDirection: "column", gap: 14 }}>
              ...
              <div style={{ display: "flex", gap: 10, marginTop: 8 }}>
                ...
                <button style={{ ...s.btn, background: "var(--panel-bg)", flex: 1 }} onClick={() => setShowNewBatch(false)}>Cancel</button>
              </div>
            </div>
          </div>
        </div>
      ...
      {showAdd && (
        <div style={s.modal} onClick={() => setShowAdd(false)}>
          <div style={s.mbox} onClick={e => e.stopPropagation()}>
            <div style={{ fontSize: 16, fontWeight: 800, color: "var(--text-primary)", marginBottom: 20 }}>Add Payment Row</div>
            <div style={{ display: "flex", flexDirection: "column", gap: 14 }}>
              <div>
              ...
              <div style={{ display: "flex", gap: 10, marginTop: 8 }}>
                ...
                <button style={{ ...s.btn, background: "var(--panel-bg)", flex: 1 }} onClick={() => setShowAdd(false)}>Cancel</button>
              </div>
            </div>
          </div>
        </div>
