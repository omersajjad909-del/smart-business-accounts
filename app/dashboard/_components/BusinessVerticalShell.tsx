...
export function BusinessVerticalShell({
  ...
  return (
    <div style={{ minHeight: "100vh", padding: "28px 32px", color: "var(--ink-solid, #fff)", fontFamily: shellFont }}>
      ...
      <div style={{ display: "grid", gridTemplateColumns: "1.05fr .95fr", gap: 18 }}>
        <section style={{ background: "rgba(var(--ink),.03)", border: "1px solid rgba(var(--ink),.08)", borderRadius: 16, overflow: "hidden" }}>
          <div style={{ padding: "16px 18px", borderBottom: "1px solid rgba(var(--ink),.08)", fontSize: 15, fontWeight: 800 }}>
        ...
        <section style={{ background: "rgba(var(--ink),.03)", border: "1px solid rgba(var(--ink),.08)", borderRadius: 16, overflow: "hidden" }}>
          ...
          <div style={{ padding: 18, display: "grid", gap: 10 }}>
            {workflow.map((step, index) => (
              <div key={step} style={{ display: "flex", gap: 10, alignItems: "flex-start", padding: "10px 12px", borderRadius: 10, background: "rgba(var(--ink),.03)", border: "1px solid rgba(var(--ink),.05)" }}>
