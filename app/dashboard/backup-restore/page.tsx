...
A safety snapshot is taken first, so this can be rolled back.`)) return;
  ...
  const card: React.CSSProperties = {
    borderRadius: 14, background: "rgba(var(--ink),0.04)",
    border: "1px solid rgba(var(--ink),0.08)", padding: isMobile ? "12px 11px" : "20px 24px",
  };
  const btn = (color: string, disabled?: boolean): React.CSSProperties => ({
    padding: "9px 18px", borderRadius: 9, border: "none",
    background: disabled ? "rgba(var(--ink),0.06)" : color,
    color: disabled ? "rgba(var(--ink),var(--ta-30, 0.3))" : "white",
  ...
  if (!canAccess) return (
    <div style={{ padding: 32, color: "var(--tx-f87171, #f87171)", textAlign: "center" }}>
      ...
      <div style={{ fontSize: 13, color: "rgba(var(--ink),var(--ta-40, 0.4))", marginTop: 6 }}>You do not have permission to access Backup & Restore.</div>
    </div>
  );
  ...
  return (
    <div style={{ padding: 24, maxWidth: 1000, margin: "0 auto", fontFamily: "inherit" }}>
      ...
      <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", marginBottom: 24 }}>
        <div>
          <h1 style={{ fontSize: 22, fontWeight: 800, color: "var(--ink-solid, white)", margin: "0 0 4px" }}>Data Backup & Restore</h1>
          <p style={{ fontSize: 13, color: "rgba(var(--ink),var(--ta-40, 0.4))", margin: 0 }}>
      ...
      <div style={{ ...card, background: "rgba(99,102,241,0.06)", border: "1px solid rgba(99,102,241,0.2)", marginBottom: 20, display: "flex", gap: 12, alignItems: "flex-start" }}>
        ...
        <div style={{ fontSize: 13, color: "rgba(var(--ink),var(--ta-60, 0.6))", lineHeight: 1.6 }}>
          <strong style={{ color: "var(--ink-solid, white)" }}>How it works:</strong> Backups include all your accounts, invoices, vouchers, inventory, HR records and more.
      ...
      <div style={{ ...card, marginBottom: 20, background: "rgba(251,191,36,.05)", border: "1px solid rgba(251,191,36,.2)" }}>
        <div style={{ display: "flex", alignItems: "flex-start", gap: 12, marginBottom: 14 }}>
          ...
          <div>
            ...
            <div style={{ fontSize: 12, color: "rgba(var(--ink),var(--ta-50, .5))", lineHeight: 1.6 }}>
        ...
        <div style={{ display: "flex", alignItems: "center", gap: 12, flexWrap: "wrap" }}>
          <label style={{ display: "flex", alignItems: "center", gap: 10, padding: "10px 16px", borderRadius: 9, border: "1px dashed rgba(251,191,36,.4)", background: "rgba(251,191,36,.06)", cursor: "pointer", fontSize: 13, color: uploadFile ? "var(--tx-fbbf24, #fbbf24)" : "rgba(var(--ink),var(--ta-50, .5))", fontWeight: 600 }}>
            <span>📁</span>
          ...
          {uploadFile && (
            <>
              ...
              <button
                ...
                style={{ padding: "10px 16px", borderRadius: 9, border: "1px solid rgba(var(--ink),.1)", background: "transparent", color: "rgba(var(--ink),var(--ta-40, .4))", fontSize: 13, cursor: "pointer" }}
              >
                Clear
              </button>
            </>
      ...
      <div style={card}>
        <table style={{ width: "100%", borderCollapse: "collapse", fontSize: 13 }}>
          <thead>
            <tr style={{ borderBottom: "1px solid rgba(var(--ink),0.08)" }}>
              {["File Name", "Type", "Size", "Status", "Created", "Actions"].map(h => (
                <th key={h} style={{ padding: "10px 12px", textAlign: "left", fontSize: 11, fontWeight: 700, color: "rgba(var(--ink),var(--ta-35, 0.35))", textTransform: "uppercase", letterSpacing: ".08em" }}>{h}</th>
              ))}
            </tr>
          </thead>
          <tbody>
            {loading ? (
              <tr><td colSpan={6} style={{ padding: 32, textAlign: "center", color: "rgba(var(--ink),var(--ta-30, 0.3))" }}>Loading…</td></tr>
            ) : backups.length === 0 ? (
              <tr><td colSpan={6} style={{ padding: 32, textAlign: "center", color: "rgba(var(--ink),var(--ta-30, 0.3))" }}>No backups yet. Click "+ Create Backup" to get started.</td></tr>
            ) : backups.map((b) => (
              <tr key={b.id} style={{ borderBottom: "1px solid rgba(var(--ink),0.05)" }}>
                <td style={{ padding: "12px", color: "rgba(var(--ink),0.75)", fontFamily: "monospace", fontSize: 12 }}>{b.fileName}</td>
                <td style={{ padding: "12px", color: "rgba(var(--ink),var(--ta-50, 0.5))" }}>{b.backupType}</td>
                <td style={{ padding: "12px", color: "rgba(var(--ink),var(--ta-50, 0.5))" }}>{fmtSize(b.fileSize)}</td>
                ...
                <td style={{ padding: "12px", color: "rgba(var(--ink),var(--ta-40, 0.4))", fontSize: 12 }}>
                  ...
                  {b.status === "COMPLETED" && (
                    <div style={{ display: "flex", gap: 6 }}>
                      ...
                      <button
                        ...
                        style={{ padding: "5px 12px", borderRadius: 7, border: "1px solid rgba(248,113,113,0.3)", background: "rgba(248,113,113,0.08)", color: restoringId === b.id ? "rgba(var(--ink),var(--ta-30, 0.3))" : "var(--tx-f87171, #f87171)", fontSize: 12, cursor: restoringId === b.id ? "not-allowed" : "pointer", fontFamily: "inherit", fontWeight: 600 }}
                      >
