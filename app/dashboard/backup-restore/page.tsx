...
A safety snapshot is taken first, so this can be rolled back.`)) return;
  ...
  const btn = (color: string, disabled?: boolean): React.CSSProperties => ({
    ...
    color: disabled ? "rgba(var(--ink),0.3)" : "white",
    ...
    <div style={{ padding: 24, maxWidth: 1000, margin: "0 auto", fontFamily: "inherit" }}>
      ...
      <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", marginBottom: 24 }}>
        <div>
          ...
          <p style={{ fontSize: 13, color: "rgba(var(--ink),0.4)", margin: 0 }}>
      ...
      <div style={{ ...card, background: "rgba(99,102,241,0.06)", border: "1px solid rgba(99,102,241,0.2)", marginBottom: 20, display: "flex", gap: 12, alignItems: "flex-start" }}>
        ...
        <div style={{ fontSize: 13, color: "rgba(var(--ink),0.6)", lineHeight: 1.6 }}>
          ...
          {uploadFile && (
            <>
              ...
              <button
                ...
                style={{ padding: "10px 16px", borderRadius: 9, border: "1px solid rgba(var(--ink),.1)", background: "transparent", color: "rgba(var(--ink),.4)", fontSize: 13, cursor: "pointer" }}
              >
                Clear
              </button>
            </>
          ...
          <thead>
            <tr style={{ borderBottom: "1px solid rgba(var(--ink),0.08)" }}>
              {["File Name", "Type", "Size", "Status", "Created", "Actions"].map(h => (
                <th key={h} style={{ padding: "10px 12px", textAlign: "left", fontSize: 11, fontWeight: 700, color: "rgba(var(--ink),0.35)", textTransform: "uppercase", letterSpacing: ".08em" }}>{h}</th>
              ))}
            </tr>
          </thead>
          <tbody>
            {loading ? (
              <tr><td colSpan={6} style={{ padding: 32, textAlign: "center", color: "rgba(var(--ink),0.3)" }}>Loading…</td></tr>
            ) : backups.length === 0 ? (
              <tr><td colSpan={6} style={{ padding: 32, textAlign: "center", color: "rgba(var(--ink),0.3)" }}>No backups yet. Click "+ Create Backup" to get started.</td></tr>
            ) : backups.map((b) => (
              <tr key={b.id} style={{ borderBottom: "1px solid rgba(var(--ink),0.05)" }}>
                ...
                <td style={{ padding: "12px", color: "rgba(var(--ink),0.5)" }}>{b.backupType}</td>
                <td style={{ padding: "12px", color: "rgba(var(--ink),0.5)" }}>{fmtSize(b.fileSize)}</td>
                ...
                <td style={{ padding: "12px", color: "rgba(var(--ink),0.4)", fontSize: 12 }}>
