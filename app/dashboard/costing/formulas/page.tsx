...
const input: React.CSSProperties = {
  ...
  color: "var(--ink-solid, white)", fontSize: 13, outline: "none", fontFamily: FONT,
...
const groupBlock: React.CSSProperties = {
  border: "1px solid rgba(var(--ink),.07)", borderRadius: 12,
  padding: "11px 12px 12px", background: "rgba(var(--ink),.015)",
};
...
.fxHead{font-size:9.5px;font-weight:700;letter-spacing:.08em;text-transform:uppercase;
  color:rgba(var(--ink),.32);padding:0 2px 3px}
...
export default function FormulasPage() {
  ...
  if (editing) {
    ...
    return (
      <div className="fxWrap" style={{ fontFamily: FONT, color: "var(--ink-solid, white)" }}>
        <style>{CSS}</style>
        ...
        <div className="fxBar">
          ...
          <select value={d.category} onChange={(e) => patch((x) => { x.category = e.target.value; })}
            style={{
              ...
              paddingRight: 30,
              backgroundImage: "linear-gradient(45deg, transparent 50%, rgba(var(--ink),.45) 50%), linear-gradient(135deg, rgba(var(--ink),.45) 50%, transparent 50%)",
          ...
          <div style={{ display: "flex", gap: 3, padding: 3, borderRadius: 11, background: "rgba(var(--ink),.05)", border: `1px solid ${BORDER}` }}>
              ...
              {d.steps.map((st, i) => {
                ...
                return (
                  <div key={i} style={{
                    ...
                    background: stepErr ? "rgba(248,113,113,.06)" : "rgba(var(--ink),.02)",
  ...
  /* ─────────────────────────── List ─────────────────────────── */
  return (
    <div style={{ fontFamily: FONT, color: "var(--ink-solid, white)", padding: "24px 20px 80px", maxWidth: 1180, margin: "0 auto" }}>
