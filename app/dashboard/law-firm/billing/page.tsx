...
const S = {
  ...
  tab: (a: boolean) => ({ background: a ? '#92400e' : 'rgba(var(--ink),.06)', color: a ? '#fff' : 'rgba(var(--ink),var(--ta-60, .6))', border: 'none', borderRadius: 8, padding: '8px 16px', cursor: 'pointer', fontWeight: 600, fontSize: 13 }),
        ...
        <table style={S.table}>
          ...
          <tbody>
            {!loading && filtered.length === 0 && (
              <tr><td colSpan={9} style={{ padding: 32, textAlign: 'center', color: 'rgba(var(--ink),var(--ta-25, .25))', fontSize: 14 }}>No invoices found.</td></tr>
            )}
            {filtered.map(i => (
              <tr key={i.id}>
                <td style={S.td}><div style={{ fontWeight: 600 }}>{i.invoiceId}</div><div style={{ fontSize: 12, color: 'rgba(var(--ink),var(--ta-40, .4))' }}>{i.caseId}</div></td>
