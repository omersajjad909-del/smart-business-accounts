...
export default function TimeBillingPage() {
  ...
  const handleAdd = async () => {
    ...
    if (!caseId || !caseTitle || !task) {
      toast.error('Case ID, case title, and task are required.');
      return;
    }
    if (hours <= 0 || rate <= 0) {
      toast('Hours and rate must be positive.');
      return;
    }
    if (duplicateEntry) {
      toast('A time entry for today has already been logged.');
      return;
    }
        ...
        <table style={S.table}>
          ...
          <tbody>
            {!loading && filtered.length === 0 && (
              <tr><td colSpan={9} style={{ padding: 32, textAlign: 'center', color: 'rgba(var(--ink),var(--ta-25, .25))', fontSize: 14 }}>No time entries found.</td></tr>
            )}
            {filtered.map(e => (
              <tr key={e.id}>
                <td style={S.td}>{e.date}</td>
                <td style={S.td}>{e.lawyer}</td>
                <td style={S.td}><div style={{ fontWeight: 600 }}>{e.caseTitle}</div><div style={{ fontSize: 11, color: 'rgba(var(--ink),var(--ta-40, .4))' }}>{e.caseId}</div></td>
