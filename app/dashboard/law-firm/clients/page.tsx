        ...
        <table style={S.table}>
          ...
          <tbody>
            {!loading && clients.length === 0 && (
              <tr><td colSpan={8} style={{ padding: 32, textAlign: 'center', color: 'rgba(var(--ink),var(--ta-25, .25))', fontSize: 14 }}>No clients found.</td></tr>
            )}
            {clients.map(c => (
              <tr key={c.id}>
                <td style={S.td}><div style={{ fontWeight: 600 }}>{c.name}</div><div style={{ fontSize: 12, color: 'rgba(var(--ink),var(--ta-40, .4))' }}>{c.clientId} · Joined {c.joined}</div></td>
                <td style={S.td}><span style={S.badge(c.type === 'Company' ? '#818cf8' : '#38bdf8')}>{c.type}</span></td>
                <td style={S.td}><div>{c.phone}</div><div style={{ fontSize: 12, color: 'rgba(var(--ink),var(--ta-40, .4))' }}>{c.email}</div></td>
                <td style={S.td}>{c.city}</td>
                <td style={S.td}><span style={{ color: 'var(--tx-38bdf8, #38bdf8)', fontWeight: 600 }}>{c.activeCases}</span> <span style={{ color: 'rgba(var(--ink),var(--ta-40, .4))' }}>/ {c.totalCases}</span></td>
