  ...
  return (
    <div style={{ minHeight: "100vh", padding: isMobile ? "15px 14px" : "28px 32px", color: "var(--ink-solid, #fff)", fontFamily: pharmacyFont }}>
      ...
      <div style={{ display: "grid", gridTemplateColumns: isMobile ? "1fr" : "repeat(3,minmax(0,1fr))", gap: 12, marginBottom: 18 }}>
        {[
          { label: "Sales Count", value: sales.length, color: "var(--tx-60a5fa, #60a5fa)" },
          { label: "Units Sold", value: sales.reduce((sum, row) => sum + row.quantity, 0), color: "var(--tx-34d399, #34d399)" },
          { label: "Revenue", value: `Rs. ${sales.reduce((sum, row) => sum + row.amount, 0).toLocaleString()}`, color: "var(--tx-f59e0b, #f59e0b)" },
        ].map((card) => (
