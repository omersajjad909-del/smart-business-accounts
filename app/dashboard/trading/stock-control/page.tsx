  ...
  return (
    <div style={{ padding: isMobile ? "15px 14px" : "28px 32px", fontFamily: tradingFont, color: "var(--text-primary)", minHeight: "100vh" }}>
      ...
      <div style={{ display: "grid", gridTemplateColumns: isMobile ? "repeat(2,1fr)" : "repeat(4,minmax(0,1fr))", gap: 12, marginBottom: 24 }}>
        {[
          { label: "Stock Items", value: stock.length, color: "var(--tx-38bdf8, #38bdf8)" },
          { label: "Stock Value", value: formatMoney(stockValue), color: "var(--tx-34d399, #34d399)" },
          { label: "Low Stock Watch", value: lowStock.length, color: "var(--tx-f59e0b, #f59e0b)" },
          { label: "Outward Qty", value: outwardQty, color: "var(--tx-a78bfa, #a78bfa)" },
        ].map((card) => (
            ...
            <tbody>
              {stock.slice(0, 12).map((row) => (
                <tr key={row.itemId}>
                  ...
                  <td style={{ padding: "12px 16px", borderBottom: "1px solid rgba(var(--ink),.04)", color: Number(row.stockQty || 0) <= 5 ? "var(--tx-f59e0b, #f59e0b)" : "var(--tx-38bdf8, #38bdf8)", fontWeight: 700 }}>{row.stockQty}</td>
                  <td style={{ padding: "12px 16px", borderBottom: "1px solid rgba(var(--ink),.04)", color: "var(--tx-34d399, #34d399)", fontWeight: 700 }}>{formatMoney(row.stockValue)}</td>
        ...
        <div style={{ display: "grid", gap: 14 }}>
              ...
              {lowStock.slice(0, 6).map((row) => (
                <div key={row.itemId} style={{ padding: "12px 14px", borderRadius: 12, background: Number(row.stockQty || 0) <= 0 ? "rgba(248,113,113,.08)" : "rgba(245,158,11,.08)", border: Number(row.stockQty || 0) <= 0 ? "1px solid rgba(248,113,113,.18)" : "1px solid rgba(245,158,11,.18)" }}>
                  <div style={{ display: "flex", justifyContent: "space-between", gap: 12 }}>
                    ...
                    <span style={{ color: Number(row.stockQty || 0) <= 0 ? "var(--tx-f87171, #f87171)" : "var(--tx-f59e0b, #f59e0b)", fontWeight: 800 }}>{row.stockQty}</span>
                  </div>
          ...
          <div style={{ background: tradingBg, border: `1px solid ${tradingBorder}`, borderRadius: 16, overflow: "hidden" }}>
            ...
            <div style={{ padding: 16, display: "grid", gap: 10 }}>
              {outward.slice(0, 5).map((row) => (
                <div key={row.id} style={{ padding: "12px 14px", borderRadius: 12, background: "var(--panel-bg)", border: `1px solid ${tradingBorder}` }}>
                  <div style={{ display: "flex", justifyContent: "space-between", gap: 12 }}>
                    ...
                    <span style={{ color: "var(--tx-a78bfa, #a78bfa)", fontWeight: 700 }}>{formatDate(row.date)}</span>
                  </div>
              ...
              {deadStock.length > 0 && <div style={{ fontSize: 12, color: "var(--tx-f87171, #f87171)" }}>{deadStock.length} item(s) are already at zero or negative stock.</div>}
            </div>
          </div>
        </div>
      </div>
        ...
        <div style={{ position: "fixed", inset: 0, background: "rgba(0,0,0,.75)", backdropFilter: "blur(10px)", display: "flex", alignItems: "center", justifyContent: "center", zIndex: 1000, padding: 16 }}>
          <div style={{ background: "var(--dk-13161f, #13161f)", border: "1px solid rgba(167,139,250,.25)", borderRadius: 22, padding: 28, width: 500, maxWidth: "95vw", fontFamily: tradingFont, boxShadow: "0 32px 80px rgba(0,0,0,.6)" }}>
            ...
            <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", marginBottom: 22 }}>
              <div>
                <h3 style={{ margin: 0, fontSize: 18, fontWeight: 800, color: "var(--tx-f1f5f9, #f1f5f9)" }}>
            ...
            {!adjItem ? (
                  ...
                  {stock.map((row) => {
                    ...
                    return (
                      <button key={row.itemId}
                        ...
                        onMouseLeave={e => (e.currentTarget.style.borderColor = "rgba(var(--ink),.08)")}
                      >
                        <div style={{ width: 10, height: 10, borderRadius: "50%", background: dotColor, flexShrink: 0, boxShadow: `0 0 8px ${dotColor}` }} />
                        <div style={{ flex: 1 }}>
                          <div style={{ fontWeight: 700, fontSize: 14, color: "var(--tx-f1f5f9, #f1f5f9)" }}>{row.itemName}</div>
              ...
              <div style={{ display: "grid", gap: 18 }}>
                ...
                <div style={{ display: "flex", alignItems: "center", gap: 14, padding: isMobile ? "12px 10px" : "14px 16px", borderRadius: 12, background: "rgba(167,139,250,.08)", border: "1px solid rgba(167,139,250,.2)" }}>
                  <div style={{ flex: 1 }}>
                    <div style={{ fontWeight: 800, fontSize: 15, color: "var(--tx-f1f5f9, #f1f5f9)" }}>{adjItem.itemName}</div>
                  ...
                  <div style={{ textAlign: "center" }}>
                    ...
                    <div style={{ fontSize: 26, fontWeight: 800, color: "var(--tx-a78bfa, #a78bfa)", lineHeight: 1 }}>{adjItem.stockQty}</div>
                  </div>
                ...
                <div>
                  ...
                  <div style={{ display: "flex", alignItems: "center", gap: 8 }}>
                    <button onClick={() => setAdjQty(q => String(Math.max(0, Number(q || 0) - 1)))}
                      style={{ width: 40, height: 44, borderRadius: 8, border: "1px solid rgba(var(--ink),.1)", background: "rgba(var(--ink),.05)", color: "var(--tx-f1f5f9, #f1f5f9)", fontSize: 20, cursor: "pointer", flexShrink: 0 }}>−</button>
                    <input autoFocus type="number" min={0} value={adjQty} onChange={e => setAdjQty(e.target.value)}
                      style={{ flex: 1, padding: "10px 14px", borderRadius: 10, border: "1.5px solid rgba(167,139,250,.4)", background: "rgba(167,139,250,.08)", color: "var(--tx-f1f5f9, #f1f5f9)", fontSize: 22, fontWeight: 800, textAlign: "center", outline: "none", fontFamily: tradingFont }} />
                ...
                {adjQty !== "" && (() => {
                  ...
                  return (
                    <div style={{ padding: "12px 16px", borderRadius: 12, background: isNone ? "rgba(var(--ink),.04)" : isUp ? "rgba(52,211,153,.08)" : "rgba(248,113,113,.08)", border: `1px solid ${isNone ? "rgba(var(--ink),.08)" : isUp ? "rgba(52,211,153,.25)" : "rgba(248,113,113,.25)"}`, display: "flex", alignItems: "center", justifyContent: "space-between" }}>
                      ...
                      {!isNone && (
                        <span style={{ fontSize: 20, fontWeight: 800, color: isUp ? "var(--tx-34d399, #34d399)" : "var(--tx-f87171, #f87171)" }}>
                          {isUp ? "+" : ""}{diff}
                        </span>
                      )}
                    </div>
