...
const S = {
  ...
  card: { background: 'rgba(var(--ink),.03)', border: '1px solid rgba(var(--ink),.07)', borderRadius: 12, padding: isMobile ? "12px 11px" : "20px 24px" },
  statLabel: { fontSize: 13, color: 'rgba(var(--ink),var(--ta-50, .5))', marginBottom: 6 },
  statVal: { fontSize: 28, fontWeight: 700 },
  tabs: { display: 'flex', gap: 8, marginBottom: 24 },
  tab: (a: boolean) => ({ background: a ? '#16a34a' : 'rgba(var(--ink),.06)', color: a ? '#fff' : 'rgba(var(--ink),var(--ta-60, .6))', border: 'none', borderRadius: 8, padding: '8px 16px', cursor: 'pointer', fontWeight: 600, fontSize: 13 }),
  table: { width: '100%', borderCollapse: 'collapse' as const },
  th: { textAlign: 'left' as const, padding: '12px 16px', fontSize: 12, color: 'rgba(var(--ink),var(--ta-50, .5))', borderBottom: '1px solid rgba(var(--ink),.07)', fontWeight: 600 },
  ...
  actBtn: { flex: 1, background: 'rgba(var(--ink),.06)', border: '1px solid rgba(var(--ink),.1)', color: 'var(--ink-solid, #fff)', borderRadius: 8, padding: '10px', fontSize: 14, cursor: 'pointer' },
};
        ...
        <table style={S.table}>
          ...
          <tbody>
            {!loading && filtered.length === 0 && (
              <tr><td colSpan={9} style={{ padding: 32, textAlign: 'center', color: 'rgba(var(--ink),var(--ta-25, .25))', fontSize: 14 }}>No members found.</td></tr>
            )}
            {filtered.map(m => (
              <tr key={m.id}>
                <td style={S.td}><div style={{ fontWeight: 600 }}>{m.name}</div><div style={{ fontSize: 12, color: 'rgba(var(--ink),var(--ta-40, .4))' }}>{m.memberId}</div></td>
