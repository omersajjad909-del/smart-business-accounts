      ...
      <div style={{ display: "grid", gridTemplateColumns: isMobile ? "1fr" : "340px 1fr", gap: 18 }}>
        ...
        <div style={{ background: ispBg, border: `1px solid ${ispBorder}`, borderRadius: 18, overflow: "hidden" }}>
          ...
          <div style={{ display: "grid", gap: 12, padding: 18 }}>
            {!loading && bills.length === 0 && <div style={{ color: "rgba(var(--ink),var(--ta-28, .28))" }}>No bills generated yet.</div>}
            {bills.map((row) => (
              <div key={row.id} style={{ border: "1px solid rgba(var(--ink),.07)", background: "rgba(var(--ink),.03)", borderRadius: 14, padding: 16 }}>
                <div style={{ display: "flex", justifyContent: "space-between", gap: 12 }}>
                  <div>
                    ...
                    <div style={{ fontSize: 12, color: "var(--tx-93c5fd, #93c5fd)", marginTop: 8 }}>Due {row.dueDate}</div>
                  </div>
                  <div style={{ textAlign: "right" }}>
                    <div style={{ fontSize: 20, fontWeight: 900, color: "var(--tx-34d399, #34d399)" }}>Rs. {row.amount.toLocaleString()}</div>
