...
const MODAL_BG = "var(--dk-12161f, #12161f)";
...
const lbl: React.CSSProperties = { display: "block", fontSize: 11, color: "rgba(var(--ink),var(--ta-40, 0.4))", marginBottom: 5, fontWeight: 500, textTransform: "uppercase", letterSpacing: "0.04em" };
    ...
    <div style={{ minHeight: "100vh", background: "var(--app-bg,var(--dk-0d1117, #0d1117))", color: "var(--text-primary,#fff)", fontFamily: FF, padding: isMobile ? "15px 14px" : "28px 32px" }}>
      ...
      <div style={{ display: "flex", justifyContent: "space-between", alignItems: "flex-start", marginBottom: 28 }}>
        <div>
          ...
          <p style={{ margin: "5px 0 0", color: "var(--text-muted,rgba(var(--ink),var(--ta-45, 0.45)))", fontSize: 13 }}>
      ...
      <div style={{ display: "grid", gridTemplateColumns: "repeat(5,1fr)", gap: 14, marginBottom: 26 }}>
        {[
          ...
          <div key={k.label} style={{ ...card, textAlign: "center" }}>
            ...
            <div style={{ fontSize: 11, color: "var(--text-muted,rgba(var(--ink),var(--ta-45, 0.45)))", marginTop: 5, fontWeight: 500 }}>{k.label}</div>
          </div>
        ))}
      </div>
          ...
          {TABS.map(t => (
            <button key={t} onClick={() => setTab(t)}
              style={{
                ...
                color: tab === t ? "#a5b4fc" : "rgba(var(--ink),var(--ta-50, 0.5))",
              }}>
      ...
      <div style={{ display: "grid", gridTemplateColumns: isMobile ? "1fr" : selectedItem ? "1fr 380px" : "1fr", gap: 18 }}>
        ...
        <div style={{ ...card, padding: 0, overflow: "hidden" }}>
          {loading
            ? <div style={{ padding: 48, textAlign: "center", color: "rgba(var(--ink),var(--ta-30, 0.3))", fontSize: 14 }}>Loading…</div>
            : (
                  ...
                  <thead>
                    <tr style={{ borderBottom: `1px solid ${BD}` }}>
                      {["LC/TT No","Type","Direction","Bank","Beneficiary / Applicant","Amount","Ccy","Issue Date","Expiry","Terms","Status",""].map(h => (
                        <th key={h} style={{ padding: "13px 14px", textAlign: "left", color: "rgba(var(--ink),var(--ta-38, 0.38))", fontWeight: 500, fontSize: 11, whiteSpace: "nowrap", textTransform: "uppercase", letterSpacing: "0.04em" }}>{h}</th>
                      ))}
                    </tr>
                  </thead>
                  <tbody>
                    {filtered.length === 0 && (
                      <tr><td colSpan={12} style={{ padding: 48, textAlign: "center", color: "rgba(var(--ink),var(--ta-20, 0.2))", fontSize: 14 }}>No records found.</td></tr>
                    )}
                    {filtered.map(item => {
                      ...
                      return (
                        <tr key={item.id}
                          ...
                          <td style={{ padding: "12px 14px", color: "rgba(var(--ink),var(--ta-70, 0.7))", maxWidth: 160, whiteSpace: "nowrap", overflow: "hidden", textOverflow: "ellipsis" }}>{item.issuingBank}</td>
                          <td style={{ padding: "12px 14px", maxWidth: 180 }}>
                            <div style={{ fontWeight: 500, overflow: "hidden", textOverflow: "ellipsis", whiteSpace: "nowrap" }}>{item.beneficiaryName}</div>
                            {item.applicantName && <div style={{ fontSize: 11, color: "rgba(var(--ink),var(--ta-40, 0.4))", overflow: "hidden", textOverflow: "ellipsis", whiteSpace: "nowrap" }}>{item.applicantName}</div>}
                          </td>
                          <td style={{ padding: "12px 14px", fontWeight: 700, whiteSpace: "nowrap" }}>{fmt(item.amount)}</td>
                          <td style={{ padding: "12px 14px", color: "rgba(var(--ink),var(--ta-50, 0.5))", fontSize: 12 }}>{item.currency}</td>
                          <td style={{ padding: "12px 14px", color: "rgba(var(--ink),var(--ta-50, 0.5))", whiteSpace: "nowrap", fontSize: 12 }}>{item.issueDate}</td>
                          <td style={{ padding: "12px 14px", whiteSpace: "nowrap", fontSize: 12, color: expRed ? "var(--tx-ef4444, #ef4444)" : "rgba(var(--ink),var(--ta-50, 0.5))", fontWeight: expRed ? 700 : 400 }}>
                            {item.expiryDate || "—"}
                            {expRed && <span style={{ marginLeft: 4, fontSize: 10 }}>({expDays}d)</span>}
                          </td>
                          <td style={{ padding: "12px 14px", color: "rgba(var(--ink),var(--ta-50, 0.5))", fontSize: 12, whiteSpace: "nowrap" }}>{terms}</td>
                          <td style={{ padding: "12px 14px" }}>
        ...
        {selectedItem && (
          <div style={{ ...card, position: "relative", alignSelf: "start" }}>
            <button onClick={() => setDetail(null)} style={{ position: "absolute", top: 14, right: 14, background: "none", border: "none", color: "rgba(var(--ink),var(--ta-40, 0.4))", fontSize: 18, cursor: "pointer" }}>✕</button>
            ...
            <p style={{ margin: "0 0 18px", fontSize: 13, color: "rgba(var(--ink),var(--ta-40, 0.4))" }}>{selectedItem.issuingBank}</p>
            ...
            {[
              ...
              <div key={k} style={{ display: "flex", justifyContent: "space-between", padding: "9px 0", borderBottom: `1px solid ${BD}` }}>
                <span style={{ color: "rgba(var(--ink),var(--ta-40, 0.4))", fontSize: 12 }}>{k}</span>
            ...
            {selectedItem.docsRequired.length > 0 && (
              <div style={{ marginTop: 14 }}>
                <div style={{ fontSize: 11, color: "rgba(var(--ink),var(--ta-38, 0.38))", fontWeight: 600, textTransform: "uppercase", letterSpacing: "0.04em", marginBottom: 8 }}>Documents Required</div>
                {selectedItem.docsRequired.map(d => (
                  <div key={d} style={{ fontSize: 12, padding: "4px 0", color: "rgba(var(--ink),var(--ta-65, 0.65))" }}>✓ {d}</div>
                ))}
              </div>
            )}
            ...
            {selectedItem.goodsDesc && (
              <div style={{ marginTop: 14 }}>
                <div style={{ fontSize: 11, color: "rgba(var(--ink),var(--ta-38, 0.38))", fontWeight: 600, textTransform: "uppercase", letterSpacing: "0.04em", marginBottom: 5 }}>Goods Description</div>
                <div style={{ fontSize: 12, color: "rgba(var(--ink),var(--ta-65, 0.65))", lineHeight: 1.6 }}>{selectedItem.goodsDesc}</div>
              </div>
            )}

            {selectedItem.notes && (
              <div style={{ marginTop: 14, padding: "10px 12px", background: "rgba(var(--ink),0.03)", borderRadius: 8, fontSize: 12, color: "rgba(var(--ink),var(--ta-55, 0.55))", lineHeight: 1.6 }}>
                {selectedItem.notes}
              </div>
            )}
        ...
        <div style={{ position: "fixed", inset: 0, background: "rgba(0,0,0,0.75)", display: "flex", alignItems: "center", justifyContent: "center", zIndex: 100 }}
          ...
          <div style={{ background: MODAL_BG, border: `1px solid ${BD}`, borderRadius: 16, padding: 32, width: 680, maxHeight: "90vh", overflowY: "auto", fontFamily: FF }}>
            ...
            <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 24 }}>
              ...
              <button onClick={() => setShowModal(false)} style={{ background: "none", border: "none", color: "rgba(var(--ink),var(--ta-40, 0.4))", fontSize: 20, cursor: "pointer" }}>✕</button>
            </div>
                  ...
                  {(["LC", "TT"] as const).map(t => (
                    <button key={t} onClick={() => { setF("type", t); if (!form.id) setF("refNo", genRef(t)); }}
                      style={{ flex: 1, padding: "9px 0", borderRadius: 8, fontFamily: FF, fontSize: 13, fontWeight: 700, cursor: "pointer", transition: "all 0.15s",
                        ...
                        color: form.type === t ? (t === "LC" ? "#a78bfa" : "#38bdf8") : "rgba(var(--ink),var(--ta-45, 0.45))" }}>
                      {t}
                    </button>
                  ))}
                </div>
                  ...
                  {(["Import", "Export"] as const).map(d => (
                    <button key={d} onClick={() => setF("direction", d)}
                      style={{ flex: 1, padding: "9px 0", borderRadius: 8, fontFamily: FF, fontSize: 13, fontWeight: 700, cursor: "pointer", transition: "all 0.15s",
                        ...
                        color: form.direction === d ? (d === "Import" ? "#fb923c" : "#22c55e") : "rgba(var(--ink),var(--ta-45, 0.45))" }}>
                      {d}
                    </button>
                  ))}
                </div>
                  ...
                  return (
                    <button key={doc} onClick={() => toggleDoc(doc)}
                      style={{ padding: "6px 12px", borderRadius: 7, fontSize: 12, fontWeight: 600, cursor: "pointer", fontFamily: FF, transition: "all 0.12s",
                        ...
                        color: checked ? "#a5b4fc" : "rgba(var(--ink),var(--ta-40, 0.4))" }}>
                      {checked ? "✓ " : ""}{doc}
                    </button>
                  );
                })}
            ...
            <div style={{ display: "flex", gap: 12, marginTop: 24 }}>
              ...
              <button onClick={() => setShowModal(false)}
                style={{ padding: "11px 24px", background: "transparent", border: `1px solid ${BD}`, borderRadius: 9, color: "rgba(var(--ink),var(--ta-50, 0.5))", fontFamily: FF, fontSize: 14, cursor: "pointer" }}>
                Cancel
              </button>
            </div>
          </div>
