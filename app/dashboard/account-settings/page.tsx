...
export default function AccountSettingsPage() {
  ...
  return (
    <div style={{ fontFamily: ff, color: "var(--ink-solid, white)", paddingBottom: 40 }}>
      <ImageAdjusterModal
            ...
            <div style={{ flex: 1 }}>
              ...
              <div style={{ display: "flex", gap: 10, flexWrap: "wrap", marginTop: 12 }}>
                ...
                <button type="button" disabled={!avatar || uploadingPhoto} onClick={removeAvatar}
                  style={{ padding: "10px 14px", borderRadius: 12, border: "1px solid rgba(var(--ink),.12)", background: "transparent", color: "rgba(var(--ink),.8)", fontSize: 13, fontWeight: 700, cursor: !avatar || uploadingPhoto ? "not-allowed" : "pointer", opacity: !avatar || uploadingPhoto ? 0.55 : 1 }}>
                  Remove
                </button>
              </div>
            </div>
        ...
        <section style={{ display: "grid", gap: 18, alignContent: "start" }}>
          ...
          <div style={cardStyle}>
            ...
            <form onSubmit={handlePasswordSubmit}>
              {/* Same autocomplete hints as the admin settings form: without
                  new-password here, the browser never offers to generate a
                  strong one, and customers pick their own weak ones. */}
              <div style={{ display: "grid", gap: 14 }}>
                {/* Which account the new password belongs to — hidden, but the
                    password manager needs it to update the right saved entry. */}
                <input
                  type="text"
                  name="username"
