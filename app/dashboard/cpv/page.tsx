...
export default function CPVPage() {
  ...
  async function loadAll() {
    ...
    if (Array.isArray(aData)) setAccounts(aData.filter((a: Account) => a.partyType !== "CUSTOMER"));
    if (Array.isArray(bData)) setBankAccounts(bData.map((b: any) => ({ id: b.id, name: b.accountName || `${b.bankName} - ${b.accountNo}` })));
  }

  async function saveCPV() {
    if (!accountId) { toast.error("Account select karein"); return; }
    const n = Number(amount);
    if (!n || n <= 0) { toast.error("Valid amount enter karein"); return; }
    setSaving(true);
    try {
      const method = editing ? "PUT" : "POST";
      const body: any = { accountId, bankAccountId: bankId || null, paymentMode: bankId ? "BANK" : "CASH", amount, date, narration };
      if (editing) body.id = editing.id;
      const res  = await fetch("/api/cpv", { method, headers: h(true), body: JSON.stringify(body) });
      const data = await res.json();
      if (!res.ok) { toast.error(data?.error || "Save failed"); return; }
      setSaved(data);
      toast.success(editing ? "CPV updated!" : "CPV saved!");
      resetForm();
      setEditing(null);
      await loadAll();
  ...
  async function deleteVoucher(id: string) {
    if (!await confirmToast("Delete this CPV?")) return;
    const res = await fetch(`/api/cpv?id=${id}`, { method: "DELETE", headers: h() });
    ...
    <>
      ...
      <div style={{ padding: "24px 28px", fontFamily: ff, color: "var(--text-primary)", maxWidth: 1000 }}>
        ...
        <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 24 }}>
          <div>
            <h1 style={{ margin: 0, fontSize: 24, fontWeight: 700, color: accent }}>Cash Payment Voucher (CPV)</h1>
            <p style={{ margin: "4px 0 0", fontSize: 13, color: "var(--text-muted)" }}>Record outgoing payments to suppliers, banks, or expenses</p>
          </div>
            ...
            <table style={{ width: "100%", borderCollapse: "collapse" }}>
              <thead>
                <tr style={{ borderBottom: "1px solid var(--border)" }}>
                  {["Voucher No","Date","Paid To","Amount","Mode","Narration","Actions"].map(h => (
              ...
              <tbody>
                {vouchers.length === 0 ? (
                  <tr><td colSpan={7} style={{ padding: 32, textAlign: "center", color: "var(--text-muted)", fontSize: 14 }}>No CPVs found</td></tr>
                ) : vouchers.map((v, idx) => (
        ...
        {!saved && (
          <div style={panel}>
            <div style={{ fontSize: 16, fontWeight: 700, marginBottom: 20, color: accent }}>{editing ? "Edit CPV" : "New CPV"}</div>
            <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 16, marginBottom: 16 }}>
              <div>
                <label style={lbl}>Paid To (Supplier / Expense / Bank) *</label>
            ...
            <div style={{ marginBottom: 20 }}>
              ...
              <textarea style={{ ...inp, height: 72, resize: "vertical" }} value={narration} onChange={e => setNarration(e.target.value)} placeholder="Payment description…" />
            </div>
            <div style={{ display: "flex", gap: 10 }}>
              <button onClick={saveCPV} disabled={saving} style={{ background: accent, color: "#fff", border: "none", borderRadius: 8, padding: "10px 24px", fontFamily: ff, fontSize: 14, fontWeight: 700, cursor: saving ? "not-allowed" : "pointer", opacity: saving ? 0.7 : 1 }}>
        ...
        {saved && (
          <div style={{ display: "flex", flexDirection: "column", gap: 16 }}>
            <div style={{ ...panel, display: "flex", gap: 10 }}>
              ...
              <button onClick={() => { setSaved(null); resetForm(); }} style={{ background: "transparent", border: "1px solid var(--border)", borderRadius: 8, padding: "9px 20px", fontFamily: ff, fontSize: 14, color: "var(--text-muted)", cursor: "pointer" }}>New CPV</button>
            </div>
            {/* Screen preview */}
            <div style={{ ...panel, background: "#fff", color: "#111", padding: 40 }}>
              <div style={{ display: "flex", justifyContent: "space-between", alignItems: "flex-start", borderBottom: "3px solid #111", paddingBottom: 16, marginBottom: 28 }}>
                <div>
                  <div style={{ fontSize: 28, fontWeight: 900, letterSpacing: -0.5 }}>{company?.name || "COMPANY NAME"}</div>
                ...
                <div style={{ textAlign: "right" }}>
                  <div style={{ fontSize: 20, fontWeight: 900, textTransform: "uppercase", textDecoration: "underline", marginBottom: 6 }}>Payment Voucher</div>
              ...
              <div style={{ display: "flex", flexDirection: "column", gap: 20, marginBottom: 32 }}>
                {[["Paid To", selName || saved.accountName], ["Amount", `${company?.baseCurrency || "PKR"} ${fmt(Number(amount))}/-`], ["Description", narration || "Official transaction recorded."]].map(([lbl, val]) => (
                  <div key={lbl} style={{ display: "flex", gap: 16, borderBottom: "1px solid #e0e0e0", paddingBottom: 10, alignItems: "flex-end" }}>
                    <span style={{ fontSize: 11, fontWeight: 900, textTransform: "uppercase", width: 130, flexShrink: 0 }}>{lbl}:</span>
                    <span style={{ fontSize: lbl === "Amount" ? 20 : 16, fontWeight: 700, flex: 1, borderBottom: "2px solid #111", paddingBottom: 2, paddingLeft: 4 }}>{val}</span>
                  </div>
                ))}
              </div>
              <div style={{ display: "flex", justifyContent: "space-between", alignItems: "flex-end", marginTop: 40 }}>
                <div style={{ background: "#f5f5f5", border: "3px solid #111", padding: "12px 20px", minWidth: 180 }}>
                  <div style={{ fontSize: 10, fontWeight: 900, textTransform: "uppercase", borderBottom: "1px solid #111", marginBottom: 6 }}>Net Amount Paid</div>
                  <div style={{ fontSize: 26, fontWeight: 900 }}>{fmt(Number(amount))}</div>
                </div>
                <div style={{ display: "flex", gap: 48 }}>
                  {["Receiver Sign","Accountant"].map(l => (
      ...
      {saved && (
        <div className="print-area" style={{ fontFamily: "'Outfit','Arial',sans-serif", color: "#000", background: "#fff", padding: "8mm 10mm", fontSize: 12 }}>
          <div style={{ display: "flex", justifyContent: "space-between", borderBottom: "3px solid #000", paddingBottom: 12, marginBottom: 24 }}>
            ...
            <div style={{ textAlign: "right" }}>
              <div style={{ fontSize: 17, fontWeight: 900, textDecoration: "underline" }}>PAYMENT VOUCHER</div>
              <div style={{ fontWeight: 700 }}>No: {saved.voucherNo}</div>
              <div style={{ fontWeight: 700 }}>Date: {date}</div>
            </div>
          </div>
          <div style={{ display: "flex", flexDirection: "column", gap: 14, marginBottom: 28 }}>
            {[["Paid To", selName || saved.accountName], [`Amount (${company?.baseCurrency||"PKR"})`, `${fmt(Number(amount))}/-`], ["Description", narration || "Official transaction."]].map(([l,v]) => (
          ...
          <div style={{ display: "flex", justifyContent: "space-between", marginTop: 36 }}>
            <div style={{ background: "#f0f0f0", border: "2px solid #000", padding: "10px 16px" }}>
              <div style={{ fontSize: 9, fontWeight: 900, textTransform: "uppercase", borderBottom: "1px solid #000", marginBottom: 4 }}>Net Amount Paid</div>
