...
export default function SupplierStatementPage() {
  ...
  useEffect(() => {
    ...
    }).then(r => r.json()).then(d => setSuppliers(Array.isArray(d) ? d : Array.isArray(d?.accounts) ? d.accounts : [])).catch(() => {});
  }, []);

  const fetchStatement = useCallback((s: Supplier, from: string, to: string) => {
    setLoading(true);
    const user = getCurrentUser();
    fetch(`/api/reports/supplier-statement?supplierId=${s.id}&from=${from}&to=${to}`, {
      credentials: "include",
      headers: { "x-user-id": user?.id || "", "x-user-role": user?.role || "", "x-company-id": user?.companyId || "" },
            ...
            <div style={{ marginBottom: 28, position: "relative" }}>
              ...
              {dropOpen && !selected && (
                <div style={{
                  ...
                  {filtered.length === 0 ? (
                    <div style={{ padding: 14, textAlign: "center", color: "rgba(var(--ink),var(--ta-25, .25))", fontSize: 12 }}>No suppliers found</div>
                  ) : filtered.map(s => (
                    ...
                    >
                      <div>{s.name}</div>
                      {s.phone && <div style={{ fontSize: 11, color: "rgba(var(--ink),var(--ta-30, .3))" }}>{s.phone}</div>}
                    </div>
                  ))}
                </div>
              )}
        ...
        <div style={{ maxWidth: 1100, margin: "0 auto" }}>
          <div style={{ marginBottom: 20, display: "flex", alignItems: "center", justifyContent: "space-between" }}>
            <div style={{ display: "flex", alignItems: "center", gap: 12 }}>
              <div style={{ width: 4, height: 28, borderRadius: 2, background: "linear-gradient(180deg,#a78bfa,#8b5cf6)" }}/>
              <h1 style={{ fontSize: 22, fontWeight: 800, letterSpacing: "-.4px", margin: 0 }}>Supplier Statement</h1>
            </div>
          ...
          <div id="stmt-printable" style={{
            ...
            <div style={{
              ...
              <div>
                <div style={{ fontSize: 10, fontWeight: 700, color: "rgba(var(--ink),var(--ta-30, .3))", letterSpacing: ".1em", textTransform: "uppercase", marginBottom: 6 }}>Supplier Statement</div>
                <div style={{ fontSize: 22, fontWeight: 900, color: "var(--ink-solid, white)", letterSpacing: "-.4px" }}>{selected?.name}</div>
                {selected?.phone && <div style={{ fontSize: 12, color: "rgba(var(--ink),var(--ta-35, .35))", marginTop: 4 }}>{selected.phone}</div>}
              </div>
