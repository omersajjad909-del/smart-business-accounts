...
/* ── KPI Card ─────────────────────────────────────────────── */
function KpiCard({ label, value, icon, color, bg, border, sub }: {
  ...
  return (
    <div style={{
      ...
      <div style={{ fontSize: 24, fontWeight: 900, color: color, letterSpacing: "-.5px", lineHeight: 1 }}>{value}</div>
...
function AIWidget({ companyId, role, userId }: { companyId: string; role: string; userId: string }) {
  ...
  useEffect(() => {
    ...
    Promise.allSettled([
      ...
      if (!ctx) { setWidgetFailed(true); setWidgetLoading(false); return; }
  ...
  async function quickChat() {
    ...
    const msg = chat.trim();
    setChat(""); setChatLoading(true); setChatReply("");
    try {
  ...
  if (widgetLoading && !data) return (
    <div style={{ borderRadius: 14, padding: "16px 20px", background: "rgba(99,102,241,.05)", border: "1px solid rgba(99,102,241,.15)", marginBottom: 18, display: "flex", alignItems: "center", gap: 12 }}>
  ...
  if (widgetFailed || !data) return (
    <div style={{ borderRadius: 14, padding: "16px 20px", background: "var(--panel-bg)", border: "1px solid var(--border)", marginBottom: 18, display: "flex", alignItems: "center", justifyContent: "space-between", gap: 14, flexWrap: "wrap" }}>
      <div>
  ...
  return (
    <div style={{ marginBottom: 18 }}>
      <div style={{ borderRadius: 16, border: "1px solid rgba(99,102,241,.2)", background: "var(--panel-bg)", overflow: "hidden" }}>

        {/* Top row — metric strip */}
        <div style={{ display: "grid", gridTemplateColumns: "auto 1fr 1fr 1fr 1fr auto", gap: 0, borderBottom: "1px solid var(--border)" }}>

          {/* AI Label */}
          <div style={{ padding: "14px 18px", borderRight: "1px solid var(--border)", display: "flex", alignItems: "center", gap: 8, background: "rgba(99,102,241,.04)" }}>
            <div style={{ width: 30, height: 30, borderRadius: 8, background: "linear-gradient(135deg,#6366f1,#4f46e5)", display: "flex", alignItems: "center", justifyContent: "center", fontSize: 14, flexShrink: 0 }}>🤖</div>
            <div>
              <div style={{ fontSize: 10, fontWeight: 800, color: "#818cf8", textTransform: "uppercase", letterSpacing: ".06em" }}>AI</div>
              <div style={{ fontSize: 10, color: "var(--text-muted)", whiteSpace: "nowrap" }}>Intelligence</div>
            </div>
          </div>

          {/* Health Score */}
          <div style={{ padding: "14px 18px", borderRight: "1px solid var(--border)", display: "flex", alignItems: "center", gap: 10 }}>
            <div style={{ position: "relative", width: 40, height: 40, flexShrink: 0 }}>
              <svg width="40" height="40" viewBox="0 0 40 40">
                <circle cx="20" cy="20" r="15" fill="none" stroke="var(--border)" strokeWidth="3.5" />
                <circle cx="20" cy="20" r="15" fill="none" stroke={scoreColor} strokeWidth="3.5"
                  strokeDasharray={`${(data.healthScore / 100) * 94} 94`}
                  strokeLinecap="round" transform="rotate(-90 20 20)" />
              </svg>
              <div style={{ position: "absolute", inset: 0, display: "flex", alignItems: "center", justifyContent: "center", fontSize: 10, fontWeight: 900, color: scoreColor }}>{data.healthScore}</div>
            </div>
            <div>
              <div style={{ fontSize: 10, color: "var(--text-muted)", fontWeight: 700, textTransform: "uppercase", letterSpacing: ".06em" }}>Health</div>
              <div style={{ fontSize: 13, fontWeight: 800, color: scoreColor }}>{data.healthScore >= 75 ? "Good" : data.healthScore >= 55 ? "Fair" : "Poor"}</div>
            </div>
          </div>

          {/* Revenue Trend */}
          <div style={{ padding: "14px 18px", borderRight: "1px solid var(--border)" }}>
            <div style={{ fontSize: 10, color: "var(--text-muted)", fontWeight: 700, textTransform: "uppercase", letterSpacing: ".06em", marginBottom: 3 }}>Revenue</div>
            <div style={{ fontSize: 15, fontWeight: 800, color: data.revenueChange >= 0 ? "#10b981" : "#ef4444" }}>
              {data.revenueChange >= 0 ? "▲" : "▼"} {sign(data.revenueChange)}
            </div>
            <div style={{ fontSize: 10, color: "var(--text-muted)" }}>vs last month</div>
          </div>

          {/* Expense Trend */}
          <div style={{ padding: "14px 18px", borderRight: "1px solid var(--border)" }}>
            <div style={{ fontSize: 10, color: "var(--text-muted)", fontWeight: 700, textTransform: "uppercase", letterSpacing: ".06em", marginBottom: 3 }}>Expenses</div>
            <div style={{ fontSize: 15, fontWeight: 800, color: data.expenseChange <= 0 ? "#10b981" : data.expenseChange > 20 ? "#ef4444" : "#f59e0b" }}>
          ...
          {/* Cash Risk */}
          <div style={{ padding: "14px 18px", borderRight: "1px solid var(--border)" }}>
            <div style={{ fontSize: 10, color: "var(--text-muted)", fontWeight: 700, textTransform: "uppercase", letterSpacing: ".06em", marginBottom: 3 }}>Cash Risk</div>
            <div style={{ fontSize: 15, fontWeight: 800, color: riskColor, textTransform: "capitalize" }}>
              {data.cashRisk === "high" ? "🔴" : data.cashRisk === "medium" ? "🟡" : "🟢"} {data.cashRisk}
            </div>
            <div style={{ fontSize: 10, color: "var(--text-muted)" }}>30-day outlook</div>
          </div>
        ...
        {/* Bottom row — alerts + quick chat toggle */}
        <div style={{ display: "grid", gridTemplateColumns: "1fr auto", alignItems: "center" }}>
          ...
          <div style={{ padding: "10px 18px", borderLeft: "1px solid var(--border)" }}>
        ...
        {chatOpen && (
          <div style={{ borderTop: "1px solid var(--border)", padding: "12px 18px" }}>
            ...
            {chatReply && (
              <div style={{ padding: "10px 14px", borderRadius: 10, background: "var(--app-bg)", border: "1px solid var(--border)", fontSize: 12, color: "var(--text-primary)", lineHeight: 1.7, maxHeight: 120, overflowY: "auto" }}>
...
export default function DashboardContent() {
  ...
  useEffect(() => {
    ...
    if (preferredDemoBusiness && preferredDemoBusiness !== businessType) setBusinessType(preferredDemoBusiness);
    ...
    <div style={{ minHeight: "100vh", background: "transparent", padding: 0, fontFamily: "inherit" }}>
      <style>{`
        ...
        @media(max-width:980px){
          .db-alert-grid,.db-chart-grid{ grid-template-columns:1fr !important; }
        }
        @media(max-width:720px){
          .db-header{ flex-direction:column !important; align-items:flex-start !important; }
          .db-biz-kpis{ flex-wrap:wrap !important; }
        }
      `}</style>
      ...
      {/* ── Page header ── */}
      <div className="db-header" style={{ marginBottom: 22, display: "flex", alignItems: "center", justifyContent: "space-between", gap: 12 }}>
        <div>
          <h1 style={{ fontSize: 22, fontWeight: 900, color: "var(--text-primary)", margin: 0, letterSpacing: "-.4px" }}>Dashboard</h1>
          <p style={{ fontSize: 12, color: "var(--text-muted)", marginTop: 4 }}>Business overview &amp; key metrics</p>
        </div>
        <div style={{ display: "flex", alignItems: "center", gap: 10 }}>
          {loading && (
            <div style={{ display: "flex", alignItems: "center", gap: 8, padding: "6px 14px", borderRadius: 8, background: "rgba(99,102,241,.07)", border: "1px solid rgba(99,102,241,.2)" }}>
              <div style={{ width: 13, height: 13, border: "2px solid rgba(99,102,241,.3)", borderTopColor: "#6366f1", borderRadius: "50%", animation: "spin .7s linear infinite" }} />
      ...
      {btMeta && btMeta.id !== "trading" && (
        <div style={{ marginBottom: 18, padding: "12px 18px", borderRadius: 14, background: `${btMeta.color}08`, border: `1px solid ${btMeta.color}22`, display: "flex", alignItems: "center", gap: 12, flexWrap: "wrap" }}>
          <span style={{ fontSize: 22 }}>{btMeta.icon}</span>
          <div style={{ flex: 1 }}>
            <div style={{ fontSize: 13, fontWeight: 700, color: btMeta.color }}>{btMeta.label} Dashboard</div>
            <div style={{ fontSize: 11, color: "var(--text-muted)", marginTop: 1 }}>{btMeta.tagline}</div>
          </div>
          <div className="db-biz-kpis" style={{ display: "flex", gap: 8 }}>
            {btMeta.kpis.slice(0, 4).map(kpi => (
              <div key={kpi.key} style={{ padding: "6px 12px", borderRadius: 9, background: "var(--panel-bg)", border: "1px solid var(--border)", textAlign: "center" }}>
                <div style={{ fontSize: 14 }}>{kpi.icon}</div>
                <div style={{ fontSize: 10, color: "var(--text-muted)", fontWeight: 600, marginTop: 2 }}>{kpi.label}</div>
              </div>
            ))}
          </div>
        </div>
      ...
      <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fit,minmax(200px,1fr))", gap: 14, marginBottom: 20 }}>
        <KpiCard label="Total Revenue"   value={`${cur} ${stats.sales.toLocaleString()}`}      icon="📈" color="#34d399" bg="rgba(52,211,153,.07)"  border="rgba(52,211,153,.2)"  />
        <KpiCard label="Total Purchases" value={`${cur} ${stats.purchases.toLocaleString()}`}  icon="📦" color="#818cf8" bg="rgba(129,140,248,.07)" border="rgba(129,140,248,.2)" />
        <KpiCard
          label="Net Profit"
          value={`${cur} ${stats.profit.toLocaleString()}`}
          icon={stats.profit >= 0 ? "💰" : "📉"}
          color={stats.profit >= 0 ? "#34d399" : "#f87171"}
          bg={stats.profit >= 0 ? "rgba(52,211,153,.07)" : "rgba(248,113,113,.07)"}
          border={stats.profit >= 0 ? "rgba(52,211,153,.2)" : "rgba(248,113,113,.2)"}
          sub={stats.profit < 0 ? "Net loss this period" : undefined}
        />
        <KpiCard label="Customers"       value={stats.customers.toString()}                    icon="👥" color="#fbbf24" bg="rgba(251,191,36,.07)"  border="rgba(251,191,36,.2)"  sub="Active accounts" />
      </div>
      ...
      {/* ── Alert Row ── */}
      <div className="db-alert-grid" style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 14, marginBottom: 20 }}>
        {/* Overdue Receivables */}
        <div style={{ borderRadius: 14, padding: "18px 20px", background: "rgba(248,113,113,.07)", border: "1px solid rgba(248,113,113,.2)", display: "flex", alignItems: "center", justifyContent: "space-between", gap: 12 }}>
          <div>
            <div style={{ fontSize: 11, color: "rgba(248,113,113,.8)", fontWeight: 700, textTransform: "uppercase", letterSpacing: ".06em", marginBottom: 6 }}>
              Overdue Receivables {stats.overdueReceivablesCount > 0 ? `(${stats.overdueReceivablesCount})` : ""}
            </div>
        ...
        {/* Low Stock */}
        <div style={{ borderRadius: 14, padding: "18px 20px", background: "rgba(251,191,36,.07)", border: "1px solid rgba(251,191,36,.2)", display: "flex", alignItems: "center", justifyContent: "space-between", gap: 12 }}>
          <div>
            <div style={{ fontSize: 11, color: "rgba(251,191,36,.8)", fontWeight: 700, textTransform: "uppercase", letterSpacing: ".06em", marginBottom: 6 }}>Low Stock Alert</div>
      ...
      {companyInfo && currentUser?.role === "ADMIN" && (
        <div style={{
          ...
          background: subStatus === "TRIALING" ? "rgba(251,191,36,.06)" : "var(--panel-bg)",
          border: subStatus === "TRIALING" ? "1px solid rgba(251,191,36,.25)" : "1px solid var(--border)",
        }}>
          <div style={{ display: "flex", alignItems: "center", gap: 8 }}>
            <div style={{ width: 8, height: 8, borderRadius: "50%", background: subStatus === "ACTIVE" ? "#34d399" : "#f59e0b", flexShrink: 0 }} />
            <span style={{ fontSize: 12, color: "var(--text-muted)" }}>Plan: <strong style={{ color: "#a5b4fc" }}>{companyInfo.plan}</strong></span>
          </div>
          <span style={{ fontSize: 12, color: "var(--text-muted)" }}>
            Status: <strong style={{ color: subStatus === "ACTIVE" ? "#34d399" : "#f59e0b" }}>{subStatus}</strong>
          </span>
          {subStatus === "TRIALING" && (
            <span style={{ fontSize: 11, color: "rgba(251,191,36,.8)" }}>⚠️ Trial active — upgrade to unlock all features</span>
          )}
      ...
      {/* ══════════════════════════════════════════════════════════
          CHARTS
      ══════════════════════════════════════════════════════════ */}
      <div className="db-chart-grid" style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 16, marginBottom: 20 }}>

        {/* 1. Revenue vs Purchases Trend */}
        ...
        {/* 2. Top Customers */}
        <ChartCard>
          <SectionHeader title="Top Customers" sub="By total revenue this month" />
          {topCustomers.length === 0 ? (
            <div style={{ height: 180, display: "flex", flexDirection: "column", alignItems: "center", justifyContent: "center", gap: 8 }}>
              <span style={{ fontSize: 32 }}>👥</span>
              <div style={{ fontSize: 12, color: "var(--text-muted)", textAlign: "center" }}>No customer sales yet</div>
            </div>
          ) : (
        ...
        <ChartCard>
          ...
          {topItems.length === 0 ? (
            <div style={{ height: 180, display: "flex", flexDirection: "column", alignItems: "center", justifyContent: "center", gap: 8 }}>
              <span style={{ fontSize: 32 }}>📦</span>
              <div style={{ fontSize: 12, color: "var(--text-muted)", textAlign: "center" }}>No items sold yet</div>
            </div>
          ) : (
            <ResponsiveContainer width="100%" height={180}>
              <BarChart data={topItems} margin={{ top: 4, right: 4, left: -20, bottom: 20 }}>
                <CartesianGrid strokeDasharray="3 3" stroke="var(--border)" />
                <XAxis dataKey="name" tick={{ fill: "var(--text-muted)", fontSize: 9 } as any} axisLine={false} tickLine={false} angle={-30} textAnchor="end" />
                <YAxis tick={{ fill: "var(--text-muted)", fontSize: 10 } as any} axisLine={false} tickLine={false} />
        ...
        return (
          <div style={{ marginBottom: 20 }}>
            <div style={{ display: "flex", alignItems: "center", gap: 10, marginBottom: 12 }}>
              ...
              {btMeta && (
                <div style={{ fontSize: 10, padding: "2px 10px", borderRadius: 20, background: `${accentColor}12`, border: `1px solid ${accentColor}25`, color: accentColor, fontWeight: 700 }}>
