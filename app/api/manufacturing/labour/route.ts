...
export async function PATCH(req: NextRequest) {
  try {
    ...
    // The ledger account carries the worker's name, so a rename that stopped at
    // the labour row would leave the chart of accounts showing the old one.
    if (accountId) {
      await prisma.account.updateMany({
        where: { id: accountId, companyId },
        data: { name: `${name} — Labour Payable` },
...
export async function DELETE(req: NextRequest) {
  try {
    ...
    // A worker who has been charged to a run or paid owns ledger history;
    // deleting the row would leave posted vouchers pointing at an account
    // nobody can name. Only a worker who never traded can be removed.
    if (accountId) {
      const account = await prisma.account.findFirst({
        where: { id: accountId, companyId },
        select: { openDebit: true, openCredit: true },
      ...
      const hasOpening =
        Number(account?.openDebit || 0) !== 0 || Number(account?.openCredit || 0) !== 0;
      if (entries > 0 || hasOpening) {
        return NextResponse.json(
          {
            error:
              "This worker already has ledger entries, so deleting them would break posted vouchers. Reverse or clear their ledger first.",
          },
          { status: 409 },
        );
      }
    ...
    await prisma.businessRecord.deleteMany({ where: { id, companyId, category: "labour" } });
    if (accountId) await prisma.account.deleteMany({ where: { id: accountId, companyId } });
